...
export const GET: RequestHandler = async ({ url, fetch }) => {
  ...
  for (let i = 0; i < FOREST_LEAF_COUNT; i++) {
    leaves.push({
      x: Math.random() * 100,
      y: Math.random() * 100,
  ...
  // Create the OG image HTML with forest scene
  const html = `
    <div style="display: flex; width: 100%; height: 100%; background: linear-gradient(180deg, #fed7aa 0%, #fdba74 50%, #fb923c 100%); font-family: 'Lexend', sans-serif;">
      <div style="display: flex; align-items: center; justify-content: center; width: 300px; height: 100%; background: rgba(120, 53, 15, 0.1); border-right: 2px solid rgba(120, 53, 15, 0.2);">
        <div style="display: flex; width: 180px; height: 180px;">
          ${logoSvg}
        </div>
      </div>
      <div style="display: flex; position: relative; width: 900px; height: 100%; overflow: hidden;">
        ...
        <div style="position: absolute; bottom: 40px; left: 40px; right: 40px; display: flex; flex-direction: column;">
          <div style="font-size: 64px; font-weight: 700; color: #78350f; margin-bottom: 12px; text-shadow: 2px 2px 4px rgba(255, 255, 255, 0.3);">
            The Forest
          </div>
          <div style="font-size: 28px; color: #92400e; text-shadow: 1px 1px 2px rgba(255, 255, 255, 0.3);">
  ...
  // Stale-while-revalidate for random generation
