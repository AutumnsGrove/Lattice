...
type GroveTermCategory = "foundational" | "platform" | "content" | "tools" | "operations";
 ...
 */
function findRelatedTerms(content: string, allTerms: Set<string>, currentTerm: string): string[] {
...
function parseGroveNaming(content: string): GroveTermManifest {
	...
	function commitTerm() {
		...
		const contentForRelated = termLines
			...
			.join("\n");
		const seeAlso = findRelatedTerms(contentForRelated, allTermSlugs, displayTerm);
...
async function main() {
	...
	// Output directory
	const outputDir = join(projectRoot, "libs/engine/src/lib/data");
