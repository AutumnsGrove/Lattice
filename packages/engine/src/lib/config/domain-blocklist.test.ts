...
describe('getBlockedMessage', () => {
	it('should return correct message for system', () => {
		expect(getBlockedMessage('system')).toBe('This username is reserved for system use');
	});

	it('should return correct message for grove_service', () => {
		expect(getBlockedMessage('grove_service')).toBe('This username is reserved for a Grove service');
	});
	...
	it('should return generic message for impersonation', () => {
		expect(getBlockedMessage('impersonation')).toBe('This username is not available');
	});

	it('should return generic message for offensive', () => {
		expect(getBlockedMessage('offensive')).toBe('This username is not available');
	});
	...
	it('should return correct message for future_reserved', () => {
		expect(getBlockedMessage('future_reserved')).toBe('This username is reserved');
	});

	it('should return generic message for unknown reasons', () => {
		// Type assertion to test edge case
		expect(getBlockedMessage('unknown' as BlocklistReason)).toBe('This username is not available');
	});
});
...
describe('Edge Cases', () => {
	...
	it('should not have duplicates in COMPLETE_BLOCKLIST', () => {
		...
		expect(uniqueUsernames.size).toBeGreaterThan(COMPLETE_BLOCKLIST.length * 0.9);
	});
});
