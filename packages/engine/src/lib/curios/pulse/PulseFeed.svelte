...
<div class="pulse-feed" role="feed" aria-label="Development event feed">
  ...
  <div class="events-list">
    {#each filteredEvents as event (event.id)}
      <article class="event-item" aria-label="{getLabel(event.eventType, event.action, event.data)} in {event.repoName}">
        <div class="event-icon" style="color: {getColor(event.eventType, event.action, event.data)}">
          <svelte:component this={getIcon(event.eventType, event.action, event.data)} size={18} />
        </div>

        <div class="event-content">
          <div class="event-header">
            <span class="event-type" style="color: {getColor(event.eventType, event.action, event.data)}">
    ...
    {:else}
      <p class="empty-feed">
        {activeFilters.size > 0 ? "No events match the current filter." : "No events yet. Push some code!"}
      </p>
    {/each}
  </div>
