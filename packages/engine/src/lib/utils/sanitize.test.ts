...
import { sanitizeHTML, sanitizeSVG, sanitizeMarkdown, sanitizeURL } from './sanitize';
...
describe('Sanitization Security Tests', () => {
	...
	describe('Event Handler Injection', () => {
		...
		it.each(eventPayloads)('sanitizeHTML removes event handlers: %s', (payload) => {
			const result = sanitizeHTML(payload);
			expect(result.toLowerCase()).not.toMatch(/on\w+\s*=/i);
		});

		it.each(eventPayloads)('sanitizeMarkdown removes event handlers: %s', (payload) => {
			const result = sanitizeMarkdown(payload);
			expect(result.toLowerCase()).not.toMatch(/on\w+\s*=/i);
		});
	});
	...
	describe('JavaScript Protocol Injection', () => {
		...
		it.each(protocolPayloads)('sanitizeHTML blocks javascript: protocol: %s', (payload) => {
			const result = sanitizeHTML(payload);
	...
	describe('Object/Embed Injection', () => {
		...
		it.each(objectPayloads)('sanitizeHTML removes object/embed: %s', (payload) => {
			const result = sanitizeHTML(payload);
	...
	describe('Style/Link Injection', () => {
		...
		it.each(stylePayloads)('sanitizeHTML removes style/link elements: %s', (payload) => {
			const result = sanitizeHTML(payload);
	...
	describe('Form Injection', () => {
		...
		it.each(formPayloads)('sanitizeHTML removes form elements: %s', (payload) => {
			const result = sanitizeHTML(payload);
	...
	describe('SVG XSS Prevention', () => {
		...
		it.each(svgPayloads)('sanitizeSVG removes dangerous content: %s', (payload) => {
	...
	describe('Nested and Malformed Tags', () => {
		...
		// Note: These are known SSR regex sanitizer limitations
		// In browser mode, DOMPurify handles these edge cases properly
		// The SSR sanitizer prioritizes common attack vectors
		it.skip('handles unclosed tags (browser-only)', () => {
			// This edge case is handled by DOMPurify in browser
...
ipt>`;
	...
	describe('Markdown Content Sanitization', () => {
		...
		it('preserves images', () => {
			const imgHtml = '<img src="https://example.com/image.png" alt="Alt text">';
		...
		it('removes script in markdown', () => {
			const malicious = '<p>Safe text</p><script>alert(1)</script><p>More safe text</p>';
