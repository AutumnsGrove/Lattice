...
describe("Rate Limiter", () => {
  ...
  describe("clear", () => {
    ...
    it("should clear all keys for a type", () => {
      ...
      expect(limiter.getRemaining("subscription", "user-123")?.remaining).toBe(
        59,
      );
    });
    ...
    it("should clear all limits when called with no arguments", () => {
      ...
      expect(limiter.getRemaining("token", "user-123")?.remaining).toBe(10);
      expect(limiter.getRemaining("subscription", "user-456")?.remaining).toBe(
        60,
      );
    });
  });
...
describe("withRateLimit", () => {
  ...
  it("should throw RateLimitError when over limit", async () => {
    ...
    const wrapped = withRateLimit(limiter, "token", () => "user", mockFn);
  ...
  it("should use getKey function to determine rate limit key", async () => {
    const mockFn = vi.fn(async (userId: string, resource: string) => ({
      userId,
      resource,
    }));
    const wrapped = withRateLimit(
    ...
    // user-1 should be blocked
    await expect(wrapped("user-1", "resource-a")).rejects.toThrow(
      RateLimitError,
    );
