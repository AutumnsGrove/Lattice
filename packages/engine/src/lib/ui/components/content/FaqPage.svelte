			...
			<Accordion items={gettingStartedItems} type="single" collapsible class="w-full">
				{#snippet contentSnippet(item)}
					...
					{:else if item.value === 'is-grove-free'}
						<GroveSwap term="your-grove">Grove</GroveSwap> offers a free tier so you can start writing right away. If you want more features -- like custom domains, extra storage, or advanced customization -- our <a href="https://grove.place/pricing" class="text-emerald-700 dark:text-emerald-400 underline underline-offset-2">paid plans</a> (<GroveTerm term="seedling" />, <GroveTerm term="sapling" />, <GroveTerm term="oak" />, and <GroveTerm term="evergreen" />) grow with you.
			...
			<Accordion items={troubleshootingItems} type="single" collapsible class="w-full">
				{#snippet contentSnippet(item)}
					{#if item.value === 'blog-not-loading'}
						First, check our <GroveTerm term="clearing" href="https://status.grove.place">status page</GroveTerm> to see if there's a known issue. If everything looks normal, try clearing your browser cache and refreshing. If it persists, <a href="https://grove.place/contact" class="text-emerald-700 dark:text-emerald-400 underline underline-offset-2">reach out to us</a> -- we're here to help.
					...
					{:else if item.value === 'images-not-uploading'}
						Check that your image is under the size limit for your plan and in a supported format (JPG, PNG, WebP, GIF). If uploads keep failing, it may be a temporary storage issue -- try again in a few minutes or <a href="https://grove.place/contact" class="text-emerald-700 dark:text-emerald-400 underline underline-offset-2">contact support</a>.
					{:else if item.value === 'contact-support'}
						You can reach us through the <a href="https://grove.place/contact" class="text-emerald-700 dark:text-emerald-400 underline underline-offset-2">contact page</a>, or email us directly. We're a small team, so responses might take a day, but we read everything and genuinely care about helping you.
					{/if}
				{/snippet}
			</Accordion>
		</GlassCard>
