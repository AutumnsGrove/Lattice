<script lang="ts">
	/**
	 * GlassLogoArchive - Legacy Grove Asterisk Logo with Glassmorphism (Archived)
	 *
	 * This is the original asterisk-style Grove logo with glass effects,
	 * preserved for backwards compatibility and historical reference.
	 *
	 * For new implementations, use the updated GlassLogo component which
	 * features the new tree design with seasonal variants.
	 *
	 * @deprecated Use GlassLogo.svelte for new implementations
	 *
	 * @example Basic glass logo
	 * ```svelte
	 * <GlassLogoArchive class="w-16 h-20" />
	 * ```
	 *
	 * @example Seasonal glass logo
	 * ```svelte
	 * <GlassLogoArchive season="winter" class="w-16 h-20" />
	 * <GlassLogoArchive season="spring" breathing />
	 * <GlassLogoArchive season="autumn" variant="frosted" />
	 * ```
	 */
	...
	const randomId = `glass-logo-archive-${Math.random().toString(36).slice(2, 9)}`;
