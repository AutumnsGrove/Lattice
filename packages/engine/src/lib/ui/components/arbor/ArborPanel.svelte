...
<div class="arbor-layout" class:leaf-pattern={showLeafPattern}>
	...
	>
		<ArborSidebarHeader {brandTitle} {showLogo} {showExpanded} customHeader={sidebarHeader} />

		<ArborNav
			items={navItems}
			{showExpanded}
			{userPermissions}
			navLabel="{brandTitle} navigation"
		/>
	...
	<main id="arbor-main" class="arbor-content" class:expanded={sidebarCollapsed}>
		{#if messages?.length}
...
<style>
	...
	.arbor-skip-link {
		...
		border-radius: 0 0 var(--border-radius-button, 6px) 0;
		transition: top 0.1s;
	}

	.arbor-skip-link:focus {
		top: 0;
	}
