...
describe("checkRateLimit", () => {
  ...
  it("fails closed on KV errors when failClosed is true", async () => {
    // Create a KV that always throws
    const errorKV = {
      ...mockKV,
      get: vi.fn().mockRejectedValue(new Error("KV unavailable")),
    ...
    const { result, response } = await checkRateLimit({
      kv: errorKV,
      key: "test:user-closed",
      limit: 5,
      windowSeconds: 60,
      failClosed: true,
    });

    // Should reject request when KV is unavailable
    expect(result.allowed).toBe(false);
    expect(result.remaining).toBe(0);
    expect(response).toBeDefined();
...
describe("buildRateLimitKey", () => {
  it("combines endpoint and identifier", () => {
    expect(buildRateLimitKey("posts/create", "user123")).toBe(
      "posts/create:user123",
    );
    expect(buildRateLimitKey("auth/login", "192.168.1.1")).toBe(
      "auth/login:192.168.1.1",
    );
  });

  it("handles special characters", () => {
    expect(buildRateLimitKey("api/v1", "user@example.com")).toBe(
      "api/v1:user@example.com",
    );
  });
});
