...
describe("checkTenantRateLimit", () => {
  ...
  it("uses namespaced key for each tenant", async () => {
    ...
    const aliceResult = await checkTenantRateLimit(
      mockKV,
      "alice",
      "seedling",
      "requests",
    );
    const bobResult = await checkTenantRateLimit(
      mockKV,
      "bob",
      "seedling",
      "requests",
    ...
    expect(aliceResult.remaining).toBe(
      TIER_RATE_LIMITS.seedling.requests.limit - 2,
    );
    expect(bobResult.remaining).toBe(
      TIER_RATE_LIMITS.seedling.requests.limit - 2,
    );
  });
});
...
describe("getTenantLimitInfo", () => {
  ...
  it("returns different limits for different tiers", () => {
    ...
    expect(evergreenLimits.requests.limit).toBeGreaterThan(
      seedlingLimits.requests.limit,
    );
