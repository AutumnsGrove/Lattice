...
describe('Database Service', () => {
	...
	describe('queryOne', () => {
		...
		it('should return null when no rows match', async () => {
			...
			const result = await queryOne(db, 'SELECT * FROM users WHERE id = ?', ['nonexistent']);
		...
		it('should throw DatabaseError on query failure', async () => {
			...
			await expect(queryOne(db, 'SELECT * FROM users', [])).rejects.toThrow(DatabaseError);
		});
	});
	...
	describe('queryOneOrThrow', () => {
		...
		it('should throw DatabaseError when not found', async () => {
			...
			await expect(
				queryOneOrThrow(db, 'SELECT * FROM users WHERE id = ?', ['nonexistent'])
		...
		it('should use custom error message', async () => {
			...
			await expect(
				queryOneOrThrow(db, 'SELECT * FROM users WHERE id = ?', ['1'], 'User not found')
	...
	describe('queryMany', () => {
		it('should return multiple rows', async () => {
			...
			const results = await queryMany<{ id: string }>(db, 'SELECT * FROM users', []);
		...
		it('should return empty array when no rows match', async () => {
			...
			const results = await queryMany(db, 'SELECT * FROM users WHERE id = ?', ['nonexistent']);
	...
	describe('insert', () => {
		it('should insert a row with generated id and timestamps', async () => {
			const id = await insert(db, 'users', { email: 'test@example.com', name: 'Test' });
		...
		it('should use provided id when specified', async () => {
			...
			const id = await insert(db, 'users', { email: 'test@example.com' }, { id: customId });
		...
		it('should throw on duplicate key constraint', async () => {
			...
			await expect(insert(db, 'users', { email: 'test@example.com' })).rejects.toThrow(
				DatabaseError
			);
		});

		it('should reject invalid table names', async () => {
			await expect(insert(db, 'users; DROP TABLE users;--', { email: 'x' })).rejects.toThrow(
				DatabaseError
			);
			await expect(insert(db, '123invalid', { email: 'x' })).rejects.toThrow(DatabaseError);
		});
	});

	describe('update', () => {
		it('should update matching rows', async () => {
			seedMockD1(db, 'users', [{ id: '1', email: 'old@example.com', name: 'Old' }]);

			const changes = await update(db, 'users', { email: 'new@example.com' }, 'id = ?', ['1']);
		...
		it('should reject invalid table names', async () => {
			await expect(update(db, 'DROP TABLE', { name: 'x' }, 'id = ?', ['1'])).rejects.toThrow(
				DatabaseError
			);
		});
	});
	...
	describe('deleteWhere', () => {
		...
		it('should reject invalid table names', async () => {
			await expect(deleteWhere(db, '1invalid', 'id = ?', ['1'])).rejects.toThrow(DatabaseError);
		});
	});
	...
	describe('exists', () => {
		...
		it('should reject invalid table names', async () => {
			await expect(exists(db, 'SELECT * FROM', 'id = ?', ['1'])).rejects.toThrow(DatabaseError);
		});
	});
	...
	describe('batch', () => {
		it('should execute multiple statements atomically', async () => {
			const results = await batch(db, [
				{ sql: 'INSERT INTO users (id, email) VALUES (?, ?)', params: ['1', 'a@test.com'] },
				{ sql: 'INSERT INTO users (id, email) VALUES (?, ?)', params: ['2', 'b@test.com'] }
			]);
	...
	describe('withSession', () => {
		it('should execute operations within a session', async () => {
			...
			const result = await withSession(db, async (session) => {
				const user = await queryOne<{ id: string }>(session, 'SELECT * FROM users WHERE id = ?', [
					'1'
				]);
				return user;
			});
	...
	describe('SQL Injection Prevention', () => {
		it('should reject table names with special characters', async () => {
			...
			for (const name of maliciousNames) {
				await expect(insert(db, name, { data: 'test' })).rejects.toThrow(DatabaseError);
			}
		});

		it('should allow valid table names', async () => {
			const validNames = ['users', 'user_sessions', 'UserProfiles', '_private', 'table123'];
