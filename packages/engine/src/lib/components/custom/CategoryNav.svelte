...
>
  {#each sections as section}
    ...
    <div class="relative group">
      ...
      >
        {#if SectionIcon}
          <SectionIcon class="w-5 h-5 {colors.iconText} group-hover:scale-110 motion-reduce:group-hover:scale-100 transition-transform motion-reduce:transition-none" />
        {:else}
      ...
      {#if hasHoverItems(section)}
        <div class="absolute right-full mr-3 top-1/2 -translate-y-1/2 opacity-0 invisible group-hover:opacity-100 group-hover:visible group-focus-within:opacity-100 group-focus-within:visible transition-all duration-200 motion-reduce:transition-none flex items-center gap-2 flex-wrap justify-end max-w-xs">
          ...
          {#if sectionItems.length > maxHoverItems}
            <span class="text-xs text-foreground-muted">+{sectionItems.length - maxHoverItems} more</span>
          {/if}
        </div>
      {/if}
    </div>
