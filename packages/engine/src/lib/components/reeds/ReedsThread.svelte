	...
	async function submitEdit(commentId: string, content: string) {
		try {
			const res = await fetch(`/api/reeds/${slug}/${commentId}`, {
				// csrf-ok: origin fallback, migrate to api.*()
				method: "PATCH",
...
{#if commentsEnabled}
	<section class="reeds-section" id="reeds" aria-label="Comments">
		...
		{:else}
			<p class="reeds-empty">No comments yet. Be the first to share your thoughts.</p>
		{/if}

		<!-- Main comment form -->
		{#if isLoggedIn}
			<div class="reeds-form-wrapper">
				<ReedsCommentForm {slug} allowPublic={publicEnabled} onsubmitted={refreshComments} />
			</div>
		{:else}
