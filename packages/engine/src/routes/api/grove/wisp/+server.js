...
export async function POST({ request, platform, locals }) {
	...
	if (kv) {
		...
		if (!rateResult.allowed) {
			return json(
				{ error: 'Rate limit exceeded. Try again in an hour.', resetAt: rateResult.resetAt },
				{ status: 429 }
			);
		}
	}
	...
	if (!hasProvider && (action === 'grammar' || action === 'tone' || action === 'all')) {
	...
	try {
		...
		if (action === 'grammar' || action === 'all') {
			const grammarResult = await analyzeGrammar(truncatedContent, mode, secrets);
		...
		if (action === 'tone' || action === 'all') {
			const toneResult = await analyzeTone(truncatedContent, mode, secrets, context);
		...
		// Calculate cost
		const cost = modelUsed ? calculateCost(modelUsed, totalTokens.input, totalTokens.output) : 0;
		...
		console.error('[Wisp] Analysis error:', err instanceof Error ? err.message : 'Unknown error');
 ...
 */
async function analyzeGrammar(content, mode, secrets) {
 ...
 */
async function analyzeTone(content, mode, secrets, context) {
	const modeConfig = PROMPT_MODES[mode];
