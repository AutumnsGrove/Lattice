			...
			<Accordion items={gettingStartedItems} type="single" collapsible class="w-full">
				{#snippet contentSnippet(item)}
					...
					{:else if item.value === 'how-do-i-sign-up'}
						Head to grove.place and sign in with your Google account. That's it -- no lengthy forms, no passwords to remember. Once you're in, you can pick a username and start setting up your <GroveSwap term="your-garden">garden</GroveSwap>.
					...
					{:else if item.value === 'is-grove-free'}
						<GroveSwap term="your-grove">Grove</GroveSwap> offers a free tier so you can start writing right away. If you want more features -- like custom domains, extra storage, or advanced customization -- our paid plans (<GroveTerm term="seedling" />, <GroveTerm term="sapling" />, <GroveTerm term="oak" />, and <GroveTerm term="evergreen" />) grow with you.
			...
			<Accordion items={troubleshootingItems} type="single" collapsible class="w-full">
				{#snippet contentSnippet(item)}
					{#if item.value === 'blog-not-loading'}
						First, check our <GroveTerm term="clearing">status page</GroveTerm> to see if there's a known issue. If everything looks normal, try clearing your browser cache and refreshing. If it persists, reach out to us -- we're here to help.
