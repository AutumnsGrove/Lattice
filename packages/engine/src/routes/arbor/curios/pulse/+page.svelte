  ...
  <header class="page-header">
    ...
    <div class="header-content">
      ...
      <p class="subtitle">
        ...
        Real-time activity — commits flowing, PRs merging, issues moving.
      </p>
    </div>
  </header>
    ...
    use:enhance={() => {
      ...
      return async ({ result, update }) => {
        ...
        if (result.type === "success") {
          toast.success("Configuration saved!", { description: "Your Pulse settings have been updated." });
  ...
  >
    ...
    <GlassCard class="config-section">
      ...
      <div class="toggle-row">
        <label class="toggle-label">
          <input type="checkbox" name="enabled" value="true" bind:checked={enabled} class="toggle-input" />
          <span class="toggle-switch"></span>
          <span class="toggle-text">Enable Pulse</span>
        </label>
        <p class="field-help">
          When enabled, your /pulse page shows live development activity.
        </p>
      </div>
    ...
    <GlassCard class="config-section">
      ...
      <div class="field-group">
        ...
        <div class="copy-field">
          ...
          <button type="button" class="copy-btn" onclick={() => copyToClipboard(data.webhookUrl, "url")}>
            {#if urlCopied}
      ...
      {#if generatedSecret}
        <div class="field-group">
          ...
          <div class="copy-field secret-field">
            ...
            <button type="button" class="copy-btn" onclick={() => copyToClipboard(generatedSecret!, "secret")}>
              {#if secretCopied}<CheckCircle2 size={14} />{:else}<Copy size={14} />{/if}
            </button>
          </div>
          <p class="field-help warning">This secret won't be shown again. Copy it now and paste into GitHub.</p>
        </div>
      {:else if data.hasWebhookSecret}
        <div class="field-group">
          ...
          <p class="field-help">Regenerating will invalidate the current secret. You'll need to update GitHub.</p>
        </div>
      {:else}
      ...
      <div class="setup-steps">
        ...
        <ol>
          ...
          <li>Select events: Pushes, Pull requests, Issues, Releases, Workflow runs, Stars, Forks</li>
      ...
      <div class="status-grid">
        ...
        {#if data.lastEvent}
          <div class="status-item">
            ...
            <span class="status-value">{data.lastEvent.event_type} — {new Date(data.lastEvent.occurred_at * 1000).toLocaleString()}</span>
          </div>
        {/if}
      </div>
    </GlassCard>
    ...
    <GlassCard class="config-section">
      ...
      <div class="field-group">
        ...
        <input type="text" id="reposInclude" name="reposInclude" bind:value={reposInclude} placeholder="repo1, repo2, repo3" class="field-input" />
      ...
      <div class="field-group">
        ...
        <input type="text" id="reposExclude" name="reposExclude" bind:value={reposExclude} placeholder="private-notes, dotfiles" class="field-input" />
    ...
    <GlassCard class="config-section">
      ...
      <div class="toggle-grid">
        <label class="toggle-label">
          <input type="checkbox" name="showHeatmap" value="true" bind:checked={showHeatmap} class="toggle-input" />
          <span class="toggle-switch"></span>
          <span class="toggle-text">Activity Heatmap</span>
        </label>
        <label class="toggle-label">
          <input type="checkbox" name="showFeed" value="true" bind:checked={showFeed} class="toggle-input" />
          <span class="toggle-switch"></span>
          <span class="toggle-text">Event Feed</span>
        </label>
        <label class="toggle-label">
          <input type="checkbox" name="showStats" value="true" bind:checked={showStats} class="toggle-input" />
          <span class="toggle-switch"></span>
          <span class="toggle-text">Stats Cards</span>
        </label>
        <label class="toggle-label">
          <input type="checkbox" name="showTrends" value="true" bind:checked={showTrends} class="toggle-input" />
          <span class="toggle-switch"></span>
          <span class="toggle-text">Trend Charts</span>
        </label>
        <label class="toggle-label">
          <input type="checkbox" name="showCi" value="true" bind:checked={showCi} class="toggle-input" />
          <span class="toggle-switch"></span>
...
<style>
  ...
  :global(.back-icon) { width: 1rem; height: 1rem; }
  ...
  .alert-error { background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); color: var(--color-error, #ef4444); }
  .alert-success { background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); color: var(--color-success, #22c55e); }
  :global(.alert-icon) { width: 1.25rem; height: 1.25rem; flex-shrink: 0; }

  :global(.config-section) { padding: 1.5rem; margin-bottom: 1.5rem; }
  ...
  :global(.section-icon) { width: 1.5rem; height: 1.5rem; color: var(--color-primary); }
  .section-header h2 { font-size: 1.25rem; font-weight: 600; margin: 0; color: var(--color-text); }
  ...
  .field-help { font-size: 0.8rem; color: var(--color-text-muted); margin-top: 0.5rem; line-height: 1.5; }
  .field-help.warning { color: #f59e0b; }
  .field-help code { background: var(--grove-overlay-8); padding: 0.125rem 0.375rem; border-radius: 0.25rem; font-size: 0.75rem; }
  ...
  .toggle-switch {
    ...
    background: var(--grove-overlay-12); border-radius: 1rem; transition: background 0.2s;
  }
  .toggle-switch::after {
    content: ""; position: absolute; top: 0.125rem; left: 0.125rem;
    width: 1.25rem; height: 1.25rem; background: white; border-radius: 50%; transition: transform 0.2s;
  }
  .toggle-input:checked + .toggle-switch { background: var(--color-primary); }
  .toggle-input:checked + .toggle-switch::after { transform: translateX(1.5rem); }
  .toggle-input:focus-visible + .toggle-switch { outline: 2px solid var(--color-primary); outline-offset: 2px; }
  .toggle-text { font-weight: 500; color: var(--color-text); }

  .toggle-grid { display: flex; flex-direction: column; gap: 1rem; }
  ...
  :global(.text-green) { color: #22c55e; }
  ...
  .regen-btn:hover:not(:disabled) { background: rgba(239, 68, 68, 0.15); }
  ...
  .setup-steps h3 { font-size: 0.9rem; font-weight: 600; margin: 0 0 0.75rem; color: var(--color-text); }
  .setup-steps ol { margin: 0; padding-left: 1.25rem; }
  .setup-steps li { font-size: 0.85rem; color: var(--color-text-muted); padding: 0.25rem 0; line-height: 1.5; }
  ...
  .status-label { font-size: 0.85rem; color: var(--color-text-muted); }
  .status-value { font-size: 0.85rem; color: var(--color-text); display: flex; align-items: center; gap: 0.5rem; }

  .health-dot { width: 0.5rem; height: 0.5rem; border-radius: 50%; display: inline-block; }
  .health-green { background: #22c55e; }
  .health-yellow { background: #f59e0b; }
  .health-red { background: #ef4444; }
  .health-gray { background: #6b7280; }

  .form-actions { display: flex; align-items: center; justify-content: flex-end; gap: 1rem; margin-top: 2rem; }
  :global(.button-icon) { width: 1.125rem; height: 1.125rem; margin-right: 0.5rem; }
