			...
			{#if searchQuery}
				...
				<div aria-live="polite" aria-atomic="true" class="mb-6">
					<p class="text-foreground-muted">
						Found {filteredResults.length} result{filteredResults.length !== 1 ? "s" : ""} for "<span
							class="font-medium">{searchQuery}</span
						>"
					</p>
				...
				{#if filteredResults.length > 0}
					<div class="space-y-4">
						{#each filteredResults as doc}
							<article
								class="bg-surface-elevated rounded-lg shadow-sm border border-default p-6 hover:shadow-md transition-shadow"
							>
								<div class="flex items-start justify-between mb-2">
									<div class="flex-1">
										<h2 class="text-lg font-semibold text-foreground mb-1">
											<a
												href="/knowledge/{doc.category}/{doc.slug}"
								...
								<div class="flex items-center justify-between mt-4">
									<a
										href="/knowledge/{doc.category}/{doc.slug}"
										class="text-accent hover:text-accent-muted font-medium text-sm transition-colors"
									>
										Read more
										<svg
				...
				{:else}
					...
					<div class="text-center py-12">
						<svg
							class="w-16 h-16 text-foreground-faint mx-auto mb-4"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
						...
						<p class="text-foreground-muted mb-4">
							We couldn't find any documentation matching your search. Try different keywords or
							browse our categories.
						</p>
						<div class="flex gap-4 justify-center">
							<a
								href="/knowledge/specs"
								class="px-4 py-2 bg-accent/10 text-accent rounded-lg hover:bg-accent/20 transition-colors"
							>
								Browse Specs
							</a>
							<a
