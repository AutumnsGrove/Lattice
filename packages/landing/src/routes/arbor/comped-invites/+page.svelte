...
<svelte:head>
	<title>Comped Invites - Grove Admin</title>
</svelte:head>
				...
				<div class="flex flex-col sm:flex-row gap-3">
					...
					{#if searchQuery || statusFilter || typeFilter}
						<button
							...
							class="px-4 py-2 rounded-lg bg-cream-200 dark:bg-cream-300 text-foreground-muted hover:text-foreground transition-colors"
						>
							<X class="w-4 h-4" />
						</button>
					{/if}
			...
			<GlassCard class="p-6">
				...
				>
					...
					<button
						...
						class="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-grove-600 text-white hover:bg-grove-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
					>
						{#if isAdding}
							<Loader2 class="w-4 h-4 animate-spin" />
							Creating...
				...
				<div class="space-y-3 max-h-80 overflow-y-auto">
					{#each data.auditLog as entry}
						<div class="p-2 rounded-lg bg-cream-100 dark:bg-cream-100/50 text-sm">
							...
							{#if entry.notes}
								<div class="text-xs text-foreground-muted mt-1 italic">
									"{entry.notes}"
								</div>
							{/if}
						</div>
