<script lang="ts">
	...
	import type { GroveMessage } from '@autumnsgrove/groveengine/ui';
	...
	let previewChannel = $state<'landing' | 'arbor' | null>(null);
	...
	let landingMessages = $derived(data.messages.filter((m) => m.channel === 'landing'));
	let arborMessages = $derived(data.messages.filter((m) => m.channel === 'arbor'));

	// Preview data — transforms DB row into component-compatible shape
	function toPreviewMessages(channel: 'landing' | 'arbor'): GroveMessage[] {
		return data.messages
			.filter((m) => m.channel === channel && m.published)
			.map((m) => ({
				id: m.id,
				title: m.title,
				body: m.body,
...
<div class="mb-8 flex items-center justify-between">
	<div>
		...
		<p class="text-foreground-muted font-sans mt-1">
			...
			{data.stats.byChannel.landing} landing · {data.stats.byChannel.arbor} arbor
		</p>
	</div>
	<button
		type="button"
...
<div class="mb-6 flex gap-2">
	<button
		type="button"
		onclick={() => (previewChannel = previewChannel === 'landing' ? null : 'landing')}
		class="text-xs font-sans px-3 py-1.5 rounded-lg transition-colors {previewChannel === 'landing'
	...
	<button
		type="button"
		onclick={() => (previewChannel = previewChannel === 'arbor' ? null : 'arbor')}
		class="text-xs font-sans px-3 py-1.5 rounded-lg transition-colors {previewChannel === 'arbor'
			? 'bg-grove-600 text-white'
			: 'bg-grove-100 dark:bg-slate-700 text-foreground-muted hover:text-foreground'}"
	>
		Preview Arbor
...
{#if previewChannel}
	...
	<div class="mb-8">
		...
		{#if preview.length > 0}
			<div class="max-w-2xl">
				<GroveMessages messages={preview} dismissible={previewChannel === 'arbor'} />
			</div>
		{:else}
...
<section class="mb-8">
	<h2 class="text-lg font-serif text-foreground mb-4">
		Landing Channel
		<span class="text-sm font-sans text-foreground-muted font-normal ml-2">{landingMessages.length}</span>
	</h2>

	{#if landingMessages.length === 0}
		<p class="text-sm font-sans text-foreground-muted">No landing messages yet.</p>
	{:else}
		<div class="space-y-3">
			{#each landingMessages as msg (msg.id)}
						...
						<form method="POST" action="?/update" use:enhance={() => {
							...
							<div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
								...
								<div class="grid grid-cols-2 gap-2">
									<select name="channel" value={msg.channel}
										class="px-3 py-2 text-sm font-sans border border-grove-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-foreground">
										<option value="landing">Landing</option>
										<option value="arbor">Arbor</option>
									</select>
...
<section>
	<h2 class="text-lg font-serif text-foreground mb-4">
		Arbor Channel
		<span class="text-sm font-sans text-foreground-muted font-normal ml-2">{arborMessages.length}</span>
	</h2>

	{#if arborMessages.length === 0}
		<p class="text-sm font-sans text-foreground-muted">No arbor messages yet.</p>
	{:else}
		<div class="space-y-3">
			{#each arborMessages as msg (msg.id)}
				{@const config = typeConfig[msg.message_type] || typeConfig.info}
				{@const status = statusLabel(msg)}
				{@const TypeIcon = config.icon}
				<GlassCard class="p-4">
