...
// Docs are at project root - landing is at packages/landing/
...
function validateFrontmatter(
  ...
  if (frontmatter.category === "specs") {
    if (!frontmatter.specCategory) {
      ...
      !VALID_SPEC_CATEGORIES.includes(frontmatter.specCategory as SpecCategory)
    ) {
  ...
  if (frontmatter.category === "help") {
    if (!frontmatter.section) {
      ...
      !VALID_HELP_SECTIONS.includes(frontmatter.section as HelpSection)
    ) {
  ...
  if (frontmatter.category === "exhibit") {
    if (!frontmatter.exhibitWing) {
      ...
      !VALID_EXHIBIT_WINGS.includes(frontmatter.exhibitWing as ExhibitWing)
    ) {
  ...
  if (frontmatter.lastUpdated) {
    ...
    if (!dateRegex.test(frontmatter.lastUpdated)) {
      warnings.push(
        `lastUpdated should be in YYYY-MM-DD format: ${frontmatter.lastUpdated}`,
      );
    }
  }
