<script lang="ts">
	...
	let seenDomainIds = $state<Set<string>>(
		untrack(() => new Set(data.results.map((r) => r.domain))),
	);
	let newDomainIds = $state<Set<string>>(new Set());
	let domainIdeaStatus = $state<{
		available: boolean;
		checked: boolean;
		price_cents?: number;
	} | null>(null);
	...
	async function fetchFollowupQuiz() {
		console.log(
			`[History Page] fetchFollowupQuiz called for job_id: ${job?.id}, status: ${job?.status}`,
		);
		...
		try {
			...
			if (response.ok) {
				...
				if (!data || !data.questions || !Array.isArray(data.questions)) {
					...
					followupError =
						"No follow-up questions available for this search. The search may continue without additional input.";
					return;
				}
				...
				console.log(
					`[History Page] Followup quiz loaded successfully with ${data.questions?.length || 0} questions`,
				);
			} else {
				...
				if (errorText.includes("No follow-up quiz available")) {
					followupError =
						"No additional questions are needed for this search. You can continue the search as-is or try different search parameters.";
				} else {
			...
			if (err instanceof Error && err.name === "AbortError") {
				...
				followupError =
					err instanceof Error ? err.message : "Unknown error occurred while loading questions.";
			}
		} finally {
			clearTimeout(timeoutId);
			isFetchingFollowup = false;
			console.log(
				`[History Page] fetchFollowupQuiz completed, isFetchingFollowup: ${isFetchingFollowup}`,
			);
		}
	}
	...
	async function submitFollowupAnswers() {
		...
		try {
			...
			if (err instanceof Error && err.name === "AbortError") {
				followupError =
					"Request timed out. The search may still resume - please check the job status in a moment.";
			} else {
				followupError =
					err instanceof Error ? err.message : "Unknown error occurred while resuming search.";
			}
		} finally {
			clearTimeout(timeoutId);
			isSubmittingFollowup = false;
			console.log(
				`[History Page] submitFollowupAnswers completed, isSubmittingFollowup: ${isSubmittingFollowup}`,
			);
		}
	}
			...
			<GlassCard variant="accent" class="p-6 border-2 border-amber-200/60 dark:border-amber-700/40">
				...
				{#if isFetchingFollowup}
					<div class="text-center py-8">
						<svg
							class="w-10 h-10 mx-auto animate-spin text-domain-600"
							viewBox="0 0 24 24"
							fill="none"
						>
							<circle
				...
				{:else if followupQuiz}
					<div class="space-y-6">
						<!-- Context summary -->
						<div
							class="bg-gradient-to-r from-grove-50 to-domain-50 p-4 rounded-lg border border-grove-200"
						>
							<div class="flex items-center justify-between">
								<div>
									<p class="text-sm font-sans text-foreground-muted">
										We found <span class="font-bold text-grove-700"
											>{followupQuiz.context.good_found}</span
										>
										good domains out of
										<span class="font-bold text-domain-700">{followupQuiz.context.target}</span> target.
									</p>
									<p class="text-xs text-foreground-subtle font-sans mt-1">
										Answer these questions to help us find more.
									</p>
								</div>
								<div class="text-right">
									<div class="text-xs font-sans text-foreground-subtle">Batches completed</div>
									<div class="text-lg font-mono font-bold text-domain-600">
										{followupQuiz.context.batches_completed}/6
									</div>
								</div>
							</div>
						</div>
							...
							{#each followupQuiz.questions as question, idx}
								<div class="bg-white p-4 rounded-lg border border-grove-100 shadow-sm">
									...
									{:else if question.type === "single_select" && question.options}
										<select class="input-field w-full" bind:value={followupAnswers[question.id]}>
												...
												<button
													...
													onclick={() => {
														...
														if (arr.includes(opt.value)) {
															followupAnswers[question.id] = arr.filter(
					...
					<div class="overflow-x-auto">
						<table class="w-full">
							<thead class="bg-grove-50 border-b border-grove-200">
								<tr>
									<th
										class="text-left px-4 py-3 text-sm font-sans font-medium text-foreground-muted"
										>Domain</th
									>
									<th
										class="text-center px-4 py-3 text-sm font-sans font-medium text-foreground-muted"
										>Score</th
									>
									<th
										class="text-center px-4 py-3 text-sm font-sans font-medium text-foreground-muted"
									...
									>
									<th
										class="text-right px-4 py-3 text-sm font-sans font-medium text-foreground-muted"
										>Price</th
									>
									<th
										class="text-center px-4 py-3 text-sm font-sans font-medium text-foreground-muted hidden md:table-cell"
										>Batch</th
									>
								</tr>
							</thead>
							<tbody class="divide-y divide-grove-100">
								{#each availableResults as result (result.domain)}
									<tr
										class="hover:bg-grove-50 transition-colors {newDomainIds.has(result.domain)
											? 'animate-slide-in bg-grove-50/50'
											: ''}"
									>
										<td class="px-4 py-3">
										...
										<td class="px-4 py-3 text-center">
											...
											<span class="text-xs text-foreground-subtle font-sans"
										...
										<td class="px-4 py-3 text-center">
											{#if result.price_category}
												<span
													class="badge {result.price_category === 'bundled'
														? 'badge-success'
														: result.price_category === 'recommended'
															? 'badge-info'
															: 'badge-warning'}"
												>
										...
										<td
											class="px-4 py-3 text-center text-sm text-foreground-subtle font-sans hidden md:table-cell"
										>
											{result.batch_num}
										</td>
									</tr>
			...
			<details class="glass-card overflow-hidden">
				...
				>
					...
					<span class="text-sm text-foreground-subtle font-sans"
						>{unavailableResults.length} domains</span
					>
				</summary>
