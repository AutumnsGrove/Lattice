 ...
 */
export function calculateOverallStatus(components: StatusComponent[]): OverallStatus {
	if (components.length === 0) return 'operational';

	const hasOutage = components.some(c => c.current_status === 'major_outage');
	if (hasOutage) return 'major_outage';

	const hasPartial = components.some(c => c.current_status === 'partial_outage');
	if (hasPartial) return 'partial_outage';

	const hasMaintenance = components.some(c => c.current_status === 'maintenance');
	if (hasMaintenance) return 'maintenance';

	const hasDegraded = components.some(c => c.current_status === 'degraded');
