...
<section
	class={cn('glass-card p-6', className)}
	...
	<div class="flex items-center justify-between mb-6">
		...
		>
			<svelte:component
				...
				class={cn('w-4 h-4', statusConfig.color)}
				aria-hidden="true"
			/>
		...
		<div class="space-y-2" role="list" aria-label="Recent backup history">
			{#each backupStatus.dailyHistory.slice(0, 5) as day}
				...
				>
					<div class="flex items-center gap-3">
						<Calendar
							class="w-4 h-4 text-foreground-muted/60"
							aria-hidden="true"
						/>
					...
					<div
						class="flex items-center gap-4 text-sm text-foreground-muted tabular-nums"
					>
