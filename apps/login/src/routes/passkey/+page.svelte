<script lang="ts">
	 ...
	 *
	 * DEBUG VERSION: Manual WebAuthn ceremony with step-by-step logging.
	 * Replaces the opaque authClient.passkey.addPasskey() with three explicit
	 * fetch() calls so we can see exactly which step fails and why.
	 *
	 * Step 1: GET /api/auth/passkey/generate-register-options — gets challenge
	 * Step 2: navigator.credentials.create() via startRegistration() — browser prompt
	 * Step 3: POST /api/auth/passkey/verify-registration — sends credential to server
	 *
	...
	/**
	 * Register a new passkey using manual WebAuthn ceremony with full logging.
	 *
	 * Instead of authClient.passkey.addPasskey(), we call each step manually
	...
	async function registerPasskey() {
		...
		console.log("[Passkey Debug] Starting registration for device:", name);

		try {
			// ── Step 1: Generate registration options ──
			debugStep = "Requesting challenge...";
			console.log("[Passkey Debug] Step 1: Fetching registration options...");

			const optionsUrl = `/api/auth/passkey/generate-register-options?name=${encodeURIComponent(name)}`;
			const optionsRes = await fetch(optionsUrl, {
				credentials: "include",
			});

			console.log("[Passkey Debug] Step 1 response:", {
				status: optionsRes.status,
				statusText: optionsRes.statusText,
				contentType: optionsRes.headers.get("content-type"),
				setCookies: optionsRes.headers.get("set-cookie") ? "present" : "absent",
			});

			if (!optionsRes.ok) {
				const errorBody = await optionsRes.text();
				console.error("[Passkey Debug] Step 1 FAILED:", {
					status: optionsRes.status,
					body: errorBody,
				});
				throw new Error(
					`Step 1 failed (${optionsRes.status}): ${errorBody || optionsRes.statusText}`,
				);
			}
			...
			console.log("[Passkey Debug] Step 1 success — registration options:", {
				...
				rpName: optionsData?.rp?.name,
				userIdPresent: !!optionsData?.user?.id,
				userName: optionsData?.user?.name,
				excludeCredentials: optionsData?.excludeCredentials?.length ?? 0,
				authenticatorSelection: optionsData?.authenticatorSelection,
			});

			// ── Step 2: Browser credential creation prompt ──
			debugStep = "Waiting for biometric...";
			console.log("[Passkey Debug] Step 2: Calling startRegistration()...");

			const { startRegistration } = await import("@simplewebauthn/browser");
			const credential = await startRegistration({ optionsJSON: optionsData });

			console.log("[Passkey Debug] Step 2 success — credential created:", {
				id: credential.id?.slice(0, 20) + "...",
			...
			debugStep = "Verifying passkey...";
			console.log("[Passkey Debug] Step 3: Sending credential to verify-registration...");

			const verifyRes = await fetch("/api/auth/passkey/verify-registration", {
				// csrf-ok — WebAuthn challenge + SvelteKit checkOrigin
				method: "POST",
				credentials: "include",
			...
			console.log("[Passkey Debug] Step 3 response:", {
			...
			if (!verifyRes.ok) {
				...
				console.error("[Passkey Debug] Step 3 FAILED:", {
					status: verifyRes.status,
					body: errorBody,
				});
				throw new Error(
					`Step 3 failed (${verifyRes.status}): ${errorBody || verifyRes.statusText}`,
				);
			}

			const verifyData = await verifyRes.json();
			console.log("[Passkey Debug] Step 3 success — passkey verified:", verifyData);
...
<div class="w-full max-w-md animate-fade-in">
	...
	<div class="text-center mb-8 space-y-4">
		...
		<div>
			...
			<p class="mt-2 text-sm text-foreground-muted">Add a passkey for fast, secure sign-in</p>
		</div>
	</div>
		...
		{:else}
			...
			<div class="space-y-4">
				<p class="text-sm text-foreground-muted">
					Passkeys use your device's biometrics (fingerprint, face) or security key to sign you in
					securely — no passwords needed.
				</p>
