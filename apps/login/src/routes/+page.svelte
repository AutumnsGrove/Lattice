		...
		try {
			...
			if (err instanceof Error) {
				if (err.name === "NotAllowedError") {
					...
					passkeyError = "Passkey sign-in was interrupted. Please try again.";
				} else if (
					...
					passkeyError = "Auth service is temporarily unavailable. Please try again.";
				} else {
	...
	<div class="glass-grove rounded-2xl border border-default p-8 shadow-lg">
		...
		{#if emailSent}
			...
			<div class="text-center space-y-4 py-4" role="status" aria-live="polite">
				...
				<a href="?{page.url.searchParams.toString()}" class="text-sm text-primary hover:underline">
		...
		{:else}
			...
			<div class="space-y-3">
				...
				<div class="relative">
					<button onclick={signInWithPasskey} disabled={anyLoading} class="btn-auth w-full">
						{#if passkeyLoading}
							<div class="spinner"></div>
