<script lang="ts">
	...
	const maxCodeLines = $derived(
		data.snapshots.length > 0 ? Math.max(...data.snapshots.map((s: any) => s.totalCodeLines)) : 0,
	);

	// Filter milestones to only show version releases
	const milestones = $derived(data.snapshots.filter((s: any) => s.label.startsWith("v")));
	...
	const maxDocLines = $derived(
		data.snapshots.length > 0 ? Math.max(...data.snapshots.map((s: any) => s.docLines)) : 0,
	);
		...
		<div class="max-w-4xl mx-auto">
			...
			<header class="mb-12 text-center">
				...
				<p class="text-foreground-muted font-sans text-lg max-w-xl mx-auto">
					Every tree starts as a seed. Watch Grove grow from its first commit to the forest it's
					becoming.
				</p>
			...
			{#if data.latest}
				...
				<section id="current-growth" class="mb-16 scroll-mt-24">
					<h2
						class="text-sm font-sans text-foreground-faint uppercase tracking-wide mb-6 text-center"
					>
						Current Growth
					</h2>
					...
					<div class="grid grid-cols-2 md:grid-cols-4 gap-4">
						<div class="card p-6 text-center">
							...
							{#if data.growth}
								<div class="text-xs text-foreground-faint mt-2 font-sans">
									{getGrowthIcon(data.growth.codeLines)}
									{formatNumber(Math.abs(data.growth.codeLines))} since start
								</div>
							{/if}
						</div>
						...
						<div class="card p-6 text-center">
							...
							<div class="text-xs text-foreground-faint mt-2 font-sans">and counting</div>
						</div>
					</div>
				</section>
								...
								{#each data.snapshots as snapshot, i}
									...
									<div class="flex items-center gap-2 md:gap-4">
										<div class="w-[4.5rem] md:w-32 text-right flex flex-col shrink-0">
											<span class="text-[9px] md:text-xs font-sans text-foreground-faint"
												>{snapshot.date}</span
											>
											<span class="text-[10px] md:text-xs font-mono text-accent-muted"
												>{snapshot.label}</span
											>
										</div>
										...
										<div class="w-14 md:w-20 text-left shrink-0">
											<span class="text-[10px] md:text-xs font-mono text-foreground-muted"
												>{formatNumber(snapshot.totalCodeLines)}</span
											>
										</div>
									</div>
								{/each}
				...
				{#if data.snapshots.length > 1}
					<section id="documentation" class="mb-16 scroll-mt-24">
							...
							<div class="card p-6">
								<div class="text-center mb-4">
									...
									<div class="text-sm text-foreground-muted font-sans">
										lines of code per doc line
									</div>
								</div>
								<div
									class="flex justify-between text-xs text-foreground-faint font-sans pt-4 border-t border-default"
								>
						...
						<div class="card p-4 md:p-6 mt-6">
							<h3 class="text-xs font-sans text-foreground-faint uppercase tracking-wide mb-4">
								...
								{#each data.snapshots as snapshot, i}
									...
									<div class="flex items-center gap-2 md:gap-3">
										<div class="w-12 md:w-16 text-right shrink-0">
											<span class="text-[10px] md:text-xs font-mono text-foreground-faint"
												>{snapshot.label}</span
											>
										</div>
										<div
											class="flex-1 h-3.5 md:h-4 bg-surface rounded-full overflow-hidden min-w-0"
										>
											<div
										...
										<div class="w-12 md:w-16 text-left shrink-0">
											<span class="text-[10px] md:text-xs font-mono text-foreground-muted"
												>{formatNumber(snapshot.docLines)}</span
											>
										</div>
									</div>
								{/each}
						...
						<div class="card p-6">
							...
							<div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-default">
								<div class="text-center">
									<div class="text-lg font-serif text-accent-muted">
										{formatNumber(words.totalWords)}
									</div>
									<div class="text-xs text-foreground-faint font-sans">total words</div>
								</div>
								<div class="text-center">
									<div class="text-lg font-serif text-accent-muted">
										{formatNumber(words.uniqueWords)}
									</div>
							...
							<div class="mt-4 pt-4 border-t border-default text-center">
								<p class="text-sm text-foreground-muted font-sans">
									...
									Most documented topic:
									<span class="font-medium">{words.funFacts.mostDocumentedTopic}</span>
								</p>
							</div>
						</div>
					</section>
				...
				{#if milestones.length > 0}
					<section id="milestones" class="mb-16 scroll-mt-24">
						...
						<div class="relative">
							<!-- Timeline line -->
							<div
								class="absolute left-4 md:left-1/2 top-0 bottom-0 w-px bg-divider transform md:-translate-x-1/2"
							></div>
							...
							<div class="space-y-8">
								{#each milestones as snapshot, i}
									...
									<div
										class="relative flex items-start {isLeft
											? 'md:flex-row'
											: 'md:flex-row-reverse'}"
									>
										<!-- Dot -->
										<div
											class="absolute left-4 md:left-1/2 w-3 h-3 bg-accent-muted rounded-full transform -translate-x-1/2 z-10 mt-5"
										></div>
										...
										<div class="ml-12 md:ml-0 md:w-1/2 {isLeft ? 'md:pr-12' : 'md:pl-12'}">
											<div class="card p-5 {isLeft ? 'md:text-right' : ''}">
												<!-- Header -->
												<div class="text-xs font-mono text-foreground-faint mb-1">
													{snapshot.date}
												</div>
												<div
													class="font-serif text-xl text-accent-muted mb-2 flex items-center gap-2 {isLeft
														? 'md:justify-end'
														: ''}"
												>
													<Tag class="w-4 h-4" />
												...
												{#if summary}
													<p
														class="text-sm text-foreground-muted font-sans leading-relaxed mb-3 {isLeft
															? 'md:text-right'
															: ''}"
													>
														{summary.summary}
													...
													<div class="flex flex-wrap gap-2 mb-3 {isLeft ? 'md:justify-end' : ''}">
														{#if summary.stats.features > 0}
															<span
																class="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-sans rounded-full success-bg text-success"
															>
														...
														{#if summary.stats.fixes > 0}
															<span
																class="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-sans rounded-full info-bg text-info"
															>
													...
													{#if summary.highlights.features.length > 0 || summary.highlights.fixes.length > 0}
														<details class="group mt-3 pt-3 border-t border-divider">
															<summary
																class="cursor-pointer text-xs font-medium text-accent hover:text-accent-hover transition-colors flex items-center gap-1 {isLeft
																	? 'md:justify-end'
																	: ''}"
															>
																<ChevronDown
												...
												{:else}
													...
													<div class="text-sm text-foreground-muted font-sans">
														{formatNumber(snapshot.totalCodeLines)} lines · {formatNumber(
															snapshot.commits,
														)} commits
													</div>
												{/if}
												...
												<!-- Stats footer -->
												<div
													class="text-xs text-foreground-faint font-mono mt-3 pt-3 border-t border-divider flex items-center gap-3 {isLeft
														? 'md:justify-end'
														: ''}"
												>
				...
				<section class="mb-16">
					<div class="card p-12 text-center">
						<div class="mb-4 flex justify-center">
							<Sprout class="w-10 h-10 text-accent-muted" />
						</div>
			...
			<section class="text-center py-8 border-t border-default">
				...
				<p
					class="text-foreground-subtle font-sans italic inline-flex items-center gap-2 justify-center"
				>
					Building something meaningful, one commit at a time. <Sprout
						class="w-4 h-4 text-accent-muted"
					/>
				</p>
			</section>
		</div>
					...
					<ul class="toc-list">
						{#each sections as section}
							<li class="toc-item" class:active={activeSection === section.id}>
								<button type="button" class="toc-link" onclick={() => scrollToSection(section.id)}>
									{section.text}
								</button>
							</li>
						{/each}
