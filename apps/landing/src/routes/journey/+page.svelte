				...
				<div class="card p-8 text-center bg-accent border-accent">
					...
					{#if data.latest.npmUnpackedSize > 0}
						...
						<p class="text-foreground-faint font-sans text-sm mt-4 max-w-md mx-auto">
							That's what you get when you <code class="bg-background/50 px-1.5 py-0.5 rounded text-xs">npm install @autumnsgrove/lattice</code> — components, utilities, and everything you need to build with <GroveTerm interactive term="lattice">Lattice</GroveTerm>.
						</p>
					{:else}
