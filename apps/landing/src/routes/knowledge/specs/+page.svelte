<script lang="ts">
	...
	const specsByCategory = $derived(
		Object.fromEntries(
			categories.map((cat) => [cat.id, specsList.filter((spec) => spec.specCategory === cat.id)]),
...
<main class="min-h-screen flex flex-col bg-cream-50 dark:bg-cream-50">
	...
	<!-- Hero -->
	<section
		class="relative py-12 px-6 text-center bg-gradient-to-b from-cream-100 via-cream-50 to-white dark:from-cream-200 dark:via-cream-100 dark:to-cream-50"
	>
		<div class="max-w-3xl mx-auto">
			...
			<h1 class="text-4xl md:text-5xl font-serif text-foreground mb-4">Technical Specifications</h1>
		...
		<div class="max-w-5xl mx-auto space-y-16">
					...
					<div id={getCategoryId(category.name)}>
						...
						<div class="grid gap-4 md:grid-cols-2">
							{#each categorySpecs as spec}
								{@const SpecIcon = getToolIcon(spec.icon)}
								<article
										...
										<div class="flex-1 min-w-0">
											<h3 class="text-lg font-semibold text-foreground leading-tight">
												<a
													...
													class="{colors.textHover} {colors.textHoverDark} focus-visible:outline-none transition-colors motion-reduce:transition-none"
												>
													{spec.title}
												</a>
											</h3>
			...
			>
				...
				<p class="text-foreground-muted mb-4">
					If you need clarification on any technical specification or want to suggest improvements,
					we'd love to hear from you.
				</p>
				<a
					href="mailto:hello@grove.place"
					class="inline-flex items-center px-4 py-2 {colors.buttonBg} text-white rounded-lg {colors.buttonHover} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 transition-colors motion-reduce:transition-none"
				>
					Contact Us
	...
	<section class="py-8 px-6 bg-white/70 dark:bg-cream-50/50 border-t border-divider">
		<div class="max-w-4xl mx-auto flex flex-wrap justify-center gap-4">
			<a
				href="/knowledge"
				class="px-4 py-2 rounded-lg bg-cream-100 dark:bg-cream-200 text-foreground-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 transition-colors motion-reduce:transition-none"
			>
				← Knowledge Base
			</a>
			<a
