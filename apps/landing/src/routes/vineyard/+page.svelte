<script lang="ts">
	...
	const assets: Record<string, AssetInfo> = {
		...
		TreePine: {
			component: TreePine,
			category: "Trees",
			props: ["color", "trunkColor", "season", "animate"],
		},
		TreeCherry: {
			component: TreeCherry,
			category: "Trees",
			props: ["color", "trunkColor", "season", "animate"],
		},
		TreeAspen: {
		...
		Mushroom: {
			component: Mushroom,
			category: "Ground",
			props: ["capColor", "stemColor", "spotted"],
		},
		MushroomCluster: {
			component: MushroomCluster,
			category: "Ground",
			props: ["capColor", "stemColor"],
		},
		...
		FlowerWild: {
			component: FlowerWild,
			category: "Ground",
			props: ["petalColor", "centerColor", "stemColor", "animate"],
		},
		Tulip: {
			component: Tulip,
			category: "Ground",
			props: ["petalColor", "stemColor", "variant", "animate"],
		},
		Crocus: {
			component: Crocus,
			category: "Ground",
			props: ["petalColor", "centerColor", "stemColor", "variant", "animate"],
		},
		Daffodil: {
			component: Daffodil,
			category: "Ground",
			props: ["petalColor", "trumpetColor", "stemColor", "animate"],
		},
		Firefly: {
			component: Firefly,
			category: "Creatures",
			props: ["glowColor", "bodyColor", "animate", "intensity"],
		},
		Butterfly: {
			component: Butterfly,
			category: "Creatures",
			props: ["wingColor", "accentColor", "animate"],
		},
		Bird: {
			component: Bird,
			category: "Creatures",
			props: ["bodyColor", "breastColor", "beakColor", "animate", "facing"],
		},
		BirdFlying: {
			component: BirdFlying,
			category: "Creatures",
			props: ["color", "animate", "facing"],
		},
		Cardinal: {
			component: Cardinal,
			category: "Creatures",
			props: ["bodyColor", "maskColor", "beakColor", "animate", "facing"],
		},
		Chickadee: {
			component: Chickadee,
			category: "Creatures",
			props: ["capColor", "cheekColor", "bodyColor", "animate", "facing"],
		},
		Robin: {
			component: Robin,
			category: "Creatures",
		...
		StarCluster: {
			component: StarCluster,
			category: "Sky",
			props: ["color", "animate", "density"],
		},
		StarShooting: {
			component: StarShooting,
			category: "Sky",
			props: ["color", "animate", "direction"],
		},
		...
		LilyPad: {
			component: LilyPad,
			category: "Water",
			props: ["padColor", "flowerColor", "hasFlower", "animate"],
		},
		Reeds: {
			component: Reeds,
			category: "Water",
			props: ["color", "season", "animate", "variant"],
		},
		...
		LeafFalling: {
			component: LeafFalling,
			category: "Botanical",
			props: ["color", "season", "animate", "variant"],
		},
		PetalFalling: {
			component: PetalFalling,
			category: "Botanical",
			props: ["color", "variant", "animate", "opacity"],
		},
		...
		DandelionPuff: {
			component: DandelionPuff,
			category: "Botanical",
			props: ["seedColor", "animate"],
		},
		Vine: {
			component: Vine,
			category: "Botanical",
		...
		LatticeWithVine: {
			component: LatticeWithVine,
			category: "Structural",
			props: ["woodColor", "vineColor", "season", "hasFlowers"],
		},
		...
		Lantern: {
			component: Lantern,
			category: "Structural",
			props: ["frameColor", "lit", "animate", "variant"],
		},
	};
...
<main class="max-w-6xl mx-auto px-6 py-12">
	...
	<nav class="flex flex-wrap justify-center gap-3 mb-12 sm:hidden">
		<a
			href="#glass"
			class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border-subtle)] text-[var(--color-foreground-muted)] hover:text-[var(--color-primary)] text-sm transition-colors"
		>
	...
	<div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-12">
		<div
			class="p-4 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border-subtle)] text-center"
		>
		...
		>
			...
			<p class="text-xs text-[var(--color-foreground-subtle)]">
				{Object.keys(assets).length} SVG components
			</p>
		</div>
		<div
			class="p-4 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border-subtle)] text-center"
	...
	<details id="glass" class="vineyard-section mb-8 scroll-mt-20" open>
			...
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-sm text-[var(--color-foreground-muted)]">
						Frosted glassmorphism with that cozy Grove warmth
					</p>
				</div>
			</div>
		...
		<div class="section-content grid gap-6">
			<!-- Glass Base -->
			<div
				class="p-5 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border-subtle)]"
			>
				<h3
					class="text-lg font-semibold mb-4 flex items-center gap-2 text-[var(--color-foreground)]"
				>
					<code
						class="text-sm px-2 py-1 rounded bg-[var(--color-accent-bg)] text-[var(--color-accent-text)]"
						>&lt;Glass&gt;</code
					>
					<span class="text-sm font-normal text-[var(--color-foreground-muted)]"
						>Base glass container</span
					>
				</h3>

				<div class="grid md:grid-cols-2 gap-6">
					<div
						class="bg-gradient-to-br from-grove-100 to-grove-200 dark:from-grove-900 dark:to-grove-800 rounded-xl p-8 flex items-center justify-center min-h-[200px]"
					>
						<Glass
						...
						>
							...
							<p class="text-sm text-[var(--color-foreground-muted)] mt-1">
								With {glassVariant} variant
							</p>
						</Glass>
					</div>
					...
					<div class="space-y-4">
						<div>
							<label
								for="glass-variant"
								class="block text-sm font-medium mb-2 text-[var(--color-foreground)]">Variant</label
							>
						...
						<div>
							<label
								for="glass-intensity"
								class="block text-sm font-medium mb-2 text-[var(--color-foreground)]"
								>Intensity</label
							>
						...
						<div>
							<label class="flex items-center gap-3 cursor-pointer">
								<input
									type="checkbox"
									bind:checked={glassGossamerEnabled}
					...
					{#each ["spring", "summer", "autumn", "winter"] as s}
						<button
							class="px-3 py-1 text-sm rounded-full transition-colors {logoSeason === s
								...
								: 'bg-[var(--color-accent-bg)] text-[var(--color-foreground-muted)] hover:bg-[var(--color-surface-hover)]'}"
							onclick={() => (logoSeason = s as typeof logoSeason)}>{s}</button
						>
					{/each}
				</div>
		...
		<summary class="section-header">
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-sm text-[var(--color-foreground-muted)]">
						{Object.keys(assets).length} SVG components across {categories.length} categories
					</p>
				</div>
			</div>
			<ChevronDown class="w-5 h-5 text-[var(--color-foreground-subtle)] section-chevron" />
		...
		<div class="section-content">
			...
			>
				<div class="grid md:grid-cols-2 gap-8">
					...
					<div>
						<div
							class="bg-gradient-to-b from-sky-100 to-emerald-50 dark:from-sky-900/30 dark:to-emerald-900/20 rounded-xl p-8 flex items-center justify-center min-h-[300px] border border-[var(--color-border-subtle)]"
						>
							{#if CurrentComponent}
								{#key selectedAsset + JSON.stringify(propValues)}
									<svelte:boundary
										onerror={(e) => {
											componentError = e instanceof Error ? e.message : String(e);
										}}
									>
					...
					<div class="space-y-6">
						...
						{#if getCurrentAsset()}
							<div class="space-y-4 max-h-[400px] overflow-y-auto pr-2">
								<h4
									class="text-sm font-medium uppercase tracking-wide text-[var(--color-foreground-subtle)]"
								>
									Properties
								</h4>
								...
								{#each getCurrentAsset().props as prop}
									<div class="space-y-2">
										<label
											for="prop-{prop}"
											class="block text-sm font-medium text-[var(--color-foreground)]">{prop}</label
										>
												...
												<div class="flex flex-wrap gap-1">
													{#each colorPresets as preset}
														<button
															type="button"
															onclick={() => {
																propValues[prop] = preset.value;
																pendingColorValues[prop] = preset.value;
															}}
										...
										{:else if isBooleanProp(prop)}
											<label class="flex items-center gap-3 cursor-pointer">
												...
												<span class="text-sm text-[var(--color-foreground-muted)]"
													>{propValues[prop] !== false ? "Enabled" : "Disabled"}</span
												>
											</label>
										...
										{:else if isNumericProp(prop)}
											...
											<div class="space-y-1">
												...
												/>
												<div
													class="flex justify-between text-xs text-[var(--color-foreground-subtle)]"
												>
													<span>{range.min}</span>
													<span class="font-medium"
														>{propValues[prop]?.toFixed(range.step < 1 ? 1 : 0) ?? "default"}</span
													>
													<span>{range.max}</span>
												</div>
											</div>
		...
		<summary class="section-header">
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-sm text-[var(--color-foreground-muted)]">
						10 fonts for every mood, from cozy headers to crisp code
					</p>
				</div>
			</div>
			<ChevronDown class="w-5 h-5 text-[var(--color-foreground-subtle)] section-chevron" />
		...
		<div class="section-content grid gap-6">
			...
			>
				...
				<div class="space-y-4">
					...
					<div
						class="p-6 bg-[var(--color-surface-elevated)] rounded-lg border border-[var(--color-border-subtle)]"
					>
						<FontProvider
							font={selectedFont}
							as="p"
							class="text-2xl text-[var(--color-foreground)]"
						>
							The quick brown fox jumps over the lazy dog.
						</FontProvider>
						<p class="text-sm text-[var(--color-foreground-subtle)] mt-2">
							{fonts.find((f: { id: string; description?: string }) => f.id === selectedFont)
								?.description}
						</p>
					</div>
				</div>
			...
			>
				...
				<div class="space-y-4">
					<div
						class="p-4 bg-gradient-to-r from-purple-100 to-pink-100 dark:from-purple-900/30 dark:to-pink-900/20 rounded-lg"
					>
						<Alagard as="h3" class="text-2xl text-purple-900 dark:text-purple-200 mb-2">
							Welcome to the Fantasy Realm
						</Alagard>
						<p class="text-sm text-purple-700 dark:text-purple-300">
							Alagard - pixel art medieval display font
						</p>
					</div>
					<div
						class="p-4 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-lg"
					>
					...
					<div
						class="p-4 bg-gradient-to-r from-rose-50 to-pink-50 dark:from-rose-900/20 dark:to-pink-900/20 rounded-lg"
					>
						<Caveat as="h3" class="text-2xl text-rose-900 dark:text-rose-200 mb-2">
							A personal note just for you...
						</Caveat>
						<p class="text-sm text-rose-700 dark:text-rose-300">
							Caveat - handwritten script, personal and informal
						</p>
					</div>
				</div>
			</div>
			...
			>
				...
				<div class="grid md:grid-cols-2 gap-4">
					<div
						class="p-4 bg-[var(--color-surface-elevated)] rounded-lg border border-[var(--color-border-subtle)]"
					>
						<Lexend as="h3" class="text-xl text-[var(--color-foreground)] mb-2"
							>Lexend (Default)</Lexend
						>
						<Lexend as="p" class="text-[var(--color-foreground-muted)]"
					...
					<div
						class="p-4 bg-[var(--color-surface-elevated)] rounded-lg border border-[var(--color-border-subtle)]"
					>
						<Quicksand as="h3" class="text-xl text-[var(--color-foreground)] mb-2"
							>Quicksand</Quicksand
						>
						<Quicksand as="p" class="text-[var(--color-foreground-muted)]"
					...
					<div
						class="p-4 bg-[var(--color-surface-elevated)] rounded-lg border border-[var(--color-border-subtle)]"
					>
						<PlusJakartaSans as="h3" class="text-xl text-[var(--color-foreground)] mb-2"
							>Plus Jakarta Sans</PlusJakartaSans
						>
						<PlusJakartaSans as="p" class="text-[var(--color-foreground-muted)]"
			...
			>
				...
				<div class="space-y-4">
					<div class="p-4 bg-bark-900 rounded-lg">
						...
						<IBMPlexMono as="pre" class="text-gray-100 text-sm"
...
> Server running at localhost:5173`}</Cozette
			...
			>
				...
				<div class="grid md:grid-cols-2 gap-4">
					<div
						class="p-4 bg-grove-50 dark:bg-grove-950/30 rounded-lg border border-grove-200 dark:border-grove-800"
					>
						<Atkinson as="h3" class="text-lg text-grove-900 dark:text-grove-200 mb-2"
					...
					>
						<OpenDyslexic as="h3" class="text-lg text-blue-900 dark:text-blue-200 mb-2"
	...
	<details id="palettes" class="vineyard-section mb-8 scroll-mt-20" open>
		<summary class="section-header">
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-sm text-[var(--color-foreground-muted)]">
						The rich, natural hues that bring Grove to life
					</p>
				</div>
			</div>
			<ChevronDown class="w-5 h-5 text-[var(--color-foreground-subtle)] section-chevron" />
		...
		<div class="section-content grid gap-6">
			...
			{@render PaletteRow({
				name: "Pinks",
				colors: pinks,
				description: "Blush, rose, and soft florals",
			})}
			{@render PaletteRow({
				name: "Midnight Bloom",
				colors: midnightBloom,
				description: "A late-night tea cafe palette",
			})}
		</div>
	</details>
</main>
		...
		<div class="flex items-center justify-center h-full">
			<GlassCard variant="frosted" class="max-w-sm mx-4" title="Overlay Demo">
				<p class="text-[var(--color-foreground-muted)]">
					Click anywhere on the backdrop to close this overlay.
				</p>
				<div class="mt-4">
		...
		>
			{#each Object.entries(colors) as [key, color]}
				<div class="group relative">
					...
					></button>
					<div
						class="absolute top-full left-1/2 -translate-x-1/2 mt-2 px-2 py-1 bg-bark-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-10"
					>
						{key}: {color}
					</div>
				</div>
