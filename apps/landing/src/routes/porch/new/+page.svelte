	...
	<div class="max-w-2xl mx-auto">
		...
		{#if form?.success && form?.visitNumber}
			<div role="status" aria-live="polite">
				<GlassCard
					class="mb-6 bg-success-bg/80 dark:bg-success-bg/20 border-success dark:border-success/50"
				>
					<div class="flex items-start gap-4">
						<div
							class="w-10 h-10 bg-success-bg dark:bg-success-bg/30 rounded-lg flex items-center justify-center text-success dark:text-success flex-shrink-0"
						>
						...
						<div class="flex-1">
							...
							<p class="text-sm text-success dark:text-success font-sans mb-2">
		...
		{#if form?.error}
			<div role="alert">
				<GlassCard class="mb-6 bg-error-bg/80 dark:bg-error-bg/20 border-error dark:border-error/50">
					<div class="flex items-start gap-4">
						<div
							class="w-10 h-10 bg-error-bg dark:bg-error-bg/30 rounded-lg flex items-center justify-center text-error dark:text-error flex-shrink-0"
						>
							<svg class="w-6 h-6" viewBox="0 0 20 20" fill="currentColor">
								<path
									fill-rule="evenodd"
				...
				>
							...
							{#each categories as cat}
								<button
									...
									class="p-3 rounded-lg border transition-all text-left {category === cat.id
										? 'border-primary bg-primary/10 text-primary dark:bg-primary/20 dark:text-primary dark:border-primary'
										: 'border-border bg-white/50 text-foreground/70 hover:border-border dark:border-cream-300 dark:bg-cream-200/50 dark:hover:border-cream-400'}"
									disabled={submitting}
								>
					...
					<div class="mb-5">
						...
						<input
							...
							class="w-full px-4 py-3 rounded-lg border border-border dark:border-cream-300 bg-white/50 dark:bg-cream-200/50 text-foreground placeholder-foreground/40 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent font-sans transition-all"
							disabled={submitting}
						/>
					</div>
					...
					<div class="mb-5">
						<label for="email" class="block text-sm font-sans font-medium text-foreground mb-2">
							Email <span class="text-error dark:text-error">*</span>
						</label>
						<input
							type="email"
							id="email"
					...
					<div class="mb-5">
						<label for="subject" class="block text-sm font-sans font-medium text-foreground mb-2">
							Subject <span class="text-error dark:text-error">*</span>
						</label>
						<input
							type="text"
							id="subject"
					...
					<div class="mb-5">
						<label for="message" class="block text-sm font-sans font-medium text-foreground mb-2">
							What's going on? <span class="text-error dark:text-error">*</span>
						</label>
						<textarea
							id="message"
							name="message"
							...
							minlength="10"
							maxlength="5000"
							class="w-full px-4 py-3 rounded-lg border border-border dark:border-cream-300 bg-white/50 dark:bg-cream-200/50 text-foreground placeholder-foreground/40 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent font-sans transition-all resize-y"
							disabled={submitting}
						></textarea>
					...
					<button
						...
						class="w-full px-6 py-3 bg-primary text-primary-foreground rounded-lg font-sans font-medium hover:bg-primary disabled:bg-primary/50 dark:disabled:bg-primary/50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
					>
						{#if submitting}
