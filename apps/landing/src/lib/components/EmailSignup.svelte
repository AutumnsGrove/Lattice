...
{#if status === 'success'}
	<div role="status" aria-live="polite"
		class="{variant === 'dark' ? 'bg-accent-subtle border-accent-muted' : 'bg-accent border-accent'} border rounded-lg px-6 py-4 text-center max-w-md animate-in"
	>
		<svg class="{variant === 'dark' ? 'text-warning' : 'text-accent-muted'} w-8 h-8 mx-auto mb-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
			<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" stroke-linecap="round" />
			<polyline points="22 4 12 14.01 9 11.01" stroke-linecap="round" stroke-linejoin="round" />
		</svg>
		<p class="{variant === 'dark' ? 'text-foreground' : 'text-accent'} font-sans font-medium">You're on the list!</p>
		<p class="{variant === 'dark' ? 'text-foreground-muted' : 'text-accent-muted'} text-sm font-sans mt-1">Check your inbox for a welcome note.</p>
		<p class="{variant === 'dark' ? 'text-foreground-subtle' : 'text-accent-subtle'} text-xs font-sans mt-2">We'll be in touch when Grove blooms.</p>
	</div>
{:else if status === 'already_signed_up'}
	<div role="status" aria-live="polite"
		class="{variant === 'dark' ? 'bg-accent-subtle border-accent-muted' : 'bg-accent border-accent'} border rounded-lg px-6 py-4 text-center max-w-md animate-in"
	>
		<svg class="{variant === 'dark' ? 'text-success' : 'text-accent-muted'} w-8 h-8 mx-auto mb-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
		...
		<p class="{variant === 'dark' ? 'text-foreground' : 'text-accent'} font-sans font-medium">You're already on the list!</p>
		<p class="{variant === 'dark' ? 'text-foreground-muted' : 'text-accent-muted'} text-sm font-sans mt-1">Thank you for being so excited.</p>
		<p class="{variant === 'dark' ? 'text-foreground-subtle' : 'text-accent-subtle'} text-xs font-sans mt-2">We'll be in touch when Grove blooms.</p>
	</div>
{:else}
	<form onsubmit={handleSubmit} class="w-full max-w-md">
		...
		{#if status === 'error'}
			<p id="email-error" role="alert" class="{variant === 'dark' ? 'text-error' : 'text-error'} text-sm mt-2 font-sans">{errorMessage}</p>
		{/if}

		<p class="{variant === 'dark' ? 'text-foreground-subtle' : 'text-foreground-faint'} text-xs mt-3 text-center font-sans">
