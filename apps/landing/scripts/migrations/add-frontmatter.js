...
function parseKnowledgeBase() {
  ...
  const extractArray = (name) => {
    ...
    const regex = new RegExp(
      `export const ${name}:\\s*Doc\\[\\]\\s*=\\s*\\[([\\s\\S]*?)\\];`,
      "m",
    );
...
function buildFrontmatter(doc, existingFrontmatter = {}) {
  ...
  if (existingFrontmatter.order !== undefined)
    fm.order = existingFrontmatter.order;
...
function updateMarkdownFile(filePath, doc) {
  try {
    ...
    const { data: existingFrontmatter, content: markdownContent } =
      matter(content);
