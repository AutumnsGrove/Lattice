					...
					{:else}
						...
						<div class="space-y-4">
							<div class="flex items-center justify-between">
								<h3 class="text-sm font-sans font-medium text-bark dark:text-neutral-100">
							...
							<div class="bg-grove-50 border border-grove-200 rounded-lg p-4 space-y-3">
								...
								<div>
									<label
										for="vibe_parsed"
										class="block text-xs font-sans text-foreground-muted mb-1">Vibe</label
									>
									<select
										id="vibe_parsed"
								...
								<div>
									...
									<input
										...
										oninput={(e) => {
											if (parsedVibe) parsedVibe.domain_idea = e.currentTarget.value || null;
										}}
											...
											<button
												...
												onclick={() => {
													...
													if (parsedVibe.tld_preferences.includes(tld)) {
														parsedVibe.tld_preferences = parsedVibe.tld_preferences.filter(
															(t) => t !== tld,
														);
													} else {
							...
							<div class="bg-grove-50 border border-grove-200 rounded-lg p-3">
								...
								<div class="text-sm font-sans text-bark dark:text-neutral-100 font-medium">
				...
				>
					...
					<div>
								...
								<div class="border-b border-grove-100 last:border-b-0">
									...
									>
										<div class="flex items-center gap-2">
											...
											<span class="font-sans font-medium text-bark dark:text-neutral-100 text-sm"
												>{group.label}</span
											>
						...
						<div class="flex items-center justify-between mt-3 pt-3 border-t border-grove-100">
							<div>
								<span class="text-sm font-sans font-medium text-bark dark:text-neutral-100"
									>Diverse TLDs</span
								>
					...
					<div>
						...
						<div class="card p-4 bg-grove-50">
							<div class="text-base font-sans text-bark dark:text-neutral-100 font-medium">
								{aiProviderInfo.model} via {aiProviderInfo.provider}
							</div>
			...
			{#if currentJob}
				...
				<GlassCard class="p-4 sm:p-6">
					<div class="flex items-center justify-between mb-3 sm:mb-4">
						<h2 class="font-serif text-base sm:text-lg text-bark dark:text-neutral-100">
							Search Status
						</h2>
						<div class="flex items-center gap-2">
					...
					<div class="space-y-3">
						<div class="flex justify-between text-sm font-sans">
							...
							<span class="text-bark dark:text-neutral-100 font-medium"
								>{currentJob.business_name}</span
							>
						</div>
						<div class="flex justify-between text-sm font-sans">
						...
						<div class="flex justify-between text-sm font-sans">
							...
							{:else if currentJob.duration_seconds}
								<span class="text-bark dark:text-neutral-100 font-mono tabular-nums"
				...
				{#if currentJob.status === "needs_followup" && followupQuiz}
					<GlassCard variant="accent" class="p-4 sm:p-6">
						<h2 class="font-serif text-base sm:text-lg text-bark dark:text-neutral-100 mb-2">
							Refine Your Search
						</h2>
						<p class="text-sm text-foreground-muted font-sans mb-4">
							We found {followupQuiz.context.good_found} good domains out of {followupQuiz.context
								.target} target. Answer these questions to help us find more.
						</p>
							...
							{#each followupQuiz.questions as question}
								<div>
									...
									{:else if question.type === "single_select" && question.options}
										<select class="input-field" bind:value={followupAnswers[question.id]}>
				...
				{#if pricingSummary && (currentJob.status === "complete" || currentJob.status === "needs_followup")}
					<GlassCard class="p-4 sm:p-6">
						<h2
							class="font-serif text-base sm:text-lg text-bark dark:text-neutral-100 mb-3 sm:mb-4"
						>
							Pricing Summary
						</h2>
						...
						{#if tokenUsage}
							<div
								class="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-grove-200 space-y-1.5 sm:space-y-2"
							>
								<div
									class="flex justify-between text-[10px] sm:text-xs font-sans text-foreground-subtle"
								>
									<span>API Usage</span>
								...
								>
									...
									<span class="font-mono text-domain-600 font-medium"
										>{estimateCost(tokenUsage)}</span
									>
								</div>
							</div>
						{/if}
				...
				{#if jobResults.length > 0}
					<GlassCard>
						<div
							class="px-3 py-3 sm:p-4 border-b border-grove-200 flex justify-between items-center"
						>
							<h2 class="font-serif text-base sm:text-lg text-bark dark:text-neutral-100">
								Available Domains
							</h2>
						...
						<div class="max-h-[600px] overflow-y-auto divide-y divide-grove-100">
							{#each jobResults
								...
								.sort((a, b) => b.score - a.score) as result}
								...
								<div class="hover:bg-grove-50 transition-colors">
									...
									>
										<div class="flex-1 min-w-0">
											<div class="flex flex-wrap items-center gap-1.5 sm:gap-2">
												<span
													class="font-mono text-bark dark:text-neutral-100 font-medium text-sm sm:text-base break-all"
													>{result.domain}</span
												>
										...
										<div
											class="text-right flex-shrink-0 ml-3 sm:ml-4 flex items-center gap-2 sm:gap-3"
										>
											<span
												class="{getPriceClass(
									...
									{#if isExpanded && evalData}
										<div
											class="px-3 sm:px-4 pb-3 sm:pb-4 pt-0 border-t border-grove-100 bg-grove-50/50"
										>
											<div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-3">
												<!-- Evaluation scores -->
												<div class="space-y-2">
													<h4
														class="text-xs font-sans font-medium text-foreground-muted uppercase tracking-wide"
													>
														Evaluation
													</h4>
													...
													<div class="space-y-1.5">
														{#if evalData.renewal_cents}
															<div class="flex items-center justify-between text-sm font-sans">
																...
																<span class="text-bark dark:text-neutral-100"
														...
														{#if evalData.rdap_registrar}
															<div class="flex items-center justify-between text-sm font-sans">
																...
																<span
																	class="text-bark dark:text-neutral-100 truncate max-w-[150px]"
