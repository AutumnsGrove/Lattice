...
<div class="space-y-8">
	<!-- Page Header -->
	<div>
		<h1 class="text-2xl font-serif text-bark dark:text-foreground">Dashboard</h1>
		<p class="text-foreground-muted dark:text-foreground-muted font-sans mt-1">
			Overview of domain search activity
		</p>
	</div>
	...
	<div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
		<GlassCard hoverable class="p-6">
			...
			<div class="text-3xl font-serif text-bark dark:text-foreground">{totalJobs}</div>
		</GlassCard>
		<GlassCard hoverable class="p-6">
			<div class="text-sm font-sans text-foreground-muted dark:text-foreground-muted mb-1">
				Currently Running
		...
		<GlassCard hoverable class="p-6">
			...
			<div class="text-3xl font-serif text-bark dark:text-foreground">{totalDomainsFound}</div>
		</GlassCard>
	</div>

	<!-- Quick Actions -->
	<div class="flex gap-4">
		<GlassButton variant="accent" href="/arbor/searcher">Start New Search</GlassButton>
		<GlassButton variant="ghost" href="/arbor/history">View All History</GlassButton>
	</div>

	<!-- Recent Jobs -->
	<GlassCard>
		<div class="p-4 border-b glass-divider">
			<h2 class="font-serif text-lg text-bark dark:text-foreground">Recent Searches</h2>
		</div>
		{#if data.jobs.length === 0}
			<div class="p-8 text-center">
				<p class="text-foreground-muted dark:text-foreground-muted font-sans">
					No searches yet. Start your first domain search!
				</p>
			</div>
		{:else}
			<div class="divide-y glass-divide">
				{#each data.jobs.slice(0, 5) as job}
					<a
						href="/arbor/history/{job.id}"
						class="block p-4 hover:bg-surface-hover dark:hover:bg-card transition-colors"
					>
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-3">
								<div
									class="status-dot status-dot-{job.status === 'running'
										? 'running'
										: job.status === 'complete'
											? 'complete'
											: job.status === 'failed'
								...
								<div>
									...
									<div class="text-sm text-foreground-muted dark:text-foreground-muted font-sans">
										{job.client_email}
									</div>
								</div>
							</div>
							<div class="text-right">
								...
								<div
									class="text-xs text-foreground-subtle dark:text-foreground-subtle font-sans mt-1"
								>
						...
						<div
							class="mt-2 flex gap-4 text-sm text-foreground-muted dark:text-foreground-muted font-sans"
						>
