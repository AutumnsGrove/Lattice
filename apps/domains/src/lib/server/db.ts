...
export async function createSession(
  ...
  const tokenExpiresAt = tokens?.expiresIn
    ? futureTimestamp(tokens.expiresIn * 1000)
    : null;
...
export async function getSearchJob(
  ...
  return queryOne<DomainSearchJob>(
    db,
    "SELECT * FROM domain_search_jobs WHERE id = ?",
    [id],
  );
}
...
export async function updateSearchJobStatus(
  ...
  await execute(
    db,
    `UPDATE domain_search_jobs SET ${fields.join(", ")} WHERE id = ?`,
    values,
  );
}
