...
export const POST: RequestHandler = async ({ url, platform, locals }) => {
	...
	const maxObjects = Math.min(parseInt(url.searchParams.get("maxObjects") || "500", 10), 1000);
	...
	try {
		...
		do {
			...
			for (const obj of listResult.objects) {
				...
				try {
					...
					result.failed++;
					mapping.status = "failed";
					mapping.reason = copyErr instanceof Error ? copyErr.message : "copy failed";
					result.keyMappings.push(mapping);
				}
			}
		...
		throw error(500, `Migration failed: ${err instanceof Error ? err.message : "unknown error"}`);
	}
};
