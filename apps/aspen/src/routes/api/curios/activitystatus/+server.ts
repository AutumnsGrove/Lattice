...
export const POST: RequestHandler = async ({ request, platform, locals }) => {
	...
	// Handle expiration
	const expirationHours = typeof body.expiresInHours === "number" ? body.expiresInHours : null;
...
export const DELETE: RequestHandler = async ({ platform, locals }) => {
	...
	try {
		await db.prepare(`DELETE FROM activity_status WHERE tenant_id = ?`).bind(tenantId).run();
	} catch (err) {
		logGroveError("API", API_ERRORS.OPERATION_FAILED, {
