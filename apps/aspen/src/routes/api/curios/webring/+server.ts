...
import {
	...
	type WebringRecord,
} from "@autumnsgrove/lattice/curios/webring";
...
export const POST: RequestHandler = async ({ request, platform, locals }) => {
	...
	const position = isValidPosition(body.position as string) ? (body.position as string) : "footer";

	const id = generateWebringId();

	// Enforce per-tenant webring limit
	const countResult = await db
		.prepare(`SELECT COUNT(*) as count FROM webring_memberships WHERE tenant_id = ?`)
		.bind(tenantId)
		.first<{ count: number }>();
