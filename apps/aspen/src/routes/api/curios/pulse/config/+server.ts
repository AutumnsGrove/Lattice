...
import { DEFAULT_PULSE_CONFIG, buildWebhookUrl } from "@autumnsgrove/lattice/curios/pulse";
...
export const GET: RequestHandler = async ({ platform, locals }) => {
	...
	try {
		...
		hasWebhookSecret = await secretsManager.hasSecret(tenantId, "pulse_webhook_secret");
	} catch {
	...
	return json({
		config: {
			...
			reposInclude: config.repos_include ? safeParseJson(config.repos_include) : null,
			reposExclude: config.repos_exclude ? safeParseJson(config.repos_exclude) : null,
...
export const PUT: RequestHandler = async ({ request, platform, locals }) => {
	...
	const feedMaxItems = Math.max(10, Math.min(500, Number(body.feedMaxItems) || 100));
	...
	try {
		...
		if (enabled) {
			try {
				const secretsManager = await createSecretsManager(platform?.env);
				const hasSecret = await secretsManager.hasSecret(tenantId, "pulse_webhook_secret");
				if (!hasSecret) {
					...
					webhookSecret = Array.from(bytes)
						...
						.join("");
					await secretsManager.setSecret(tenantId, "pulse_webhook_secret", webhookSecret);
				}
			} catch (err) {
