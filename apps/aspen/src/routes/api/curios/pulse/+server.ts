...
import { PULSE_EVENT_TYPES } from "@autumnsgrove/lattice/curios/pulse";
import { z } from "zod";
import { safeJsonParse } from "@autumnsgrove/lattice/server/utils";
...
export const GET: RequestHandler = async ({ url, platform, locals }) => {
	...
	const config = await db
		.prepare(`SELECT enabled FROM pulse_curio_config WHERE tenant_id = ? AND enabled = 1`)
		.bind(tenantId)
