...
export const POST: RequestHandler = async ({ request, platform, locals }) => {
	...
	const provider = isValidProvider(String(body.provider ?? "manual"))
		...
		: "manual";
	const displayStyle = isValidDisplayStyle(String(body.displayStyle ?? "compact"))
		? String(body.displayStyle)
		: "compact";
	const showAlbumArt = typeof body.showAlbumArt === "boolean" ? body.showAlbumArt : true;
	const showProgress = typeof body.showProgress === "boolean" ? body.showProgress : false;
	const fallbackText = sanitizeFallbackText(body.fallbackText as string | null);
	const lastFmUsername = sanitizeLastFmUsername(body.lastFmUsername as string | null);
