...
export const POST: RequestHandler = async ({ request, platform, locals }) => {
	...
	const trackName = sanitizeTrackText(body.trackName as string | null, MAX_TRACK_NAME_LENGTH);
	const artist = sanitizeTrackText(body.artist as string | null, MAX_ARTIST_LENGTH);

	if (!trackName || !artist) {
		throwGroveError(400, API_ERRORS.MISSING_REQUIRED_FIELDS, "API");
	}

	const album = sanitizeTrackText(body.album as string | null, MAX_ALBUM_LENGTH);
	const albumArtUrl = typeof body.albumArtUrl === "string" ? body.albumArtUrl.trim() || null : null;
	...
	return json({ success: true, track: { trackName, artist, album } }, { status: 201 });
};
