...
export const GET: RequestHandler = async ({ platform }) => {
	...
	const checks = await Promise.all([checkD1(platform?.env?.DB), checkKV(platform?.env?.CACHE_KV)]);
 ...
 */
async function checkD1(db: D1Database | undefined): Promise<HealthCheckResponse["checks"][number]> {
	if (!db) {
		return {
			name: "d1_database",
			status: "fail",
