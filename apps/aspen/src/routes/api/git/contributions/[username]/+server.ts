...
export const GET: RequestHandler = async ({ params, platform, url, request, locals }) => {
	...
	if (db && tenantId) {
		try {
			const config = await db
				.prepare(`SELECT github_token_encrypted FROM git_dashboard_config WHERE tenant_id = ?`)
				.bind(tenantId)
			...
			if (config?.github_token_encrypted) {
				...
				token = await safeDecryptToken(config.github_token_encrypted, encryptionKey);
			}
		} catch (err) {
			console.warn("Failed to fetch tenant token, falling back to global:", err);
		}
	}
	...
	const getHeaders = () =>
		rateLimitResult ? thresholdHeaders(rateLimitResult, RATE_LIMIT.limit) : undefined;
	...
	if (kv) {
		try {
			...
			console.warn(`[Git Contributions] Cache read failed for ${username}:`, err);
		}
	}
	...
	try {
		...
		if (kv) {
			try {
				...
				console.warn(`[Git Contributions] Cache write failed for ${username}:`, err);
			}
		}
