 ...
 */
async function getConfigForTenant(db: D1Database, tenantId: string): Promise<ParsedConfig> {
	try {
...
export const GET: RequestHandler = async ({ platform, locals }) => {
	...
	return json(
		{ config },
		{
			headers: rateLimitResult ? thresholdHeaders(rateLimitResult, READ_LIMIT.limit) : undefined,
		},
	);
};
...
export const PUT: RequestHandler = async ({ request, platform, locals }) => {
	...
	const { enabled, githubUsername, githubToken, showOnHomepage, cacheTtlSeconds } = body;
	...
	try {
      ...
      ON CONFLICT(tenant_id) DO UPDATE SET
        ...
        updated_at = strftime('%s', 'now')`,
			[tenantId, enabled ? 1 : 0, trimmedUsername, githubTokenForDb, showOnHomepage ? 1 : 0, ttl],
		);
