 ...
 */
function createOGResponse(tags: Record<string, string>, title?: string): Response {
			...
			try {
				...
				const response = await fetch(oembedUrl.href, {
					headers: {
						...
						"User-Agent": "GroveBot/1.0 (+https://grove.place; oEmbed Consumer)",
					},
...
describe("trusted provider embed flow", () => {
	it("returns embed data for YouTube URL with oEmbed response", async () => {
		...
		const result = await simulateOEmbedEndpoint("https://www.youtube.com/watch?v=dQw4w9WgXcQ");
	...
	it("returns embed data for Strawpoll URL", async () => {
		...
		const result = await simulateOEmbedEndpoint("https://strawpoll.com/polls/abc123");
	...
	it("uses extractEmbedUrl even when oEmbed fetch fails", async () => {
		...
		const result = await simulateOEmbedEndpoint("https://www.youtube.com/watch?v=abc123xyz");
	...
	it("extracts iframe src from oEmbed HTML when no extractEmbedUrl", async () => {
		...
		const result = await simulateOEmbedEndpoint("https://soundcloud.com/artist-name/cool-track");
	...
	it("includes sandbox permissions from provider config", async () => {
		const mockFetch = vi
			.fn()
			.mockResolvedValueOnce(createOEmbedResponse({ type: "rich", title: "Poll" }));
		globalThis.fetch = mockFetch;

		const result = await simulateOEmbedEndpoint("https://strawpoll.com/my-poll");
	...
	it("returns aspect ratio from provider config", async () => {
		const mockFetch = vi
			.fn()
			.mockResolvedValueOnce(createOEmbedResponse({ type: "video", title: "Video" }));
		globalThis.fetch = mockFetch;

		const result = await simulateOEmbedEndpoint("https://www.youtube.com/watch?v=test123");
...
describe("oEmbed response validation in flow", () => {
	it("rejects oEmbed with invalid type and still returns embed via extractEmbedUrl", async () => {
		...
		const mockFetch = vi
			.fn()
			.mockResolvedValueOnce(createOEmbedResponse({ type: "malicious" as any, title: "Bad" }));
		globalThis.fetch = mockFetch;

		const result = await simulateOEmbedEndpoint("https://www.youtube.com/watch?v=abc123");
	...
	it("rejects oEmbed with wrong content-type and uses extractEmbedUrl", async () => {
		...
		const result = await simulateOEmbedEndpoint("https://www.youtube.com/watch?v=def456");

		expect(result.type).toBe("embed");
		expect(result.provider).toBe("YouTube");
		expect(result.embedUrl).toBe("https://www.youtube-nocookie.com/embed/def456");
		expect(result.title).toBeUndefined();
	});
	...
	it("rejects oEmbed with oversized content-length", async () => {
		...
		const result = await simulateOEmbedEndpoint("https://www.youtube.com/watch?v=ghi789");
	...
	it("strips HTTP thumbnail URLs from oEmbed response", async () => {
		...
		const result = await simulateOEmbedEndpoint("https://www.youtube.com/watch?v=thumb123");
...
describe("URL normalization in flow", () => {
	it("matches YouTube URL with uppercase hostname", async () => {
		const mockFetch = vi
			.fn()
			.mockResolvedValueOnce(createOEmbedResponse({ type: "video", title: "Test" }));
		globalThis.fetch = mockFetch;
	...
	it("matches Spotify URL with tracking params", async () => {
		const mockFetch = vi
			.fn()
			.mockResolvedValueOnce(createOEmbedResponse({ type: "rich", title: "Track" }));
		globalThis.fetch = mockFetch;
...
describe("OG preview fallback flow", () => {
	it("returns OG preview for unknown URLs", async () => {
		...
		const result = await simulateOEmbedEndpoint("https://example.com/blog/post-1");
	...
	it("returns preview with error when OG fetch fails", async () => {
		const mockFetch = vi.fn().mockRejectedValueOnce(new Error("DNS lookup failed"));
		globalThis.fetch = mockFetch;

		const result = await simulateOEmbedEndpoint("https://nonexistent-domain.example/page");
...
describe("SSRF protection for preview fallback", () => {
	...
	it("blocks cloud metadata endpoints in preview fallback", async () => {
		const result = await simulateOEmbedEndpoint("http://169.254.169.254/latest/meta-data/");
