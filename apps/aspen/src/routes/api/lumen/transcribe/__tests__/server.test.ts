...
vi.mock("@sveltejs/kit", () => ({
	...
	error: (status: number, body: { message: string; code: string; category: string }) => {
...
function createMockRequest(options: {
	...
	if (options.audio !== null) {
		const audioBlob = options.audio ?? new Blob(["test"], { type: "audio/webm" });
...
describe("POST /api/lumen/transcribe - Validation", () => {
	...
	it("should reject invalid audio MIME type", async () => {
		const ctx = createMockContext({});
		const formData = new FormData();
		formData.append("audio", new Blob(["test"], { type: "text/plain" }), "recording.txt");
		ctx.request = new Request("https://test.grove.place/api/lumen/transcribe", {
			method: "POST",
			body: formData,
		});
	...
	it("should reject oversized audio files", { timeout: 15000 }, async () => {
		...
		const formData = new FormData();
		formData.append("audio", new Blob([largeBuffer], { type: "audio/webm" }), "recording.webm");
		ctx.request = new Request("https://test.grove.place/api/lumen/transcribe", {
			method: "POST",
			body: formData,
		});
...
describe("POST /api/lumen/transcribe - Success", () => {
	...
	it("should return gutterContent for draft mode", async () => {
		mockTranscribe.mockResolvedValue({
			...
			model: "@cf/openai/whisper-large-v3-turbo",
			provider: "cloudflare-ai",
			gutterContent: [{ type: "vine", content: "A tangent", anchor: "transcript" }],
...
describe("POST /api/lumen/transcribe - Error Handling", () => {
	...
	it("should return 429 when quota is exceeded", async () => {
		mockTranscribe.mockRejectedValue(new Error("Daily transcription quota exceeded"));
	...
	it("should return 422 when transcription fails to parse", async () => {
		mockTranscribe.mockRejectedValue(new Error("Transcription failed: empty response"));
