...
// Rate limit config is now centralized in @autumnsgrove/lattice/server/rate-limits/config.ts
...
export const POST: RequestHandler = async ({ request, platform, locals }) => {
	...
	try {
		...
		// Uses Threshold SDK from @autumnsgrove/lattice/threshold
		...
		if (exportType === "full" || exportType === "posts") {
         ...
         ORDER BY created_at DESC`,
			...
			exportData.posts = (posts.results || []).map((post) => ({
				...
				publishedAt: post.published_at ? new Date(post.published_at * 1000).toISOString() : null,
		...
		if (exportType === "full" || exportType === "media") {
			const media = await platform.env.DB.prepare(
				`SELECT id, r2_key, parsed_slug, custom_title, alt_text, file_size, cdn_url, created_at
         FROM gallery_images WHERE tenant_id = ?
         ORDER BY created_at DESC`,
			)
				.bind(tenantId)
				.all<MediaRecord>();
			...
			exportData.media = (media.results || []).map((m) => ({
				r2Key: m.r2_key,
				name: m.custom_title || m.parsed_slug,
				url: m.cdn_url,
				size: m.file_size,
				altText: m.alt_text,
