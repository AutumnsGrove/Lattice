...
import { validateUUID, sanitizeFilename } from "@autumnsgrove/lattice/utils/validation";
...
export const GET: RequestHandler = async ({ params, platform, locals }) => {
	...
	try {
       ...
       WHERE id = ? AND tenant_id = ?`,
		...
		if (!record) {
			return new Response(JSON.stringify(buildErrorJson(API_ERRORS.EXPORT_NOT_FOUND)), {
				status: 404,
		...
		if (record.expires_at && record.expires_at < now) {
			return new Response(JSON.stringify(buildErrorJson(API_ERRORS.EXPORT_EXPIRED)), {
				status: 410,
				headers: { "Content-Type": "application/json" },
			});
		}
		...
		const exportDate = new Date(record.created_at * 1000).toISOString().split("T")[0];
