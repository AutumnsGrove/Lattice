...
export const GET: RequestHandler = async ({ params, platform, locals }) => {
	...
	try {
       ...
       WHERE id = ? AND tenant_id = ?`,
		...
		if (ACTIVE_PHASES.includes(status) && now - record.updated_at > STALE_THRESHOLD_SECONDS) {
			status = "failed";
			try {
		...
		const isExpired = record.expires_at && record.expires_at < now && record.status === "complete";
