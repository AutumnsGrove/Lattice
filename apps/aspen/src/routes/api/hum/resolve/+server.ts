...
import {
	detectProvider,
	isMusicUrl,
} from "@autumnsgrove/lattice/ui/components/content/hum/providers";
import type {
	HumMetadata,
	HumProvider,
	HumContentType,
} from "@autumnsgrove/lattice/ui/components/content/hum/types";
...
async function resolveViaOdesli(url: string): Promise<HumMetadata | null> {
	try {
		...
		if (linksByPlatform.spotify?.url) platformLinks["spotify"] = linksByPlatform.spotify.url;
		...
		if (linksByPlatform.tidal?.url) platformLinks["tidal"] = linksByPlatform.tidal.url;
		if (linksByPlatform.deezer?.url) platformLinks["deezer"] = linksByPlatform.deezer.url;
		...
		const title = sanitizeText(entity?.title) || sanitizeText(entity?.artistName);
 ...
 */
async function resolveViaOembed(url: string, provider: HumProvider): Promise<HumMetadata | null> {
...
async function resolveViaItunes(url: string): Promise<HumMetadata | null> {
	...
	try {
		...
		const title = sanitizeText(result.trackName) || sanitizeText(result.collectionName);
		return {
			sourceUrl: url,
...
export const GET: RequestHandler = async ({ url: requestUrl, platform, request }) => {
