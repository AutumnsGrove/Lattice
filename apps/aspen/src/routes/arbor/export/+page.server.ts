...
export const load: PageServerLoad = async ({ locals, platform, parent }) => {
	...
	const pastExports = (exportsResult.results || []).map((exp) => {
		...
		return {
			...
			completedAt: exp.completed_at ? new Date(exp.completed_at * 1000).toISOString() : null,
			expiresAt: exp.expires_at ? new Date(exp.expires_at * 1000).toISOString() : null,
		};
	});

	// Find any active export (for resuming progress display)
	const activeExport = pastExports.find((e) =>
		["pending", "querying", "assembling", "uploading", "notifying"].includes(e.status),
	);
