...
import { ARBOR_ERRORS, throwGroveError, logGroveError } from "@autumnsgrove/lattice/errors";
 ...
 */
function isAdmin(email: string | undefined, allowedEmails: string | undefined): boolean {
...
export const load: PageServerLoad = async ({ locals, platform, url }) => {
	...
	try {
		...
		let countQuery = "SELECT COUNT(*) as count FROM trace_feedback WHERE archived_at IS NULL";
		if (conditions.length > 0) {
			countQuery += " AND " + conditions.join(" AND ");
		}

		// Run all queries in parallel for performance
		const [tracesResult, countResult, statsResult, sourceStatsResult] = await Promise.all([
...
export const actions: Actions = {
	...
	markRead: async ({ request, locals, platform }) => {
		if (!locals.user || !isAdmin(locals.user.email, platform?.env?.ALLOWED_ADMIN_EMAILS)) {
			return fail(403, {
		...
		try {
			...
			await DB.prepare("UPDATE trace_feedback SET read_at = ? WHERE id = ?").bind(now, id).run();
