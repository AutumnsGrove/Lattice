<script lang="ts">
	...
	import {
		...
		formatStatusTime,
		type StatusPreset,
	} from "@autumnsgrove/lattice/curios/activitystatus";
	...
	// Current status info
	const hasStatus = $derived(data.currentStatus?.statusText || data.currentStatus?.statusEmoji);
	const isExpired = $derived(
		data.currentStatus?.expiresAt ? isStatusExpired(data.currentStatus.expiresAt) : false,
	);
...
<div class="activitystatus-admin">
	...
	<GlassCard class="current-card">
		...
		{:else}
			<p class="empty-state">No status set. Pick a preset or write your own.</p>
		{/if}
	</GlassCard>
	...
	<GlassCard class="set-card">
						...
						<div class="preset-grid">
							{#each presets as preset}
								...
								>
									...
									{#if expiresInHours}
										<input type="hidden" name="expiresInHours" value={expiresInHours} />
									{/if}
									<button type="submit" class="preset-btn" disabled={isSubmitting}>
		...
		<div class="expiration-section">
			...
			<select id="expiration" bind:value={expiresInHours} class="select-input">
