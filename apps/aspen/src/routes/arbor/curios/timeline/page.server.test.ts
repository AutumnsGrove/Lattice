...
import { encryptToken, isEncryptedToken } from "@autumnsgrove/lattice/server/encryption";
...
async function processAdminFormTokens(
	...
	if (!encryptionKey && (githubToken?.trim() || openrouterKey?.trim())) {
		warnings.push("TOKEN_ENCRYPTION_KEY not set - tokens will be stored unencrypted");
	}
...
describe("Admin Page Token Encryption", () => {
	describe("processAdminFormTokens", () => {
		...
		it("should encrypt only GitHub token when OpenRouter is empty", async () => {
			const result = await processAdminFormTokens("ghp_github_token_12345", null, TEST_KEY);
		...
		it("should encrypt only OpenRouter key when GitHub is empty", async () => {
			const result = await processAdminFormTokens(null, "sk-or-openrouter-key", TEST_KEY);
		...
		it("should store plaintext and warn when encryption key is missing", async () => {
			...
			const result = await processAdminFormTokens(githubToken, openrouterKey, undefined);
		...
		it("should return empty string for CLEAR_TOKEN_VALUE (explicit clear)", async () => {
			const result = await processAdminFormTokens(CLEAR_TOKEN_VALUE, CLEAR_TOKEN_VALUE, TEST_KEY);
		...
		it("should clear only OpenRouter when GitHub is set new", async () => {
			const result = await processAdminFormTokens("ghp_new_token", CLEAR_TOKEN_VALUE, TEST_KEY);
	...
	describe("End-to-End Admin Flow", () => {
		...
		it("should preserve existing tokens when form fields are empty", async () => {
			...
			const existingEncrypted = await encryptToken("ghp_existing_token", TEST_KEY);
			...
			const mockDbRow = {
				github_token_encrypted: saveResult.githubTokenForDb ?? existingEncrypted,
				openrouter_key_encrypted: saveResult.openrouterKeyForDb,
			};
		...
		it("should clear tokens via CLEAR_TOKEN_VALUE and show as not configured", async () => {
			...
			const simulateCaseNull = (newVal: string | null, existing: string | null) =>
				newVal === "" ? null : (newVal ?? existing);

			const afterClearDb = {
				github_token_encrypted: simulateCaseNull(clearResult.githubTokenForDb, existingGithub),
