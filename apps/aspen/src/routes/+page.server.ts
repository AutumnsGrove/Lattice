...
export const load: PageServerLoad = async ({ platform, locals, setHeaders }) => {
	...
	if (db && tenantId) {
		try {
           ...
           LIMIT 1`,
			...
			console.warn("[Home] D1 fetch failed, falling back to filesystem:", (err as Error)?.message);
	...
	if (postData) {
		...
		const processedHtml = processAnchorTags(String(postData.html_content || ""));
		...
		// Safe JSON parsing for gutter content
		let gutterContent: GutterItem[] = [];
		if (postData.gutter_content && typeof postData.gutter_content === "string") {
			try {
