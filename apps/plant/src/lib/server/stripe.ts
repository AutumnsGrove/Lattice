...
export async function createCheckoutSession(
	...
	if (!data.url || !data.id) {
		...
		throw new Error(
			`${PLANT_ERRORS.STRIPE_CHECKOUT_NO_URL.code}: ${PLANT_ERRORS.STRIPE_CHECKOUT_NO_URL.adminMessage}`,
		);
	}
...
export async function verifyWebhookSignature(
	...
	try {
		...
		const matched = v1Signatures.some((v1Sig) => secureCompare(expectedSignature, v1Sig));
...
export async function createBillingPortalSession(
	...
	if (!data.url) {
		...
		throw new Error(
			`${PLANT_ERRORS.STRIPE_PORTAL_NO_URL.code}: ${PLANT_ERRORS.STRIPE_PORTAL_NO_URL.adminMessage}`,
		);
	}
