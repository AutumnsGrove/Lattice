...
describe("Email Verification Service", () => {
	...
	describe("normalizeEmail", () => {
		...
		it("should throw on invalid email format", () => {
			expect(() => normalizeEmail("notanemail")).toThrow(PLANT_ERRORS.INVALID_EMAIL_FORMAT.code);
			expect(() => normalizeEmail("missing@tld")).toThrow(PLANT_ERRORS.INVALID_EMAIL_FORMAT.code);
		});
	});
	...
	describe("Rate Limiting Logic", () => {
		...
		it("should reset after window expires", () => {
			...
			const windowStart = Math.floor(Date.now() / 1000) - RESEND_WINDOW_SECONDS - 1;
