...
async function evaluateDeepCheck(
	...
	if (!response.ok) {
		return {
			...
			componentName: config.name,
			status:
				response.status >= 500 ? ComponentStatus.MAJOR_OUTAGE : ComponentStatus.PARTIAL_OUTAGE,
			latencyMs,
	...
	try {
		...
		const latencyResult = classifyByLatency(config, latencyMs, response.status, timestamp);
...
function evaluateShallowCheck(
	...
	if (httpStatus < 200 || httpStatus >= 300) {
		return {
			componentId: config.id,
			componentName: config.name,
			status: httpStatus >= 500 ? ComponentStatus.MAJOR_OUTAGE : ComponentStatus.PARTIAL_OUTAGE,
			latencyMs,
			httpStatus,
