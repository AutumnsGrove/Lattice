...
<script lang="ts">
	...
	async function handleReact(postId: string, emoji: string) {
		...
		try {
			...
			const res = await fetch(`/api/feed/${postId}/reaction`, {
				// csrf-ok
				method,
				credentials: "include",
...
<main class="mx-auto max-w-2xl px-4 py-8">
	<!-- Back link -->
	<div class="mb-6">
		<a href="/feed" class="text-sm text-foreground-muted hover:text-foreground transition-colors">
			&larr; Back to feed
		</a>
	</div>
		...
		{#if Object.keys(post.reactionCounts).length > 0}
			<div class="flex flex-wrap gap-2 px-6 pb-4">
				{#each Object.entries(post.reactionCounts) as [emoji, count]}
					<span
						class="inline-flex items-center gap-1 rounded-full bg-grove-50/60 px-2 py-0.5 text-sm dark:bg-cream-100/20"
					>
						<span>{emoji}</span>
