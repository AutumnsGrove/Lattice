...
{#if status === 'success'}
	<div
		class="{variant === 'dark' ? 'bg-surface-subtle border-border' : 'bg-accent border-accent'} border rounded-lg px-6 py-4 text-center max-w-md animate-in"
	>
...
{:else if status === 'already_signed_up'}
	<div
		class="{variant === 'dark' ? 'bg-surface-subtle border-border' : 'bg-accent border-accent'} border rounded-lg px-6 py-4 text-center max-w-md animate-in"
	>
		<svg class="{variant === 'dark' ? 'text-accent-muted' : 'text-accent-muted'} w-8 h-8 mx-auto mb-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
{:else}
	<form onsubmit={handleSubmit} class="w-full max-w-md">
		...
		{#if status === 'error'}
			<p class="{variant === 'dark' ? 'text-error' : 'text-error'} text-sm mt-2 font-sans">{errorMessage}</p>
		{/if}
