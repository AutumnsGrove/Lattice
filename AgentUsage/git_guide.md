...
When creating pull requests, use this exact format:

### PR Title
Use conventional commits format:
```
feat: Add dark mode toggle
fix: Correct timezone handling bug
...
Adds dark mode toggle to settings page, allowing users to switch themes.
Addresses user feedback about eye strain during nighttime use.
...
Manually tested toggle in Chrome and Firefox. Verified theme persists
across page reloads. Checked accessibility contrast ratios in both modes.
EOF
)"
```
...
- **Be specific in Changes** — "Add validateEmail() to auth module" not "Update files"
- **Never leave placeholders** — Replace all `[brackets]` and HTML comments
- **Describe actual testing** — What did you do to verify it works?
