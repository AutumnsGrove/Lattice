...
import { checkRateLimit, getEndpointLimitByKey } from "$lib/server/rate-limits/index.js";
...
describe("POST /api/export/start - Success", () => {
	...
	it("should write audit log entry", async () => {
		...
		const logs = await mockDB
			.prepare("SELECT * FROM audit_log WHERE category = ? AND action = ? AND tenant_id = ?")
...
describe("Email Template - export-ready", () => {
	...
	it("should include download URL in HTML", () => {
		...
		expect(html).toContain('href="https://test-tenant.grove.place/export/download"');
