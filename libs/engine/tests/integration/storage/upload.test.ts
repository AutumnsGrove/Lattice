...
vi.mock("$lib/utils/upload-validation.js", () => ({
	...
	isAllowedImageType: vi.fn((type) =>
		["image/jpeg", "image/png", "image/gif", "image/webp", "image/jxl"].includes(type),
	),
...
function createJpegFormData(fileBuffer?: Uint8Array): FormData {
	...
	const buffer = fileBuffer || new Uint8Array([0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10]);
...
function createGifFormData(fileBuffer?: Uint8Array): FormData {
	...
	const buffer =
		fileBuffer || new Uint8Array([0x47, 0x49, 0x46, 0x38, 0x39, 0x61, 0x00, 0x00, 0x00, 0x00]);
...
import { isAllowedImageType, validateFileSignature } from "$lib/utils/upload-validation.js";
		...
		mockThresholdCheckWithResult.mockResolvedValue({
			...
			response: new Response(
				JSON.stringify({
					...
					retryAfter: 3600,
					resetAt: new Date((Math.floor(Date.now() / 1000) + 3600) * 1000).toISOString(),
				}),
				{
					status: 429,
					headers: {
