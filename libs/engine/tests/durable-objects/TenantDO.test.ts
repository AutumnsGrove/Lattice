...
import { createJsonRequest, resetTestCounters, getTenantDOStub } from "../utils/test-helpers.js";
...
describe("TenantDO Config", () => {
	...
	it("should return cached config on GET /config", async () => {
		...
		// Get config
		const configResponse = await stub.fetch(new Request("https://tenant.internal/config"));
	...
	it("should return 404 for uninitialized tenant", async () => {
		...
		const response = await stub.fetch(new Request("https://tenant.internal/config"));
...
describe("TenantDO Draft Sync", () => {
	it("should create a new draft", async () => {
		...
		const data = (await draftResponse.json()) as any;
	...
	it("should retrieve an existing draft", async () => {
		...
		// Retrieve draft
		const getResponse = await stub.fetch(new Request("https://tenant.internal/drafts/get-test"));

		expect(getResponse.ok).toBe(true);
		const draft = (await getResponse.json()) as any;
	...
	it("should list all drafts", async () => {
		...
		// List drafts
		const listResponse = await stub.fetch(new Request("https://tenant.internal/drafts"));

		expect(listResponse.ok).toBe(true);
		const drafts = (await listResponse.json()) as any;
	...
	it("should delete a draft", async () => {
		...
		// Verify deletion
		const getResponse = await stub.fetch(new Request("https://tenant.internal/drafts/to-delete"));
	...
	it("should return 404 for non-existent draft", async () => {
		...
		const response = await stub.fetch(new Request("https://tenant.internal/drafts/does-not-exist"));
...
describe("TenantDO Error Handling", () => {
	...
	it("should return 404 for unknown routes", async () => {
		...
		const response = await stub.fetch(new Request("https://tenant.internal/unknown-endpoint"));
