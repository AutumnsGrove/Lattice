...
import { createJsonRequest, resetTestCounters, getPostMetaDOStub } from "../utils/test-helpers.js";
...
describe("PostMetaDO View Counting", () => {
	it("should increment view count", async () => {
		...
		const data = (await viewResponse.json()) as any;
	...
	it("should rate limit views from same session (5 min window)", async () => {
		...
		const data = (await secondView.json()) as any;
	...
	it("should count views from different sessions", async () => {
		...
		const metaResponse = await stub.fetch(new Request("https://post.internal/meta"));
...
describe("PostMetaDO Popular Post Thresholds", () => {
	...
	it("should recalculate popular status on tier upgrade", async () => {
		...
		let data = (await meta.json()) as any;
		expect(data.isPopular).toBe(false);
...
describe("PostMetaDO Reactions", () => {
	it("should add a like reaction", async () => {
		...
		const data = (await likeResponse.json()) as any;
	...
	it("should add a bookmark reaction", async () => {
		...
		const data = (await bookmarkResponse.json()) as any;
	...
	it("should prevent duplicate reactions from same user", async () => {
		...
		const data = (await dupeResponse.json()) as any;
	...
	it("should remove a reaction", async () => {
		...
		const data = (await removeResponse.json()) as any;
	...
	it("should get reactions via GET endpoint", async () => {
		...
		// Get reactions
		const getResponse = await stub.fetch(new Request("https://post.internal/reactions"));
...
describe("PostMetaDO Presence", () => {
	it("should track active readers", async () => {
		...
		// Get presence
		const presenceResponse = await stub.fetch(new Request("https://post.internal/presence"));

		expect(presenceResponse.ok).toBe(true);
		const data = (await presenceResponse.json()) as any;
...
describe("PostMetaDO Error Handling", () => {
	it("should return 404 for uninitialized meta GET", async () => {
		...
		const response = await stub.fetch(new Request("https://post.internal/meta"));
	...
	it("should return 404 for unknown routes", async () => {
		...
		const response = await stub.fetch(new Request("https://post.internal/unknown-path"));
