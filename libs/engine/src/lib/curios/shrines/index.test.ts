...
describe("SIZE_DIMENSIONS", () => {
	it("has dimensions for all sizes", () => {
		for (const s of SIZE_OPTIONS) {
			const dim = SIZE_DIMENSIONS[s.value];
...
describe("CONTENT_TYPE_OPTIONS", () => {
	it("includes text, icon, date, quote, decoration", () => {
		const types = CONTENT_TYPE_OPTIONS.map((c) => c.value);
...
describe("SHRINE_TEMPLATES", () => {
	it("has a template for every shrine type", () => {
		for (const t of SHRINE_TYPE_OPTIONS) {
			expect(SHRINE_TEMPLATES[t.value]).toBeDefined();
			expect(Array.isArray(SHRINE_TEMPLATES[t.value])).toBe(true);
		}
	});

	it("blank template is empty", () => {
		expect(SHRINE_TEMPLATES.blank).toHaveLength(0);
	});

	it("non-blank templates have items with valid positions", () => {
		for (const [type, items] of Object.entries(SHRINE_TEMPLATES)) {
