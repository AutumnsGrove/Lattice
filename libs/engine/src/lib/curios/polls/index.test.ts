...
describe("Polls constants", () => {
	it("has poll type options", () => {
		...
		expect(POLL_TYPE_OPTIONS.map((o) => o.value)).toEqual(["single", "multiple"]);
	});
	...
	it("has container style options", () => {
		...
		expect(CONTAINER_STYLE_OPTIONS.map((o) => o.value)).toEqual(["glass", "bulletin", "minimal"]);
	});
...
describe("isValidEmoji", () => {
	...
	it("accepts emoji with skin tone modifiers", () => {
		expect(isValidEmoji("👋🏽")).toBe(true);
	});

	it("rejects plain text", () => {
		expect(isValidEmoji("hello")).toBe(false);
		expect(isValidEmoji("spring")).toBe(false);
	});

	it("rejects mixed text and emoji", () => {
		expect(isValidEmoji("hi🌸")).toBe(false);
		expect(isValidEmoji("🌸cool")).toBe(false);
	});
	...
	it("accepts pictographic star emoji", () => {
		expect(isValidEmoji("⭐")).toBe(true);
	});
...
describe("parseOptions", () => {
	...
	it("strips empty emoji and color fields", () => {
		const options = [{ id: "opt_1", text: "Test", emoji: "", color: "" }];
		const result = parseOptions(JSON.stringify(options));
