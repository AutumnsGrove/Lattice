...
describe("Badges constants", () => {
	it("has 6 category options", () => {
		expect(BADGE_CATEGORY_OPTIONS).toHaveLength(6);
	...
	it("has 5 rarity options", () => {
		expect(BADGE_RARITY_OPTIONS).toHaveLength(5);
...
describe("sanitizeBadgeDescription", () => {
	...
	it("truncates to max length", () => {
		...
		expect(sanitizeBadgeDescription(long)).toHaveLength(MAX_BADGE_DESCRIPTION_LENGTH);
	});
});
...
describe("getRarityColor", () => {
	it("returns bark for common", () => {
		expect(getRarityColor("common")).toBe("#8B7355");
	});

	it("returns leaf for uncommon", () => {
		expect(getRarityColor("uncommon")).toBe("#5A9E6F");
	});

	it("returns crystal for rare", () => {
		expect(getRarityColor("rare")).toBe("#6BA3BE");
	});

	it("returns amethyst for epic", () => {
		expect(getRarityColor("epic")).toBe("#9B7DB8");
	});

	it("returns amber for legendary", () => {
		expect(getRarityColor("legendary")).toBe("#D4A056");
	});

	it("returns bark fallback for unknown", () => {
		expect(getRarityColor("mythic" as any)).toBe("#8B7355");
	});
});
