...
describe("getEventDisplayKey", () => {
	...
	it("returns pull_request_opened for opened PRs", () => {
		expect(getEventDisplayKey("pull_request", "opened")).toBe("pull_request_opened");
	});

	it("returns pull_request_opened for reopened PRs", () => {
		expect(getEventDisplayKey("pull_request", "reopened")).toBe("pull_request_opened");
	});

	it("returns pull_request_closed for closed (not merged) PRs", () => {
		expect(getEventDisplayKey("pull_request", "closed", { merged: false })).toBe(
			"pull_request_closed",
		);
	});
...
describe("formatRelativeTime", () => {
	...
	it("returns formatted date for older timestamps", () => {
		const now = Math.floor(Date.now() / 1000);
		// 31 days — past the 30-day threshold for absolute dates
		const result = formatRelativeTime(now - 86400 * 31);
...
describe("buildWebhookUrl", () => {
	it("builds correct webhook URL for a tenant", () => {
		expect(buildWebhookUrl("my-tenant")).toBe(`${PULSE_WEBHOOK_BASE}/my-tenant`);
	});

	it("preserves tenant ID format", () => {
		expect(buildWebhookUrl("abc-123")).toContain("abc-123");
		expect(buildWebhookUrl("tenant_with_underscores")).toContain("tenant_with_underscores");
	});
});
