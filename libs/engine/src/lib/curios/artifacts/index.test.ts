...
describe("rowToRecord", () => {
	it("transforms a database row with all v2 fields", () => {
		...
		expect(record.name).toBe("Cozy Candle");
...
describe("evaluateDiscoveryRules", () => {
	...
	it("evaluates dark-mode rule", () => {
		const rules: DiscoveryCondition[] = [{ type: "dark-mode", value: "true" }];
	...
	it("evaluates scroll-depth rule", () => {
		const rules: DiscoveryCondition[] = [{ type: "scroll-depth", value: 80 }];
	...
	it("evaluates time-of-day rule (normal range)", () => {
		const rules: DiscoveryCondition[] = [{ type: "time-of-day", value: "10-18" }];
	...
	it("evaluates time-of-day rule (overnight range)", () => {
		const rules: DiscoveryCondition[] = [{ type: "time-of-day", value: "22-6" }];
	...
	it("evaluates day-of-week rule", () => {
		const rules: DiscoveryCondition[] = [{ type: "day-of-week", value: "5,6" }];
	...
	it("evaluates season rule", () => {
		const rules: DiscoveryCondition[] = [{ type: "season", value: "winter" }];
	...
	it("evaluates specific-date rule", () => {
		const rules: DiscoveryCondition[] = [{ type: "specific-date", value: "10-31" }];
	...
	it("evaluates pages-visited rule", () => {
		const rules: DiscoveryCondition[] = [{ type: "pages-visited", value: 3 }];
	...
	it("evaluates time-on-site rule", () => {
		const rules: DiscoveryCondition[] = [{ type: "time-on-site", value: 120 }];
	...
	it("uses AND logic for multiple rules", () => {
		...
		expect(evaluateDiscoveryRules(rules, { isDarkMode: true, scrollDepth: 80 })).toBe(true);
		expect(evaluateDiscoveryRules(rules, { isDarkMode: true, scrollDepth: 20 })).toBe(false);
		expect(evaluateDiscoveryRules(rules, { isDarkMode: false, scrollDepth: 80 })).toBe(false);
	});
});
...
describe("isValidArtifactName", () => {
	it("accepts valid names", () => {
		expect(isValidArtifactName("My Candle")).toBe(true);
		expect(isValidArtifactName("a")).toBe(true);
		expect(isValidArtifactName("Fortune Cookie #1")).toBe(true);
	});
	...
	it("rejects names exceeding max length", () => {
		const tooLong = "a".repeat(MAX_ARTIFACT_NAME_LENGTH + 1);
		expect(isValidArtifactName(tooLong)).toBe(false);
	});

	it("accepts name at max length boundary", () => {
		const exact = "a".repeat(MAX_ARTIFACT_NAME_LENGTH);
		expect(isValidArtifactName(exact)).toBe(true);
	});
...
describe("ARTIFACT_CONFIG_FIELDS", () => {
	it("has an entry for every artifact type", () => {
		for (const t of ARTIFACT_TYPES) {
			expect(ARTIFACT_CONFIG_FIELDS).toHaveProperty(t.value);
			expect(Array.isArray(ARTIFACT_CONFIG_FIELDS[t.value as ArtifactType])).toBe(true);
		}
	});

	it("covers all 20 types", () => {
		expect(Object.keys(ARTIFACT_CONFIG_FIELDS)).toHaveLength(20);
	});

	it("has valid field types", () => {
		const validTypes = new Set(["text", "textarea", "select", "color", "date", "number", "toggle"]);
		for (const [, fields] of Object.entries(ARTIFACT_CONFIG_FIELDS)) {
	...
	it("select fields have options arrays", () => {
		for (const [, fields] of Object.entries(ARTIFACT_CONFIG_FIELDS)) {
			for (const field of fields) {
	...
	it("terrariumglobe has no config fields", () => {
		expect(ARTIFACT_CONFIG_FIELDS.terrariumglobe).toEqual([]);
	});

	it("hourglass has required date and text fields", () => {
		const fields = ARTIFACT_CONFIG_FIELDS.hourglass;
		expect(fields.some((f) => f.key === "eventName" && f.required)).toBe(true);
		expect(fields.some((f) => f.key === "targetDate" && f.type === "date" && f.required)).toBe(
			true,
		);
	});

	it("moodcandle has flame color select", () => {
		const fields = ARTIFACT_CONFIG_FIELDS.moodcandle;
		const flameField = fields.find((f) => f.key === "flameColor");
		expect(flameField).toBeDefined();
		expect(flameField!.type).toBe("select");
		expect(flameField!.options).toHaveLength(5);
	});
});
