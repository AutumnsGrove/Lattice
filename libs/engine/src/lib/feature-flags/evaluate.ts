 ...
 * with default values.
 */
async function loadFlagWithRules(flagId: string, db: D1Database): Promise<FeatureFlag | null> {
	...
	try {
		const rulesResult = await db
			.prepare("SELECT * FROM flag_rules WHERE flag_id = ? AND enabled = 1 ORDER BY priority DESC")
			.bind(flagId)
			.all<FlagRuleRow>();
...
function safeRowToRule(row: FlagRuleRow): FlagRule | null {
	const ruleValue = parseJsonSafe<RuleCondition>(row.rule_value, `rule ${row.id} rule_value`);
	const resultValue = parseJsonSafe<unknown>(row.result_value, `rule ${row.id} result_value`);
...
function safeRowToFlag(row: FeatureFlagRow, rules: FlagRule[]): FeatureFlag {
	const defaultValue = parseJsonSafe<unknown>(row.default_value, `flag ${row.id} default_value`);
	...
	return {
		...
		maturity: (row.maturity || "experimental") as FlagMaturity,
...
export async function evaluateFlag<T = unknown>(
	...
	// 3.5. Graduated fast-path: always-on, skip rule evaluation
	// Graduated flags are fully shipped but kept in DB for rollback
	if (flag.maturity === "graduated") {
		const result: EvaluationResult<T> = {
			value: true as T,
			flagId,
			matched: true,
	...
	// 3.6. Check greenhouse_only gate
