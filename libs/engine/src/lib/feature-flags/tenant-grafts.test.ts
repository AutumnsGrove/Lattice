...
describe("Tenant Grafts API", () => {
	...
	describe("setTenantGraftOverride", () => {
		it("creates an override for a valid greenhouse-only flag", async () => {
			...
			// Act
			const result = await setTenantGraftOverride("fireside_mode", "tenant-123", false, env);
		...
		it("rejects non-greenhouse-only flags", async () => {
			...
			// Act
			const result = await setTenantGraftOverride("public_flag", "tenant-123", false, env);

			// Assert
			expect(result).toBe(false);
			expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("not greenhouse-only"));
		...
		it("rejects non-existent flags", async () => {
			...
			// Act
			const result = await setTenantGraftOverride("nonexistent_flag", "tenant-123", false, env);
		...
		it("updates existing override instead of creating duplicate", async () => {
			...
			// Act
			const result = await setTenantGraftOverride("fireside_mode", "tenant-123", true, env);
	...
	describe("removeTenantGraftOverride", () => {
		it("removes an existing override", async () => {
			...
			// Act
			const result = await removeTenantGraftOverride("fireside_mode", "tenant-123", env);
	...
	describe("resetTenantGraftOverrides", () => {
		it("removes all overrides for a tenant", async () => {
			...
			mockDB.prepare = vi.fn((sql: string) => {
				if (sql.includes("DELETE FROM flag_rules") && sql.includes("tenant_override")) {
					deleteCalled = true;
					return {
						bind: vi.fn(() => ({
...
describe("Tenant Grafts Category Detection", () => {
	...
	it("categorizes flags by ID prefix", async () => {
		...
		// Assert
		expect(grafts.find((g) => g.id === "beta_dark_theme")?.maturity).toBe("beta");
		expect(grafts.find((g) => g.id === "stable_markdown_v2")?.maturity).toBe("stable");
		expect(grafts.find((g) => g.id === "fireside_mode")?.maturity).toBe("experimental");
	});
});
