 ...
 */
export function createFlagRow(id: string, options: Partial<FeatureFlagRow> = {}): FeatureFlagRow {
	return {
		id,
		...
		maturity: options.maturity ?? "experimental",
 ...
 */
export function createRuleRow(flagId: string, options: Partial<FlagRuleRow> = {}): FlagRuleRow {
	return {
		id: options.id ?? 1,
		flag_id: flagId,
...
export function setupMockFlag(
	...
	const ruleRows = rules.map((r, i) => createRuleRow(flagId, { ...r, id: r.id ?? i + 1 }));
