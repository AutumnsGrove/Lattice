...
import { isFeatureEnabled, getFeatureValue, getVariant, getFlag, getFlags } from "./index.js";
...
describe("Feature Flags Evaluation", () => {
	...
	describe("Rule Evaluation", () => {
		it("matches tenant rules", async () => {
			setupMockFlag(env, "tenant_flag", { enabled: 1, default_value: "false" }, [
				{
					rule_type: "tenant",
			...
			const matchResult = await isFeatureEnabled("tenant_flag", { tenantId: "tenant1" }, env);
			const noMatchResult = await isFeatureEnabled("tenant_flag", { tenantId: "tenant3" }, env);
		...
		it("matches tier rules", async () => {
			...
			const oakResult = await isFeatureEnabled("tier_flag", { tier: "oak" }, env);
			const seedlingResult = await isFeatureEnabled("tier_flag", { tier: "seedling" }, env);
		...
		it("matches user rules", async () => {
			...
			const matchResult = await isFeatureEnabled("user_flag", { userId: "user123" }, env);
			const noMatchResult = await isFeatureEnabled("user_flag", { userId: "user789" }, env);
		...
		it("matches always rules", async () => {
			setupMockFlag(env, "always_flag", { enabled: 1, default_value: "false" }, [
				{
					rule_type: "always",
					rule_value: "{}",
					result_value: "true",
		...
		it("respects rule priority order", async () => {
			...
			const specialResult = await getVariant("priority_flag", { tenantId: "special" }, env);
			const normalResult = await getVariant("priority_flag", { tenantId: "normal" }, env);
	...
	describe("Graduated Flag Fast-Path", () => {
		it("returns true for graduated flags regardless of default value", async () => {
			setupMockFlag(env, "graduated_flag", {
				enabled: 1,
				default_value: "false",
				maturity: "graduated",
			});

			const result = await isFeatureEnabled("graduated_flag", {}, env);
			expect(result).toBe(true);
		});

		it("returns true for graduated flags even when rules would say false", async () => {
			setupMockFlag(
				env,
				"graduated_with_rules",
				{
					enabled: 1,
					default_value: "false",
					maturity: "graduated",
				...
				[
					{
						rule_type: "tenant",
						rule_value: JSON.stringify({ tenantIds: ["specific_only"] }),
						result_value: "false",
						priority: 1,
					},
				],
			...
			// Even a non-matching tenant gets true — graduated bypasses rules entirely
		...
		it("returns true for graduated flags even for non-greenhouse tenants", async () => {
			setupMockFlag(env, "graduated_greenhouse", {
				enabled: 1,
				default_value: "false",
				greenhouse_only: 1,
				maturity: "graduated",
			...
			// Non-greenhouse tenant still gets true — graduation means shipped to everyone
			const result = await isFeatureEnabled(
				"graduated_greenhouse",
				{ tenantId: "t1", inGreenhouse: false },
				env,
			);
			expect(result).toBe(true);
		});

		it("respects kill switch even for graduated flags", async () => {
			setupMockFlag(env, "disabled_graduated", {
				enabled: 0,
				default_value: "false",
				maturity: "graduated",
			});

			// Disabled flag returns default value, not true
			const result = await isFeatureEnabled("disabled_graduated", {}, env);
			expect(result).toBe(false);
		});

		it("does not apply graduated fast-path to experimental flags", async () => {
			setupMockFlag(env, "experimental_flag", {
				enabled: 1,
				default_value: "false",
			...
			// Non-greenhouse tenant should be blocked by greenhouse gate
			const result = await isFeatureEnabled(
				"experimental_flag",
				{ tenantId: "t1", inGreenhouse: false },
				env,
		...
		it("does not apply graduated fast-path to beta flags", async () => {
			setupMockFlag(env, "beta_flag", {
				enabled: 1,
				default_value: "false",
				maturity: "beta",
			});

			// Beta flag with false default and no matching rules returns false
			const result = await isFeatureEnabled("beta_flag", {}, env);
			expect(result).toBe(false);
		});

		it("provides full evaluation result with matched=true for graduated flags", async () => {
			setupMockFlag(env, "graduated_full", {
				enabled: 1,
				default_value: "false",
				maturity: "graduated",
			});

			const result = await getFlag("graduated_full", {}, env);
			expect(result.value).toBe(true);
			expect(result.matched).toBe(true);
			expect(result.flagId).toBe("graduated_full");
			expect(result.matchedRuleId).toBeUndefined();
		});
	});

	describe("Greenhouse Gate", () => {
		it("blocks non-greenhouse tenants from greenhouse-only flags", async () => {
			setupMockFlag(env, "greenhouse_flag", {
				enabled: 1,
				default_value: "true",
				greenhouse_only: 1,
				maturity: "experimental",
		...
		it("allows greenhouse tenants to access greenhouse-only flags", async () => {
			setupMockFlag(env, "greenhouse_flag_2", {
				enabled: 1,
				default_value: "true",
				greenhouse_only: 1,
				maturity: "experimental",
	...
	describe("getFeatureValue", () => {
		...
		it("returns typed value when flag matches", async () => {
			setupMockFlag(env, "number_flag", { enabled: 1, flag_type: "json", default_value: "50" }, [
				{
					rule_type: "always",
					rule_value: "{}",
					result_value: "100",
