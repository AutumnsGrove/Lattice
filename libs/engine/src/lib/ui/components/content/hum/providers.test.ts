...
describe("detectProvider", () => {
	...
	describe("YouTube Music", () => {
		...
		it("does NOT match regular YouTube URLs", () => {
			expect(detectProvider("https://youtube.com/watch?v=dQw4w9WgXcQ")).toBe("unknown");
			expect(detectProvider("https://www.youtube.com/watch?v=dQw4w9WgXcQ")).toBe("unknown");
		});
	});
	...
	describe("SoundCloud", () => {
		...
		it("does NOT match SoundCloud profile URLs", () => {
			...
			expect(detectProvider("https://soundcloud.com/artist-name")).toBe("unknown");
		});
	});
