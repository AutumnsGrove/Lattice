/**
 * Threshold — Abuse Tracking & Graduated Response
 *
 ...
 *   violation counts may undercount slightly. Acceptable for abuse tracking.
...
export interface AbuseState {
  violations: number;
  lastViolation: number; // Unix timestamp (seconds)
...
export async function getAbuseState(
  ...
  try {
    ...
    console.error("[threshold:abuse] Failed to get abuse state:", error);
    return { violations: 0, lastViolation: 0, bannedUntil: null };
  }
}
...
export async function recordViolation(
  ...
  try {
    ...
    console.error("[threshold:abuse] Failed to record violation:", error);
  }
...
export async function clearAbuseState(
  ...
  try {
    ...
    console.error("[threshold:abuse] Failed to clear abuse state:", error);
  }
}
