...
describe("Sanitization Security Tests", () => {
	...
	describe("Nested and Malformed Tags", () => {
		...
		it("handles tags split across lines", () => {
			...
			// The tag itself is stripped; text content like "alert(1)" may remain
			// as safe plain text (not executable without a script element)
			const payload = `<scr
ipt>alert(1)</scr
ipt>`;
			const result = sanitizeHTML(payload);
			expect(result).not.toContain("<script");
			expect(result).not.toMatch(/<scr\s*ipt/i);
		});
	});
