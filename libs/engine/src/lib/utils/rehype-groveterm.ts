...
export interface RehypeGroveTermOptions {
	/** Manifest mapping term slugs to definitions */
 ...
 */
function processTextNode(text: string, options: RehypeGroveTermOptions): Array<Text | Html> {
 ...
 *   .use(remarkParse)
 *   .use(rehypeGroveTerm, { manifest: manifestData })
 ...
 *
 * const mdsvexConfig = {
 *   remarkPlugins: [[rehypeGroveTerm, { manifest: manifestData }]],
 * };
 * ```
 */
export const rehypeGroveTerm: Plugin<[RehypeGroveTermOptions], Root> = function (
 ...
 * @param options - Processing options (manifest is required)
 * @returns Text with [[term]] replaced by GroveTerm HTML
 * @throws Error if manifest is not provided
 */
