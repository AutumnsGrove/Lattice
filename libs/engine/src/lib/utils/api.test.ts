...
describe("API Utilities", () => {
	...
	describe("getCSRFToken", () => {
		...
		describe("Cookie Extraction", () => {
			...
			it("extracts token from cookie with multiple cookies", () => {
				mockDocument.cookie = "session=abc123; csrf_token=secure-token-xyz; user=john";
				const token = getCSRFToken();
				expect(token).toBe("secure-token-xyz");
			});

			it("extracts token with spaces around semicolon (with trailing space)", () => {
				mockDocument.cookie = "session=abc ; csrf_token=token-value ; user=john";
				const token = getCSRFToken();
	...
	describe("apiRequest", () => {
		...
		describe("Response Handling - Error", () => {
			it("throws error on non-ok response with JSON error message", async () => {
				...
				await expect(apiRequest("http://example.com/api", { method: "POST" })).rejects.toThrow(
			...
			it("uses default error message when JSON parses but has no message property", async () => {
				...
				await expect(apiRequest("http://example.com/api")).rejects.toThrow("Request failed");
			});

			it("handles error response with error field (Grove API pattern)", async () => {
				mockFetch.mockResolvedValue({
					ok: false,
					status: 403,
				...
				await expect(apiRequest("http://example.com/api")).rejects.toThrow("Access denied");
			});
		...
		describe("Type Parameters", () => {
			it("supports generic type for response", async () => {
				...
				const result = await apiRequest<UserResponse>("http://example.com/api/user");
	...
	describe("api.post", () => {
		...
		it("supports generic type parameter", async () => {
			...
			const result = await api.post<CreateResponse>("http://example.com/api", {});
	...
	describe("api.put", () => {
		...
		it("supports generic type parameter", async () => {
			...
			const result = await api.put<UpdateResponse>("http://example.com/api/1", {});
	...
	describe("api.delete", () => {
		...
		it("supports generic type parameter", async () => {
			...
			const result = await api.delete<DeleteResponse>("http://example.com/api/1");
	...
	describe("api.patch", () => {
		...
		it("supports generic type parameter", async () => {
			...
			const result = await api.patch<PatchResponse>("http://example.com/api/1", {});
	...
	describe("Integration Scenarios", () => {
		it("handles full request flow with CSRF token from cookie", async () => {
			...
			expect(callArgs[1].headers["Content-Type"]).toBe("application/json");
			expect(callArgs[1].credentials).toBe("include");
			expect(callArgs[1].body).toBe(JSON.stringify({ name: "New Item", description: "Test item" }));
		});
		...
		it("handles error flow with proper error message extraction", async () => {
			...
			await expect(api.post("http://example.com/api/users", {})).rejects.toThrow(
				"Validation failed: email must be unique",
			);
		});
