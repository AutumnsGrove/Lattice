...
describe("parseImageFilename", () => {
	...
	describe("Complex scenarios", () => {
		it("should handle full metadata extraction", () => {
			const result = parseImageFilename("minecraft/2024-12-01_build-complete.png");
			expect(result.category).toBe("minecraft");
...
describe("searchImages", () => {
	...
	it("should filter by parsed slug match", () => {
		...
		expect(result).toContainEqual(expect.objectContaining({ parsed_slug: "build" }));
	});
...
describe("filterImagesByDateRange", () => {
	...
	it("should filter by start date (inclusive)", () => {
		...
		result.forEach((img) => {
			...
			if (date) {
				expect(date >= "2024-06-20").toBe(true);
			}
		});
	});
	...
	it("should filter by date range (both start and end)", () => {
		const result = filterImagesByDateRange(testImages, "2024-01-01", "2024-12-31");
		expect(result.length).toBeGreaterThan(0);
		result.forEach((img) => {
			const date = getImageDate(img);
			if (date) {
	...
	it("should handle single-day range", () => {
		const result = filterImagesByDateRange(testImages, "2024-01-15", "2024-01-15");
		result.forEach((img) => {
			const date = getImageDate(img);
			if (date) {
	...
	it("should handle non-existent date range", () => {
		const result = filterImagesByDateRange(testImages, "2050-01-01", "2050-12-31");
		expect(result).toHaveLength(0);
	});

	it("should respect custom_date over parsed_date", () => {
		const images: GalleryImage[] = [{ custom_date: "2025-01-15", parsed_date: "2024-01-01" }];
...
describe("filterImagesByTags", () => {
	...
	it("should require all specified tags (AND logic)", () => {
		...
		expect(result).toContainEqual(expect.objectContaining({ custom_title: "Beautiful Forest" }));
	});
	...
	it("should handle multiple tag filtering", () => {
		...
		expect(result).toContainEqual(expect.objectContaining({ parsed_category: "selfies" }));
	});
...
describe("getAvailableCategories", () => {
	...
	it("should return empty array for images without categories", () => {
		const images: GalleryImage[] = [{ r2_key: "photo.jpg" }, { r2_key: "image.png" }];
		const result = getAvailableCategories(images);
		expect(result).toHaveLength(0);
	});
