 ...
 *
 * When `options.isInternalService` is true, tenant ownership is not checked —
 * the caller is a trusted internal service (e.g., reverie-exec) that has
 * already been authenticated via X-Internal-Service-Key in hooks.server.ts.
 */
export async function getVerifiedTenantId(
	db: D1Database,
	tenantId: string | undefined | null,
	user: User | null | undefined,
	options?: { isInternalService?: boolean },
): Promise<string> {
	if (!tenantId) {
	...
	// Internal services are trusted — tenant ID already validated upstream
	if (options?.isInternalService) return tenantId;

	const isOwner = await verifyTenantOwnership(db, tenantId, user.email);
	if (!isOwner) {
		const err = new Error("Access denied - you do not own this tenant") as SessionError;
		err.status = 403;
		throw err;
	}
