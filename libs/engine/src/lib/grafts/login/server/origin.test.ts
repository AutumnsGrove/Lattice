...
import { getRealOrigin, isProduction, isGrovePlatform, getCookieDomain } from "./origin.js";
...
describe("Origin Helper", () => {
	...
	describe("getRealOrigin", () => {
		describe("without X-Forwarded-Host (direct access)", () => {
			...
			it("returns url.origin for Cloudflare Pages domain", () => {
				...
				expect(getRealOrigin(request, url)).toBe("https://grove-lattice.pages.dev");
			});
		});
		...
		describe("with X-Forwarded-Host (proxied via grove-router)", () => {
			...
			it("returns HTTPS origin for proxied admin subdomain", () => {
				...
				const url = createUrl("https://internal-worker.workers.dev/auth/callback");
			...
			it("returns HTTPS origin for proxied auth subdomain", () => {
				...
				const url = createUrl("https://groveauth-frontend.pages.dev/auth/callback");
		...
		describe("OAuth redirect_uri scenarios", () => {
			...
			it("generates correct redirect_uri for landing page", () => {
				...
				const url = createUrl("https://grove-landing.m7jv4v7npb.workers.dev/auth/callback");
	...
	describe("isProduction", () => {
		...
		it("returns true for Cloudflare Pages domain", () => {
			expect(isProduction(createUrl("https://grove-lattice.pages.dev"))).toBe(true);
		});
	...
	describe("isGrovePlatform", () => {
		...
		it("returns true for subdomain.grove.place", () => {
			expect(isGrovePlatform(createUrl("https://autumn.grove.place"))).toBe(true);
		});

		it("returns true for nested subdomain", () => {
			expect(isGrovePlatform(createUrl("https://api.auth.grove.place"))).toBe(true);
		});
		...
		it("returns false for Cloudflare Pages domain", () => {
			expect(isGrovePlatform(createUrl("https://grove-lattice.pages.dev"))).toBe(false);
		});
		...
		it("returns false for domain that contains grove.place as substring", () => {
			...
			expect(isGrovePlatform(createUrl("https://grove.place.evil.com"))).toBe(false);
		});
	...
	describe("getCookieDomain", () => {
		it("returns .grove.place for production Grove subdomain", () => {
			expect(getCookieDomain(createUrl("https://autumn.grove.place"))).toBe(".grove.place");
		});

		it("returns .grove.place for production Grove root", () => {
			expect(getCookieDomain(createUrl("https://grove.place"))).toBe(".grove.place");
		});

		it("returns .grove.place for admin.grove.place", () => {
			expect(getCookieDomain(createUrl("https://admin.grove.place"))).toBe(".grove.place");
		});

		it("returns undefined for localhost", () => {
			expect(getCookieDomain(createUrl("http://localhost:5173"))).toBeUndefined();
		});

		it("returns undefined for 127.0.0.1", () => {
			expect(getCookieDomain(createUrl("http://127.0.0.1:5173"))).toBeUndefined();
		});

		it("returns undefined for Cloudflare Pages domain", () => {
			// Not on Grove platform, so no cross-subdomain cookies
			expect(getCookieDomain(createUrl("https://grove-lattice.pages.dev"))).toBeUndefined();
		});
	...
	describe("Integration: Cross-subdomain Auth Cookies", () => {
		it("enables cookie sharing between grove.place subdomains", () => {
			...
			expect(domains).toEqual([".grove.place", ".grove.place", ".grove.place", ".grove.place"]);
		});
