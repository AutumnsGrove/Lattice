 ...
 * ```
 */
export function createCallbackHandler(config: CallbackHandlerConfig = {}): RequestHandler {
	const {
		defaultReturnTo = "/arbor",
		rateLimitKvKey = "CACHE_KV",
		errorRedirect = "https://login.grove.place",
	} = config;

	return async ({ url, cookies, platform, request }): Promise<Response | never> => {
		...
		if (errorParam) {
			...
			redirect(302, `${errorRedirect}${sep}${buildErrorParams(authError)}`);
		}
