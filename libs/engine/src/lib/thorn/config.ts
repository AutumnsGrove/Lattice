...
export const THORN_CONFIG: ThornConfig = {
	...
	contentTypes: {
		...
		// ─────────────────────────────────────────────────────────────────────────
		// DM Messages (Chirp): Strict (direct harm to recipient)
		// Similar to comments — messages directly affect another person
		// ─────────────────────────────────────────────────────────────────────────
		dm_message: {
			name: "Direct Message",
			thresholds: [
				...
				{
					// Block: hate, harassment (direct harm to recipient)
					categories: ["hate", "harassment"],
					minConfidence: 0.8,
					action: "block",
				},
...
export function determineAction(
	...
	for (const threshold of config.thresholds) {
		...
		if (matchesCategory && moderationResult.confidence >= threshold.minConfidence) {
