...
const POPULAR_POST_THRESHOLDS: Record<TierKey, number> = {
	wanderer: 150, // Highest bar (no blog anyway)
...
export class PostMetaDO implements DurableObject {
    ...
    `);
		...
		if (stored?.value) {
			const parsed = parseJsonForMeta<PostMeta | null>(stored.value as string, null);
	...
	private async handleRemoveReaction(request: Request): Promise<Response> {
		...
		if (data.type === "like") {
			...
			this.meta.reactions.bookmarks = Math.max(0, this.meta.reactions.bookmarks - 1);
		}
	...
	private async trimViewLogIfNeeded(): Promise<void> {
		const countResult = this.state.storage.sql.exec("SELECT COUNT(*) as count FROM view_log").one();
	...
	async alarm(): Promise<void> {
		...
		await this.state.storage.sql.exec("DELETE FROM view_log WHERE timestamp < ?", cutoff);
