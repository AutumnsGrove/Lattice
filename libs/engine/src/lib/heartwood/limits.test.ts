...
describe("getQuotaWidgetData", () => {
  ...
  it("should set showUpgrade for warning/critical/blocked", () => {
    expect(
      getQuotaWidgetData(createStatus({ percentage_used: 50 })).showUpgrade,
    ).toBe(false);
    expect(
      getQuotaWidgetData(createStatus({ percentage_used: 92 })).showUpgrade,
    ).toBe(true);
    expect(getQuotaWidgetData(createAtLimitStatus()).showUpgrade).toBe(true);
    expect(getQuotaWidgetData(createUpgradeRequiredStatus()).showUpgrade).toBe(
      true,
    );
  });
  ...
  it("should reflect canPost status", () => {
    expect(
      getQuotaWidgetData(createStatus({ can_create_post: true })).canPost,
    ).toBe(true);
    expect(
      getQuotaWidgetData(createStatus({ can_create_post: false })).canPost,
    ).toBe(false);
  });
...
describe("getPreSubmitCheck", () => {
  ...
  it("should allow posting for healthy status", () => {
    const response = createCanPostResponse(
      true,
      createStatus({ percentage_used: 50 }),
    );
