<script lang="ts">
	...
	const currentTierName = $derived(
		TIERS[safeStatus.tier as PaidTierKey]?.display.name || "Unknown",
	);

	// Get next tier's post limit for display
	const nextTierPostLimit = $derived(nextTier ? formatLimit(TIERS[nextTier].limits.posts) : null);
			...
			<div id="upgrade-description" class="space-y-4 mb-6">
				{#if safeStatus.upgrade_required}
					<p class="text-sm text-foreground-subtle text-center">
						Your grace period has expired. To continue creating posts, please upgrade your plan or
						delete some existing posts.
					</p>
					...
					{#if oldestPostTitle}
						<div class="bg-cream-50 dark:bg-bark-700/50 rounded-lg p-3 text-sm">
							<p class="text-foreground-muted">
								If you continue, new posts may need to replace older ones. Your oldest post:
							</p>
				...
				{:else}
					<p class="text-sm text-foreground-subtle text-center">
						You've reached your post limit on the <strong>{currentTierName}</strong> plan. Upgrade to
						get more posts.
					</p>
				{/if}
