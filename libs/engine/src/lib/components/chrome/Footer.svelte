<script lang="ts">
	...
	import {
		...
		DEFAULT_CONNECT_LINKS,
		DEFAULT_LEGAL_LINKS,
		DIVIDER_VERTICAL,
	} from "$lib/ui/components/chrome/defaults";
	...
	let { resourceLinks, connectLinks, legalLinks, season, maxWidth = "default" }: Props = $props();
...
<footer class="py-12 border-t border-default">
	<div class="{maxWidthClass[maxWidth]} mx-auto px-6">
		...
		<div
			class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-8 lg:gap-6 sm:mb-10"
		>
			...
			<div class="text-center sm:text-left lg:flex-1">
				...
				<p class="text-sm font-sans text-foreground-subtle italic mb-4">A place to Be</p>
			...
			{#if resources.length > 0}
				...
				<div class="hidden sm:block text-center sm:text-left lg:flex-1">
					<h3 class="text-sm font-sans font-medium text-foreground uppercase tracking-wide mb-4">
						Resources
					</h3>
		...
		<div class="pt-6 border-t border-default">
			<div class="flex flex-col sm:flex-row items-center justify-between gap-4">
				...
				<div
					class="flex flex-wrap items-center justify-center sm:justify-start gap-x-4 gap-y-2 text-xs font-sans text-foreground-subtle"
				>
					<span>&copy; {new Date().getFullYear()} Autumn Brown</span>
					<span class="text-divider">·</span>
					<span>Made with care</span>
					<span class="text-divider hidden sm:inline">·</span>
					<a href="https://grove.place/credits" class="hover:text-accent-muted transition-colors"
						>Credits</a
					>
					<span class="text-divider">·</span>
					{#each legal as link, index}
						<a href={link.href} class="hover:text-accent-muted transition-colors"
							>{labelFor(link)}</a
						>
				...
				<div class="flex items-center gap-4">
					...
					<button
						...
						aria-label={groveModeStore.current
							? "Disable Grove Mode (show standard terms)"
							: "Enable Grove Mode (show nature-themed terms)"}
