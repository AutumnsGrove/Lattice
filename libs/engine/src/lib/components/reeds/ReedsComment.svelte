<script lang="ts">
	...
	let { comment, currentUserId, isOwner = false, onreply, onedit, ondelete }: Props = $props();
	...
	let canDelete = $derived(comment.author_id === currentUserId || isOwner);
...
>
	...
	{#if !isDeleted}
		<footer class="comment-actions">
			...
			{/if}
			{#if canEdit && onedit}
				<button class="action-btn" onclick={() => onedit?.(comment.id)}> Edit </button>
			{/if}
