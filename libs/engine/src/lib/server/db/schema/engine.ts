...
export const tenants = sqliteTable('tenants', {
	...
	// Subscription & limits
	plan: text('plan', { enum: ['free', 'seedling', 'sapling', 'oak', 'evergreen'] }).default('seedling'),
...
export const posts = sqliteTable('posts', {
	...
	uniqueIndex('idx_posts_tenant_slug_unique').on(table.tenantId, table.slug),
]);
...
export const pages = sqliteTable('pages', {
	...
	uniqueIndex('idx_pages_tenant_slug_unique').on(table.tenantId, table.slug),
]);
...
export const siteSettings = sqliteTable('site_settings', {
	...
	uniqueIndex('idx_site_settings_tenant_key').on(table.tenantId, table.settingKey),
]);
...
export const tenantSettings = sqliteTable('tenant_settings', {
	...
	uniqueIndex('idx_tenant_settings_tenant_key').on(table.tenantId, table.settingKey),
]);
...
export const emailVerifications = sqliteTable('email_verifications', {
	id: text('id').primaryKey(),
	userId: text('user_id').notNull().references(() => userOnboarding.id, { onDelete: 'cascade' }),
	email: text('email').notNull(),
	code: text('code').notNull(),
	createdAt: integer('created_at').default(sql`(unixepoch())`),
	expiresAt: integer('expires_at').notNull(),
...
export const platformBilling = sqliteTable('platform_billing', {
	...
	plan: text('plan', { enum: ['free', 'seedling', 'sapling', 'oak', 'evergreen'] }).notNull().default('seedling'),
	status: text('status', { enum: ['trialing', 'active', 'past_due', 'paused', 'canceled', 'unpaid'] }).notNull().default('active'),
...
export const webhookEvents = sqliteTable('webhook_events', {
	...
	uniqueIndex('idx_webhooks_provider_event').on(table.provider, table.providerEventId),
]);
...
export const flagRules = sqliteTable('flag_rules', {
	id: integer('id').primaryKey({ autoIncrement: true }),
	...
	uniqueIndex('idx_flag_rules_flag_priority').on(table.flagId, table.priority),
]);
...
export const meadowPosts = sqliteTable('meadow_posts', {
	id: text('id').primaryKey(),
	tenantId: text('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
	...
	uniqueIndex('idx_meadow_posts_tenant_guid').on(table.tenantId, table.guid),
]);

export const meadowVotes = sqliteTable('meadow_votes', {
	id: text('id').primaryKey(),
	userId: text('user_id').notNull(),
	postId: text('post_id').notNull().references(() => meadowPosts.id, { onDelete: 'cascade' }),
	createdAt: integer('created_at').notNull(),
}, (table) => [
	uniqueIndex('idx_meadow_votes_user_post').on(table.userId, table.postId),
]);

export const meadowReactions = sqliteTable('meadow_reactions', {
	id: text('id').primaryKey(),
	userId: text('user_id').notNull(),
	postId: text('post_id').notNull().references(() => meadowPosts.id, { onDelete: 'cascade' }),
...
export const meadowBookmarks = sqliteTable('meadow_bookmarks', {
	id: text('id').primaryKey(),
	userId: text('user_id').notNull(),
	postId: text('post_id').notNull().references(() => meadowPosts.id, { onDelete: 'cascade' }),
	createdAt: integer('created_at').notNull(),
}, (table) => [
	uniqueIndex('idx_meadow_bookmarks_user_post').on(table.userId, table.postId),
]);

export const meadowFollows = sqliteTable('meadow_follows', {
	id: text('id').primaryKey(),
	followerId: text('follower_id').notNull(),
	followedTenantId: text('followed_tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
	createdAt: integer('created_at').notNull(),
}, (table) => [
...
export const meadowReports = sqliteTable('meadow_reports', {
	...
	status: text('status', { enum: ['pending', 'reviewed', 'actioned', 'dismissed'] }).notNull().default('pending'),
	createdAt: integer('created_at').notNull(),
...
export const blockedCommenters = sqliteTable('blocked_commenters', {
	tenantId: text('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
	blockedUserId: text('blocked_user_id').notNull(),
	reason: text('reason'),
	createdAt: text('created_at').notNull().default(sql`(datetime('now'))`),
}, (table) => [
	uniqueIndex('idx_blocked_commenters_pk').on(table.tenantId, table.blockedUserId),
]);
...
export const petalAccountFlags = sqliteTable('petal_account_flags', {
	id: text('id').primaryKey(),
	userId: text('user_id').notNull(),
	flagType: text('flag_type').notNull(),
	...
	uniqueIndex('idx_petal_flags_user_type').on(table.userId, table.flagType),
]);
...
export const sentinelRuns = sqliteTable('sentinel_runs', {
	...
	name: text('name').notNull(),
	description: text('description'),
	profileType: text('profile_type', { enum: ['spike', 'sustained', 'oscillation', 'ramp', 'custom'] }).notNull(),
...
export const sentinelMetrics = sqliteTable('sentinel_metrics', {
	id: text('id').primaryKey(),
	runId: text('run_id').notNull().references(() => sentinelRuns.id, { onDelete: 'cascade' }),
	tenantId: text('tenant_id').notNull(),
...
export const sentinelSchedules = sqliteTable('sentinel_schedules', {
	...
	maintenanceMessage: text('maintenance_message').default('Scheduled infrastructure validation in progress'),
	isActive: integer('is_active').default(1),
...
export const postViews = sqliteTable('post_views', {
	...
	viewedAt: integer('viewed_at').notNull().default(sql`(unixepoch() * 1000)`),
});
...
export const groveMessages = sqliteTable('grove_messages', {
	...
	messageType: text('message_type', { enum: ['info', 'warning', 'celebration', 'update'] }).notNull().default('info'),
...
export const tenantSecrets = sqliteTable('tenant_secrets', {
	...
	uniqueIndex('idx_tenant_secrets_tenant_key').on(table.tenantId, table.keyName),
]);
...
export const communityThemes = sqliteTable('community_themes', {
	id: text('id').primaryKey(),
	creatorTenantId: text('creator_tenant_id').notNull().references(() => tenants.id),
	name: text('name').notNull(),
	description: text('description'),
	tags: text('tags'),
	...
	status: text('status', { enum: ['draft', 'pending', 'in_review', 'approved', 'featured', 'changes_requested', 'rejected', 'removed'] }).default('pending'),
	reviewedAt: integer('reviewed_at'),
	createdAt: integer('created_at').default(sql`(unixepoch())`),
	updatedAt: integer('updated_at').default(sql`(unixepoch())`),
});
