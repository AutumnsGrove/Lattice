...
describe("Turnstile Constants", () => {
	...
	it("TURNSTILE_COOKIE_MAX_AGE is 30 days in seconds", () => {
		const thirtyDaysInSeconds = 60 * 60 * 24 * 30;
		expect(TURNSTILE_COOKIE_MAX_AGE).toBe(thirtyDaysInSeconds);
		expect(TURNSTILE_COOKIE_MAX_AGE).toBe(2592000);
	});
});
...
describe("getVerificationCookieOptions", () => {
	describe("Default Options", () => {
		...
		it("uses TURNSTILE_COOKIE_MAX_AGE for maxAge", () => {
			...
			expect(options.maxAge).toBe(2592000); // 30 days in seconds
		});
	});
...
describe("Turnstile Cookie Signing Security Tests", () => {
	...
	describe("Cookie Expiration", () => {
		...
		it("uses default max age from constant", async () => {
			// TURNSTILE_COOKIE_MAX_AGE is 30 days in seconds
			expect(TURNSTILE_COOKIE_MAX_AGE).toBe(60 * 60 * 24 * 30);
		});
