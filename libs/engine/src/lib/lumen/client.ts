...
import { executeWithFallback, executeEmbedding, executeModeration } from "./router.js";
import type {
	GutterItem,
	LumenClientConfig,
...
export class LumenClient {
	...
	async run(request: LumenRequest, tier?: TierKey): Promise<LumenResponse> {
		...
		if (request.tenant && tier && this.quotaTracker && !request.options?.skipQuota) {
		...
		if (request.options?.songbird) {
			const songbirdOpts =
				typeof request.options.songbird === "object" ? request.options.songbird : undefined;
			...
			if (songbirdResult.metrics.canaryMs || songbirdResult.metrics.kestrelMs) {
				console.log(
					`[Lumen/Songbird] task=${request.task} passed=${songbirdResult.passed}` +
						...
						(songbirdResult.confidence != null ? ` confidence=${songbirdResult.confidence}` : ""),
				);
			}

			if (!songbirdResult.passed) {
				throw new LumenError("Content failed security validation", "SONGBIRD_REJECTED", {
					task: request.task,
				});
			}
		}
	 ...
	 */
	async *stream(request: LumenRequest, tier?: TierKey): AsyncGenerator<LumenStreamChunk> {
		// Check if Lumen is enabled
		if (!this.enabled) {
			throw new LumenError("Lumen is disabled", "DISABLED", {
				task: request.task,
		...
		if (!provider?.stream) {
			throw new LumenError("Streaming not available for this task", "INVALID_TASK", {
				task: request.task,
			});
		}
	 ...
	 */
	async embed(request: LumenEmbeddingRequest, tier?: TierKey): Promise<LumenEmbeddingResponse> {
		// Check if Lumen is enabled
		if (!this.enabled) {
			throw new LumenError("Lumen is disabled", "DISABLED", {
				task: "embedding",
		...
		// Execute
		const result = await executeEmbedding(request.input, this.providers, request.model);
	...
	async transcribe(
		...
		if (request.tenant && tier && this.quotaTracker && !request.options?.skipQuota) {
			await this.quotaTracker.enforceQuota(request.tenant, tier, "transcription");

			// Draft mode also requires generation quota
			if (mode === "draft") {
				await this.quotaTracker.enforceQuota(request.tenant, tier, "generation");
			}
		}
		...
		let result: { text: string; wordCount: number; duration: number } | undefined = undefined;
		let usedModel = taskConfig.primaryModel;

		try {
			result = await provider.transcribe(taskConfig.primaryModel, request.audio);
		} catch (primaryError) {
			// Try fallback chain
		...
		const draftResult = await this.structureDraftTranscript(rawText, request.tenant, tier);
	...
	private async structureDraftTranscript(
		...
		try {
			...
			console.warn("[Lumen/Scribe] Failed to parse draft response, using raw text");
	...
	async getQuotaStatus(tenantId: string, tier: TierKey) {
		if (!this.quotaTracker) {
			throw new LumenError("Quota tracking not available (no D1 database)", "INVALID_INPUT");
		}
...
export async function createLumenClientWithDecryption(
	...
	if (isEncryptedToken(config.encryptedApiKey)) {
		const decrypted = await safeDecryptToken(config.encryptedApiKey, config.encryptionKey);
		if (!decrypted) {
