...
export class QuotaTracker {
	 ...
	 */
	async checkQuota(tenantId: string, tier: TierKey, task: LumenTask): Promise<QuotaCheckResult> {
	 ...
	 */
	async enforceQuota(tenantId: string, tier: TierKey, task: LumenTask): Promise<void> {
		const { allowed, currentUsage, limit } = await this.checkQuota(tenantId, tier, task);
	...
	async getAggregateStats(
		...
		const results = await this.db
         ...
         GROUP BY task`,
			...
			.bind(tenantId, this.toSQLiteTimestamp(startDate), this.toSQLiteTimestamp(endDate))
			.all<{
				task: LumenTask;
				requests: number;
				input_tokens: number;
		...
		let totalCost = 0;
		let totalLatency = 0;
		const byTask: Record<LumenTask, { requests: number; cost: number }> = {} as Record<
			LumenTask,
