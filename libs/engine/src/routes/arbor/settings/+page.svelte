<script>
	...
	import { CANOPY_CATEGORIES, CANOPY_CATEGORY_LABELS } from "$lib/config/canopy-categories";
	...
	async function saveGroveTitle() {
		...
		try {
			...
			titleMessage = "Error: " + (error instanceof Error ? error.message : String(error));
		}
		...
		input.onchange = async () => {
			...
			try {
				...
				if (result?.url) {
					...
					toast.error("Upload completed but no photo URL was returned. Please try again.");
				}
			} catch (error) {
	...
	async function fetchCanopySettings() {
		...
		try {
			...
			canopyShowForests =
				settings.canopy_show_forests !== "false" && settings.canopy_show_forests !== false;
		...
		{:else}
			<div class="font-selector">
				<label class="font-option" class:selected={currentFont === "lexend"}>
					<input type="radio" name="font" value="lexend" bind:group={currentFont} />
					<div class="font-info">
				...
				<label class="font-option" class:selected={currentFont === "atkinson"}>
					<input type="radio" name="font" value="atkinson" bind:group={currentFont} />
					<div class="font-info">
						<span class="font-name" style="font-family: 'Atkinson Hyperlegible', sans-serif;"
				...
				<label class="font-option" class:selected={currentFont === "opendyslexic"}>
					<input type="radio" name="font" value="opendyslexic" bind:group={currentFont} />
					<div class="font-info">
						<span class="font-name" style="font-family: 'OpenDyslexic', sans-serif;"
							>OpenDyslexic</span
						>
				...
				<label class="font-option" class:selected={currentFont === "quicksand"}>
					<input type="radio" name="font" value="quicksand" bind:group={currentFont} />
					<div class="font-info">
				...
				<label class="font-option" class:selected={currentFont === "plus-jakarta-sans"}>
					<input type="radio" name="font" value="plus-jakarta-sans" bind:group={currentFont} />
					<div class="font-info">
						<span class="font-name" style="font-family: 'Plus Jakarta Sans', sans-serif;"
				...
				<label class="font-option" class:selected={currentFont === "ibm-plex-mono"}>
					<input type="radio" name="font" value="ibm-plex-mono" bind:group={currentFont} />
					<div class="font-info">
						<span class="font-name" style="font-family: 'IBM Plex Mono', monospace;"
							>IBM Plex Mono</span
						>
				...
				<label class="font-option" class:selected={currentFont === "cozette"}>
					<input type="radio" name="font" value="cozette" bind:group={currentFont} />
					<div class="font-info">
				...
				<label class="font-option" class:selected={currentFont === "alagard"}>
					<input type="radio" name="font" value="alagard" bind:group={currentFont} />
					<div class="font-info">
				...
				<label class="font-option" class:selected={currentFont === "calistoga"}>
					<input type="radio" name="font" value="calistoga" bind:group={currentFont} />
					<div class="font-info">
				...
				<label class="font-option" class:selected={currentFont === "caveat"}>
					<input type="radio" name="font" value="caveat" bind:group={currentFont} />
					<div class="font-info">
	...
	>
		...
		<div class="color-picker-section">
			<div class="color-preview-wrapper">
				<input type="color" id="accent-color" bind:value={currentAccentColor} class="color-input" />
	...
	<GlassCard variant="frosted" class="mb-6">
		...
		<p class="section-description">
			...
			<a
				...
				class="canopy-link">View the Canopy &rarr;</a
			>
		</p>
		...
		{:else}
			...
			<label class="logo-toggle canopy-toggle">
				...
				<span class="toggle-label">
					...
					<span class="toggle-description">
						When enabled, your grove appears in the Canopy — Grove's public directory where
			...
			{#if canopyVisible}
				...
				<div class="canopy-field">
					...
					<p class="field-help">
						Select categories that describe your grove. This helps others find you.
					</p>
				</div>
	...
	<GlassCard variant="frosted" class="mb-6">
		...
		<label class="logo-toggle">
			...
			<span class="toggle-label">
				...
				<span class="toggle-description">
					When enabled, your published posts appear in Meadow — Grove's community feed where
					wanderers discover each other's writing. You can exclude individual posts from the bloom
					editor.
				</span>
			</span>
		</label>
