<script lang="ts">
	...
	let { availableTiers, changingPlan, selectedPlan, hasSubscription, onChangePlan }: Props =
		$props();

	// Filter to only show available tiers
	const displayTiers = $derived(availableTiers.filter((t) => t.status === "available"));
...
{#if shouldRender}
	<GlassCard variant="default" class="mb-6">
		...
		<p class="section-description">
			Upgrade for more features or downgrade if you need less. Changes are pro-rated based on your
			billing cycle.
		</p>
