...
export const load: PageServerLoad = async ({ platform, locals }) => {
	...
	if (counter) {
		parsedConfig = {
			...
			sinceDateStyle: (counter.since_date_style || "footnote") as HitCounterSinceDateStyle,
		};
	}
...
export const actions: Actions = {
	save: async ({ request, platform, locals }) => {
		...
		const finalCountMode = VALID_COUNT_MODES.includes(countMode as HitCounterCountMode)
			? countMode
