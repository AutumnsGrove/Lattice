...
export const actions: Actions = {
	...
	deleteGarden: async ({ request, platform, locals }) => {
		...
		try {
			await db
				.prepare(`DELETE FROM link_garden_items WHERE garden_id = ? AND tenant_id = ?`)
	...
	addLink: async ({ request, platform, locals }) => {
		...
		const category = sanitizeText(formData.get("category") as string | null, MAX_CATEGORY_LENGTH);
		...
		try {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
				...
				.bind(id, gardenId, tenantId, url, title, description, faviconUrl, category, sortOrder)
				.run();
