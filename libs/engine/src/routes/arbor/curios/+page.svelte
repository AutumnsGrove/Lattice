<script lang="ts">
	...
	const curios = [
		...
		},
		{
			...
			features: ["4 display styles", "Auto-favicons", "Category grouping", "88x31 button wall"],
		},
		{
			id: "nowplaying",
			name: "Now Playing",
		...
		{#each curios as curio}
			<GlassCard class="curio-card {curio.status === 'coming-soon' ? 'coming-soon' : ''}">
				...
				<div class="curio-actions">
					{#if curio.href}
						<GlassButton href={curio.href} variant="accent">Configure</GlassButton>
					{:else}
						<GlassButton disabled variant="ghost">Coming Soon</GlassButton>
					{/if}
				</div>
			</GlassCard>
		{/each}
