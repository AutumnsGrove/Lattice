...
export const PUT: RequestHandler = async ({ params, request, platform, locals }) => {
	...
	try {
		...
		// Set published_at on first publish (draft→published) — always use the actual
		// publish moment, never a date picker value (which resolves to midnight UTC).
		...
		// Re-publish: bump published_at for already-published posts (updates RSS feed position)
		if (data.republish && currentPost.status === "published") {
			published_at = unixNow;
		}
