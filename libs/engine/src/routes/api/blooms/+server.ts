 ...
 */
async function getNextUntitledTitle(db: D1Database, tenantId: string): Promise<string> {
	const result = await db
		.prepare("SELECT title FROM posts WHERE tenant_id = ? AND title LIKE 'Untitled%'")
		.bind(tenantId)
		.all<{ title: string }>();
