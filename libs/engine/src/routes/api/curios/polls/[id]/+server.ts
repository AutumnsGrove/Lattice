...
export const GET: RequestHandler = async ({ params, request, platform, locals }) => {
	...
	const votesResult = await db
		.prepare(`SELECT selected_options FROM poll_votes WHERE poll_id = ? AND tenant_id = ?`)
		.bind(params.id, tenantId)
		.all<VoteRow>();
	...
	try {
		...
		const existing = await db
			.prepare(`SELECT id FROM poll_votes WHERE poll_id = ? AND voter_hash = ? AND tenant_id = ?`)
	...
	else if (poll.results_visibility === "after-vote" && hasVoted) showResults = true;
	else if (poll.results_visibility === "after-close" && isClosed) showResults = true;
...
export const POST: RequestHandler = async ({ params, request, platform, locals }) => {
	...
	const poll = await db
		.prepare(`SELECT id, poll_type, options, close_date FROM polls WHERE id = ? AND tenant_id = ?`)
		.bind(params.id, tenantId)
		.first<{
			id: string;
			poll_type: string;
	...
	const filtered = selectedOptions.filter((id: string) => validOptionIds.has(id));
