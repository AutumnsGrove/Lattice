...
export const POST: RequestHandler = async ({ params, request, platform, locals }) => {
	...
	const description = sanitizeText(body.description as string | null, MAX_LINK_DESCRIPTION_LENGTH);
	const category = sanitizeText(body.category as string | null, MAX_CATEGORY_LENGTH);
	const faviconUrl = buildFaviconUrl(url);

	// Enforce per-garden link limit
	const countResult = await db
		.prepare(`SELECT COUNT(*) as count FROM link_garden_items WHERE garden_id = ?`)
		.bind(params.id)
		.first<{ count: number }>();
	...
	const maxOrder = await db
		.prepare(`SELECT MAX(sort_order) as max_order FROM link_garden_items WHERE garden_id = ?`)
		.bind(params.id)
	...
	try {
		await db
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			...
			.bind(id, params.id, tenantId, url, title, description, faviconUrl, category, sortOrder)
			.run();
	} catch (err) {
	...
	return json({ success: true, link: { id, url, title, description, category } }, { status: 201 });
};
...
export const DELETE: RequestHandler = async ({ params, request, platform, locals }) => {
	...
	const existing = await db
		.prepare(`SELECT id FROM link_garden_items WHERE id = ? AND garden_id = ? AND tenant_id = ?`)
	...
	try {
		await db
			.prepare(`DELETE FROM link_garden_items WHERE id = ? AND garden_id = ? AND tenant_id = ?`)
			.bind(linkId, params.id, tenantId)
			.run();
	} catch (err) {
		logGroveError("API", API_ERRORS.OPERATION_FAILED, {
