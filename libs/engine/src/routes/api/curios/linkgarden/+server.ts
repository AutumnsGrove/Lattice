...
export const POST: RequestHandler = async ({ request, platform, locals }) => {
	...
	const description = sanitizeText(body.description as string | null, MAX_DESCRIPTION_LENGTH);
	const style = isValidGardenStyle(String(body.style ?? "list")) ? String(body.style) : "list";
	...
	return json({ success: true, garden: { id, title, description, style } }, { status: 201 });
};
