...
import { isValidScale, isValidRotation, isValidPosition, isValidZIndex } from "$lib/curios/clipart";
...
export const PATCH: RequestHandler = async ({ params, request, platform, locals }) => {
	...
	try {
		await db
			.prepare(`UPDATE clipart_placements SET ${updates.join(", ")} WHERE id = ? AND tenant_id = ?`)
			.bind(...values)
			.run();
