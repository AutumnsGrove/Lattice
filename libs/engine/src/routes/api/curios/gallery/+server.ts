...
export const GET: RequestHandler = async ({ url, platform, locals }) => {
	...
	const config = await db
		.prepare(`SELECT enabled, cdn_base_url FROM gallery_curio_config WHERE tenant_id = ?`)
		.bind(tenantId)
		.first<ConfigRow>();
	...
	if (search) {
		whereClause += " AND (custom_title LIKE ? OR parsed_slug LIKE ? OR custom_description LIKE ?)";
