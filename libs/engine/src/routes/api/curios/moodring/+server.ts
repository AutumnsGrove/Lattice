...
export const GET: RequestHandler = async ({ platform, locals }) => {
	...
	if (!row) {
		const timeColor = getInterpolatedTimeColor();
		return json({
			config: {
				mode: "time",
				manualMood: null,
	...
	switch (row.mode) {
		...
		case "random": {
			const r = getRandomColor(tenantId);
		...
		default: {
			const t = getInterpolatedTimeColor();
	...
	return json(
		{
			config: {
				...
				showMoodLog: !!row.show_mood_log,
				currentColor,
				currentMoodName,
				...(nextColor !== undefined && { nextColor, driftT }),
			},
		},
		{
			headers: {
...
export const POST: RequestHandler = async ({ request, platform, locals }) => {
	...
	const mode = isValidMode(body.mode as string) ? (body.mode as string) : "time";
