...
export const GET: RequestHandler = async ({ url, platform, locals }) => {
	...
	const config = await db
		.prepare(`SELECT enabled, show_milestones FROM journey_curio_config WHERE tenant_id = ?`)
		.bind(tenantId)
		.first<{ enabled: number; show_milestones: number }>();
	...
	const countResult = await db
		.prepare(`SELECT COUNT(*) as count FROM journey_summaries WHERE tenant_id = ?`)
		.bind(tenantId)
		.first<{ count: number }>();
