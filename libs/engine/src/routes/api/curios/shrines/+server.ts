...
export const POST: RequestHandler = async ({ request, platform, locals }) => {
	...
	try {
		const maxSort = await db
			.prepare(`SELECT COALESCE(MAX(sort_order), -1) as max_sort FROM shrines WHERE tenant_id = ?`)
			.bind(tenantId)
		...
		await db
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?)`,
			...
			.bind(id, tenantId, title, shrineType, description, size, frameStyle, contentsJson, sortOrder)
			.run();
