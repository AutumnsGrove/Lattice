...
import { isCompedAccount, logBillingAudit, type AuditLogEntry } from "$lib/server/billing.js";
...
async function sendCancellationEmail(params: CancellationEmailParams): Promise<void> {
...
export const POST: RequestHandler = async ({ request, url, platform, locals }) => {
	...
	try {
		...
		const { result: rateLimitResult, response: rateLimitResponse } = await checkBillingRateLimit(
			platform.env,
			tenantId,
		);
		...
		const session = await (
			stripeClient as {
				request: (path: string, opts: object) => Promise<{ url: string; id: string }>;
			}
		...
		if (err instanceof Error) {
			...
			if (err.message.includes("Invalid price")) {
				errorMessage = "Selected plan is not available. Please try another plan.";
			} else if (err.message.includes("Customer already exists")) {
				errorMessage = "This account already has an active subscription. Please contact support.";
       ...
       WHERE pb.tenant_id = ?`,
		...
		if (err instanceof Error) {
			...
			if (
				...
				errorMessage = "Subscription not found. Please try refreshing your billing page.";
				...
				errorMessage = "This subscription cannot be resumed. Please contact support.";
			}
		} else {
			errorDetails.rawError = String(err);
		}
