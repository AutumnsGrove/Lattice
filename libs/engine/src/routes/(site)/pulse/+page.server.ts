...
export const load: PageServerLoad = async ({ platform, locals }) => {
	...
	const tenantId =
		locals.tenantId ?? (isLanding ? platform?.env?.PULSE_LANDING_TENANT_ID : undefined);

	if (!db) throwGroveError(503, SITE_ERRORS.DB_NOT_CONFIGURED, "Site");
	if (!tenantId) throwGroveError(400, SITE_ERRORS.TENANT_CONTEXT_REQUIRED, "Site");
	...
	const [activeData, todayData, streakData, eventsResult, dailyResult, hourlyResult] =
		await Promise.all([
