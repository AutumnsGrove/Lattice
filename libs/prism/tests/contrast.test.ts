...
import type { ThemeColors } from "../src/lib/types.js";

function makeThemeColors(overrides: Partial<ThemeColors> = {}): { colors: ThemeColors } {
	return {
		colors: {
			background: "#ffffff",
			surface: "#f5f5f5",
...
describe("Contrast Utilities", () => {
	...
	describe("validateThemeContrast", () => {
		it("should pass for theme with good contrast", () => {
			const result = validateThemeContrast(makeThemeColors());
			expect(result.valid).toBe(true);
			expect(result.error).toBeUndefined();
		});

		it("should fail for theme with poor foreground contrast", () => {
			const result = validateThemeContrast(makeThemeColors({ foreground: "#cccccc" }));
		...
		it("should provide specific error message about which color pair failed", () => {
			const result = validateThemeContrast(
				makeThemeColors({ foreground: "#d0d0d0", foregroundMuted: "#e8e8e8" }),
			);
			expect(result.valid).toBe(false);
			expect(result.error).toBeDefined();
			expect(
