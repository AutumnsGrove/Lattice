<script lang="ts">
	...
	// Check if WebAuthn is supported for passkey sign-in
	...
	// Discord icon component (Discord Blurple branding)
	const DiscordIcon = `<svg viewBox="0 0 24 24" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
		...
		<GlassCard variant="frosted" class="max-w-lg mx-auto">
			...
			{:else}
				...
				<div class="space-y-3">
					...
					<!-- Discord -->
					<a href="/auth?provider=discord" class="btn-auth hover:opacity-90" style="background-color: #5865F2; border-color: #5865F2; color: white;">
						{@html DiscordIcon}
						<span>Continue with Discord</span>
					</a>

					<!-- Passkey (only shown if WebAuthn is supported) -->
					{#if webAuthnSupported}
						<a href="/auth?provider=passkey" class="btn-auth">
							<Fingerprint size={20} />
