...
Curios are the interactive, decorative, and personal elements that make a Grove site feel alive and uniquely yours. They're not themes (that's Foliage), not the editor (that's Terrarium) — they're the tchotchkes, the personality, the things that make someone go "oh, this site is _different_."
...
All curios live in `libs/engine/src/lib/curios/{name}/`. Engine-first pattern. When we hit 15+ built curios, revisit extraction to a dedicated package.
...
_Each plan is its own adventure. Pick one up, build it, ship it, move to the next._
