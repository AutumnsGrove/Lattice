...
**Target Repository:** `AutumnsGrove/GroveEngine`  
**Existing File:** `packages/engine/src/lib/server/encryption.ts`  
...
```typescript
// Add to packages/engine/src/lib/server/encryption.ts
  ...
  try {
    ...
    if (parts.length === 3 && parts[0] === "v1") {
      ...
      return { 
        ...
        stage: "parse",
        details: { partsCount: parts.length, firstPart: parts[0]?.slice(0, 20) }
      };
    }
    ...
    try {
      ...
      return { 
        success: true, 
        value,
        details: { format, ivLength: iv.byteLength, ctLength: ciphertext.byteLength }
      };
    } catch (e) {
...
```toml
# In packages/engine/wrangler.toml
...
Create `packages/engine/src/lib/server/secrets-manager.ts`:
...
export class SecretsManager {
  ...
  async debugTenantDEK(tenantId: string): Promise<{
    ...
    if (!result.encrypted_dek) {
      return { exists: false, canDecrypt: false, error: 'No DEK stored (will be created on first use)' };
    }
...
Create `packages/engine/src/lib/server/secrets.ts`:
