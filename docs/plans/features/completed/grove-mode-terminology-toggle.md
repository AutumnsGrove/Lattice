...
### File: `libs/engine/src/lib/ui/stores/grove-mode.svelte.ts`
...
Add to `libs/engine/src/lib/ui/stores/index.ts` barrel export.
...
3. The `+page.svelte` wrapper renders a banner _above_ the `{@html doc.html}` — not inside the markdown
...
- `libs/engine/src/lib/ui/components/ui/groveterm/types.ts` — add `standardTerm`, `alwaysGrove`
- `libs/engine/src/lib/data/grove-term-manifest.json` — add standard terms to entries
- `scripts/generate/grove-term-manifest.ts` — update generator
- `libs/engine/src/lib/ui/stores/grove-mode.svelte.ts` — new store
- `libs/engine/src/lib/ui/stores/index.ts` — export new store
...
- `libs/engine/src/lib/ui/components/ui/groveterm/GroveTerm.svelte` — mode-aware rendering
- `libs/engine/src/lib/ui/components/ui/groveterm/GroveTermPopup.svelte` — updated popup content
- `libs/engine/src/lib/ui/components/chrome/defaults.ts` — slug-based nav
- `libs/engine/src/lib/ui/components/chrome/tenant-nav.ts` — slug-based nav
...
- `libs/engine/src/lib/ui/components/ui/groveterm/GroveIntro.svelte` — new component
- `libs/engine/src/lib/ui/components/ui/groveterm/index.ts` — export GroveIntro
- Individual page components for Grove-named features
- `grove-term-manifest.json` — add `hasIntroPage` flags
- `apps/landing/src/routes/knowledge/[category]/[slug]/+page.server.ts` — manifest lookup for article terms
- `apps/landing/src/routes/knowledge/[category]/[slug]/+page.svelte` — render banner above article HTML
...
| File                                                                  | Purpose                                                                   |
| --------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `libs/engine/src/lib/data/grove-term-manifest.json`                   | Term definitions (80 terms)                                               |
| `libs/engine/src/lib/ui/components/ui/groveterm/`                     | GroveTerm component family                                                |
| `libs/engine/src/lib/ui/stores/season.svelte.ts`                      | Pattern reference for store                                               |
| `libs/engine/src/lib/ui/stores/theme.svelte.ts`                       | Pattern reference for store                                               |
| `libs/engine/src/lib/ui/components/chrome/defaults.ts`                | Navigation config                                                         |
| `libs/engine/src/lib/ui/components/chrome/tenant-nav.ts`              | Tenant navigation config                                                  |
| `libs/engine/src/hooks.server.ts`                                     | Existing URL redirects                                                    |
...
| `libs/engine/src/lib/ui/components/ui/groveterm/GroveIntro.svelte`    | Standardized page introduction component (new)                            |
| `apps/landing/src/routes/knowledge/[category]/[slug]/+page.server.ts` | Article page loader (enhance for term lookup)                             |
| `apps/landing/src/routes/knowledge/[category]/[slug]/+page.svelte`    | Article page wrapper (add banner rendering)                               |
| `apps/landing/src/lib/utils/docs-loader.ts`                           | Markdown rendering pipeline (no changes needed)                           |
