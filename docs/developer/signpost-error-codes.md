...
// For JSON API routes:
return json(
  { error: API_ERRORS.MY_NEW_ERROR.userMessage, error_code: API_ERRORS.MY_NEW_ERROR.code },
  { status: 400 }
);
...
import { type GroveErrorDef, logGroveError } from "@autumnsgrove/groveengine/errors";
...
*The signpost stands at every fork. Read the code, follow the path.*
