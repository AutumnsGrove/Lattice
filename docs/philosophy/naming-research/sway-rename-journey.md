...
*January 7, 2026 — A walk to find what Sway should become*
...
```
            ...
            └─────────────────────────────────────────┘
                    ...
                    │   ╰───╯      │
                    │              │
                    │  [SWAY?]     │ ← animation mode (needs name)
                    │  [Fern]      │ ← diagram mode
                    └──────────────┘
                         │
    ═══════════════════════════════════════════════
...
### Fundamentally:
- It's about **motion**—things moving
- It's about **propagation**—motion spreading through connections
- It's about **chains**—parent moves, children follow
- It's about **aliveness**—a static scene waking up

### What it DOES in the user's life:
- **Animates** — brings stillness to life
- **Connects motion** — chains of movement
- **Creates rhythm** — timing, delays, easing
- **Awakens** — the scene stirs
...
So I open Weave. I draw the connections—threads linking parent to child.
...
Not one thing swaying—many things moving *together*, motion traveling through them.
...
```
grove.place/weave → Rustle mode
```

- What it means: Wind moves through underbrush, leaves shift, branches whisper
- Motion *spreads*—it's not one thing, it's many things responding
...
```
grove.place/weave → Flutter mode
```
...
1. **It's about propagation** — A rustle isn't one leaf moving. It's motion spreading through connected things. Exactly what this mode does.

2. **It's a forest-floor phenomenon** — Pairs naturally with Fern (diagrams). Both are things you experience in the underbrush, at ground level.

3. **It implies aliveness** — "Something rustled in the leaves" — the forest waking up, responding.
...
5. **The spec already uses the concept**:
   > "jangle jangle" → rustling motion through chains
...
## Rustle
**Animation Mode** · *Within Weave*

A rustle is what happens when motion moves through connected things. Wind stirs the underbrush. Branches brush against leaves. Something shifts in the ferns, and the tremor travels. It's not one thing moving—it's everything responding to everything else.

Rustle is the animation mode within Weave. Place nature assets on a grid, draw threads between them, configure timing and easing—then wake the root node and watch motion propagate through the chain. Vines jangle. Branches tremble. Butterflies bob. Everything connected, everything alive.
