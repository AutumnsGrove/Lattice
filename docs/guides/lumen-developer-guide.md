...
## Songbird Prompt Injection Defense

Songbird is a three-layer defense system that automatically protects all Lumen requests against prompt injection attacks. Every user input passes through fast detectors before reaching the model. If any layer fails, the request is rejected immediately. Songbird runs as step 3 of the Lumen pipeline, after preprocessing and quota checks, and before the request reaches any provider.

The three layers are:

**Canary** (detection layer) runs a fast tripwire check. It asks the model to respond with exactly "SAFE" or "UNSAFE" based on whether the input tries to override instructions. If you see "HACKED" or anything else, the canary fails and the request stops immediately. Cost: ~$0.0001, latency ~50ms.

**Kestrel** (validation layer) performs semantic analysis. It checks whether the input matches expected patterns for the task context and cross-references against policies. For example, a chat task expects conversational messages, not system manipulation attempts. Kestrel responds with JSON: `{valid, confidence, reason}`. Cost: ~$0.0003, latency ~100ms.

**Robin** (execution layer) is the actual production inference. It only runs after Canary and Kestrel have verified the input is safe. If either protection layer fails, Robin never executes and the request is rejected with a generic error.

Total protection cost is ~$0.0004 per request. This translates to $0.40 per 1,000 requests or $4 per 10,000 requests. Cheap insurance against compromised responses.

### Which Tasks Activate Songbird

Songbird is enabled by default for all text-based tasks that process user input: `generation`, `chat`, `summary`, `code`, `image`, `reverie`, and `reverie-compose`. The `moderation` and `embedding` tasks skip Songbird because they are already safety or utility layers themselves and don't need injection protection.

Each task has task-specific `KestrelContext` configurations that define what legitimate input looks like. The `chat` task context expects conversational messages and rejects system manipulation attempts. The `reverie` context expects visual configuration requests and rejects code injection attempts. These contexts are defined in `libs/engine/src/lib/lumen/songbird.ts` and can be overridden per request.

### Configuration and Disabling

To customize or disable Songbird for a specific request, pass `SongbirdOptions` in the request:

```typescript
const response = await lumen.run({
  task: "generation",
  input: "Write a poem about aspens",
  tenant: tenantId,
  options: {
    songbird: {
      skipCanary: false,           // Set true to skip just the Canary layer
      context: customKestrelConfig, // Override default Kestrel context
      confidenceThreshold: 0.85,    // Raise or lower confidence bar (default 0.85)
    },
  },
}, tier);
```

Most of the time you should not need to customize these settings. If your application has unusual input patterns that trigger Songbird too aggressively, you can adjust the `confidenceThreshold` to 0.75 to be more permissive. If you're processing purely programmatic input (generated by another system, not user-submitted), you can set `skipCanary: true` to skip that layer only.

### When Songbird Rejects

If either Canary or Kestrel fails, Lumen throws a `SONGBIRD_REJECTED` error. The user sees a generic message like "Your message could not be processed. Please try rephrasing." No details about which layer failed or why. This prevents attackers from learning the defense mechanism.

Log the security event using content hash only, never the actual content. Watch for patterns: if legitimate requests start failing, the Kestrel context may need tuning or the confidence threshold might be too strict. An unexpectedly high rejection rate can indicate either configuration drift or an active attack against your application.

```typescript
try {
  const response = await lumen.run(request, tier);
} catch (err) {
  if (err.code === "SONGBIRD_REJECTED") {
    // Log event without exposing content
    console.error("Songbird rejected input", {
      layer: err.failedLayer, // "canary" or "kestrel"
      reason: err.reason,     // Kestrel reason if available
      tenant: tenantId,
    });
    // Return safe user message
    return { error: "Your message could not be processed. Please try rephrasing." };
  }
}
```
...
For the full technical specification, including implementation details, cost analysis per layer, and context patterns for each feature, read `docs/patterns/songbird-pattern.md`.
...
|------|--------------|-----------------|----------------|
| `generation` | DeepSeek V3.2 | OpenRouter | Kimi K2, Llama 3.3 70B |
| `summary` | DeepSeek V3.2 | OpenRouter | Kimi K2, Llama 3.3 70B |
| `chat` | DeepSeek V3.2 | OpenRouter | Kimi K2, Llama 3.3 70B |
