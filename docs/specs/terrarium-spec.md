...
The Terrarium workspace where you compose your scene:
...
#### Asset Palette Organization

The left sidebar organizes components by category for easy discovery:
...
When you select an asset on the canvas, a properties panel appears:
...
When an asset is selected, visual feedback helps you understand its state:

```
Canvas showing selected asset:

    ╭────────────────╮
    │                │
    │      🌲        │ ← Asset with selection handles
    │   ◆        ◆   │
    │                │ ◆ = corner drag handles
    │   ◆        ◆   │   (resize/scale)
    │                │
...
Operations:
  - Drag anywhere on asset to move
  - Drag corners to scale proportionally
  - Drag rotation handle to rotate
  - Right-click for context menu (dup, delete, send back)
  - Delete key removes asset
```
...
When you click Export, you choose how to save your creation:

```
┌────────────────────────────────────────────────────────┐
│  ✧ Export Terrarium Scene                          [×] │
├────────────────────────────────────────────────────────┤
│                                                        │
│  Export Format:                                        │
│  ┌─────────────────────────────────────────────────┐   │
...
│  Choose destination:                                   │
│  ┌─────────────────────────────────────────────────┐   │
│  │ (◉) As PNG Image (recommended)                  │   │
│  │     Scale: [1x] [2x retina] [4x]                │   │
│  │                                                 │   │
│  │ ( ) As Blog Decoration                          │   │
│  │     Zone: [Header     ▾]                        │   │
│  │     ~ Preview will show in Foliage ~            │   │
│  │                                                 │   │
│  │ ( ) As SVG File                                 │   │
│  │     For vector editing or documentation         │   │
│  └─────────────────────────────────────────────────┘   │
│                                                        │
│  ───────────────────────────────────────────────────── │
│  [ Cancel ]              [ Export ] [Export & Save ✦ ] │
│                                                        │
└────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────┐
│  ◎ Terrarium                     [Preview] [Grid 32px ▾] [Save]     │
├────────────────┬────────────────────────────────────────────────────┤
│  Asset Palette │                                                    │
│  ───────────── │     Canvas: Your Terrarium Scene                   │
│                │                                                    │
│ [🔍 Search...] │                                                    │
│                │                ╲    /                              │
│ ┌─ Trees    ┐  │                 \  /                               │
...
│ ┌─ Ground   ┐  │                                                    │
│ │ Mushroom ▶│  │        ┌──────────────────────────────┐            │
│ │ Rock      │  │        │ Lattice (selected)           │            │
│ │ Bush      │  │        ├──────────────────────────────┤            │
│ └───────────┘  │        │ Position: (320, 240)         │            │
│                │        │ Scale: [====●====] 1.2x      │            │
│ ┌─ Botanical┐  │        │ Rotation: [●────────] 15°    │            │
│ │ Vine      ▶│ │        │                              │            │
│ │ Fern       │ │        │ Props:                       │            │
│ │ Leaf       │ │        │ Material: [Oak ▾]            │            │
│ │ Acorn      │ │        │                              │            │
...
├────────────────┴────────────────────────────────────────────────────┤
│ Complexity: ████████░░░░░░░░░░░░ 85/200 pts   42% budget used       │
│ Hint: You have 115 points remaining. Add more complexity!           │
└─────────────────────────────────────────────────────────────────────┘
```
...
export function createTerrariumState() {
	...
	let selectedAsset = $derived(scene.assets.find((a) => a.id === selectedAssetId) ?? null);
...
```typescript
// libs/engine/src/lib/config/terrarium.ts
...
```typescript
// libs/engine/src/lib/utils/complexity.ts
 ...
 */
export function getAssetComplexity(asset: PlacedAsset, previousCost?: number): number {
...
```typescript
// libs/engine/src/lib/ui/components/terrarium/utils/export.ts
...
<script lang="ts">
	...
	let hasPartialFailure = $derived(loadStats.failed.length > 0 && loadStats.loaded > 0);
	...
	$effect(() => {
		...
		const componentNames = [...new Set(decoration.scene.assets.map((a) => a.componentName))];
...
{:else if loadError}
	<div class="decoration-error" role="alert">Failed to load decoration</div>
{:else}
	<div
	...
	>
		...
		{#if hasPartialFailure}
			<div class="decoration-partial-warning" role="status" aria-live="polite">
				<span class="sr-only">
					{loadStats.loaded} of {loadStats.total} decorations loaded. Missing: {loadStats.failed.join(
						", ",
					)}
				</span>
...
**Schema: `libs/engine/src/lib/schemas/decoration.ts`**
...
```typescript
// libs/engine/src/routes/api/terrarium/decorations/+server.ts
...
```
libs/engine/src/lib/ui/components/terrarium/
├── Terrarium.svelte
├── Canvas.svelte
├── AssetPalette.svelte
├── PlacedAsset.svelte
...
libs/engine/src/routes/terrarium/
├── +page.svelte
└── +layout.svelte
```
...
libs/engine/src/lib/ui/components/foliage/
└── DecorationPanel.svelte

libs/engine/src/routes/api/terrarium/
├── scenes/+server.ts
...
**Worker Implementation: `libs/engine/src/lib/workers/export.worker.ts`**
...
```typescript
// libs/engine/src/lib/utils/export-scene.ts
...
**Storage Implementation: `libs/engine/src/lib/storage/terrarium-db.ts`**
...
```typescript
// libs/engine/src/lib/ui/components/terrarium/autoSave.svelte.ts
...
<script lang="ts">
	...
	let timeAgo = $derived(lastSaved ? formatTimeAgo(lastSaved) : "Not saved");
</script>
...
```typescript
// libs/engine/src/lib/ui/components/terrarium/touchHandlers.ts
...
```typescript
// libs/engine/src/lib/utils/__tests__/complexity.test.ts
...
```typescript
// libs/engine/src/lib/storage/__tests__/terrarium-db.test.ts
...
```typescript
// libs/engine/e2e/terrarium.spec.ts
...
```typescript
// libs/engine/e2e/terrarium-a11y.spec.ts
...
```typescript
// libs/engine/src/lib/moderation/decoration-scanner.ts
...
```typescript
// libs/engine/src/lib/utils/quota.ts
...
```typescript
// libs/engine/src/lib/utils/sanitize.ts
...
**SvelteKit Hook: `libs/engine/src/hooks.server.ts`**
...
export const handle: Handle = async ({ event, resolve }) => {
	...
	const csp = event.url.pathname.startsWith("/terrarium") ? TERRARIUM_CSP : DEFAULT_CSP;
...
**Validation: `libs/engine/src/lib/utils/thumbnail-validator.ts`**
 ...
 */
export async function validateThumbnail(dataUrl: string): Promise<ThumbnailValidation> {
