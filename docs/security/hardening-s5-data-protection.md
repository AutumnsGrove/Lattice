...
`encryption.ts` is duplicated between `libs/engine` and `workers/timeline-sync`. Both are currently identical. Consider importing from engine to prevent drift, or add a cross-reference comment.
