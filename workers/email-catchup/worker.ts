...
export default {
	async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
		...
		if (!env.RESEND_API_KEY) {
			console.error("📧 RESEND_API_KEY not configured. Run: wrangler secret put RESEND_API_KEY");
...
async function processOverdueEmails(env: Env) {
	...
	for (const user of overdueUsers.results || []) {
		try {
			...
			if (!emailContent) {
				console.log(`📧 Skipping Day ${nextStage} for ${user.email}: template not available`);
				continue;
			}
...
function getNextStage(currentStage: number, audienceType: AudienceType): number {
