...
function buildStructuredFallback(emails: DigestEmail[]): string {
	...
	const sections: string[] = [`Your Email Digest — ${emails.length} unread emails\n`];
...
export function calculateNextAlarm(
	...
	const utcEquivalent = new Date(targetLocal.toLocaleString("en-US", { timeZone: "UTC" }));
	const tzEquivalent = new Date(targetLocal.toLocaleString("en-US", { timeZone: timezone }));
 ...
 */
export function getCategoryCounts(emails: DigestEmail[]): Record<string, number> {
