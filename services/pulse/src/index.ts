...
class WorkerSecretsManager {
	...
	private async decrypt(encryptedValue: string, key: CryptoKey): Promise<string | null> {
		try {
...
async function handleWebhook(request: Request, env: Env, url: URL): Promise<Response> {
	...
	if (!tenantId || !/^[a-zA-Z0-9_-]{1,128}$/.test(tenantId)) {
		return Response.json({ error: "Invalid or missing tenant_id" }, { status: 400 });
	}
	...
	if (!config) {
		return Response.json({ error: "Pulse not enabled for this tenant" }, { status: 404 });
	}
	...
	const webhookSecret = await secrets.getSecret(tenantId, "pulse_webhook_secret");

	if (!webhookSecret || webhookSecret.length < 32) {
		console.error(`[Pulse] Missing or weak webhook secret for tenant ${tenantId}`);
	...
	if (eventType === "ping") {
		const pingData = safeParseJson<{ zen?: string }>(body, {});
		return Response.json({ message: "pong", zen: pingData?.zen });
	}
