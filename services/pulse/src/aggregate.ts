	...
	for (const row of results.results) {
       ...
       ON CONFLICT(tenant_id, date, hour) DO UPDATE SET
         ...
         events = MAX(events, excluded.events)`,
			...
			.bind(crypto.randomUUID(), row.tenant_id, dateStr, hour, row.commits, row.events)
			.run();
	}

	console.log(`[Pulse Aggregate] Hourly rollup complete: ${results.results.length} tenants`);
}
...
export async function runDailyAggregation(env: Env): Promise<void> {
	...
	for (const row of tenantEvents.results) {
		...
		switch (row.event_type) {
			case "push":
				stats.commits += (data.commits as number) ?? 1;
				stats.linesAdded += (data.additions as number) ?? 0;
				stats.linesRemoved += (data.deletions as number) ?? 0;
				stats.filesChanged += (data.files_changed as number) ?? 0;
				break;
			case "pull_request":
	...
	console.log(`[Pulse Aggregate] Daily rollup complete: ${tenantStats.size} tenants`);
}

async function updateStreak(env: Env, tenantId: string, date: string): Promise<void> {
