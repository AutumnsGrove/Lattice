...
export async function createIncident(
	...
	const slug = slugify(data.title, { unique: true });
    ...
    `,
		...
		.bind(incidentId, data.title, slug, data.initialStatus, data.impact, data.type, now, now, now)
		.run();
	...
	for (const componentId of data.components) {
      ...
      `,
		...
		await db
			.prepare("UPDATE status_components SET current_status = ?, updated_at = ? WHERE id = ?")
...
export async function addIncidentUpdate(
	...
	await db
		.prepare("UPDATE status_incidents SET status = ?, updated_at = ? WHERE id = ?")
...
export async function updateIncident(
	...
	if (data.resolved !== undefined) {
		await db
			.prepare("UPDATE status_incidents SET resolved_at = ?, updated_at = ? WHERE id = ?")
...
export async function updateComponentStatus(
	...
	await db
		.prepare("UPDATE status_components SET current_status = ?, updated_at = ? WHERE slug = ?")
