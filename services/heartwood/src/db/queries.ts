...
/**
 * Look up a tenant by email in ENGINE_DB.
 * Used by session validation to include tenantId + subdomain in responses,
 * so downstream apps never need their own user→tenant lookups.
 *
 * Uses the existing idx_tenants_email index (~1-3ms).
 * Returns null silently on failure — tenant resolution must never block auth.
 */
export async function getTenantByEmail(
	...
	try {
		...
		// Tenant resolution is non-critical — never block auth
		return null;
	}
}
