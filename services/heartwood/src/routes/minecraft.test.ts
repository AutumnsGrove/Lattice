...
describe("GET /minecraft/status", () => {
	it("proxies to mc-control and returns response", async () => {
		...
		const app = createApp();
		const res = await app.request("/minecraft/status", { method: "GET" }, mockEnv);
...
describe("GET /minecraft/history", () => {
	it("passes query params to upstream", async () => {
		...
		const app = createApp();
		await app.request("/minecraft/history?limit=10&offset=5", { method: "GET" }, mockEnv);
...
describe("GET /minecraft/backups/:id/download", () => {
	it("streams file back on success", async () => {
		...
		const app = createApp();
		const res = await app.request("/minecraft/backups/123/download", { method: "GET" }, mockEnv);
