...
import { createMockEnv, createMockDb, createSequentialMockDb } from "../test-helpers.js";
...
describe("POST /cdn/upload", () => {
	...
	it("accepts audio files", async () => {
		...
		const formData = new FormData();
		formData.append("file", new File(["test"], "test.mp3", { type: "audio/mpeg" }));
	...
	it("accepts font files", async () => {
		...
		const formData = new FormData();
		formData.append("file", new File(["test"], "font.woff2", { type: "font/woff2" }));
...
describe("GET /cdn/files", () => {
	...
	it("returns files with CDN URLs", async () => {
		...
		expect(json.files[0].url).toBe("https://cdn.grove.place/images/test-image.png");
	});
	...
	it("respects limit query parameter", async () => {
		...
		const app = createApp();
		const res = await app.request("/cdn/files?limit=10", { method: "GET" }, mockEnv);
	...
	it("respects offset query parameter", async () => {
		...
		const app = createApp();
		const res = await app.request("/cdn/files?offset=25", { method: "GET" }, mockEnv);
	...
	it("filters files by folder", async () => {
		...
		const app = createApp();
		const res = await app.request("/cdn/files?folder=images", { method: "GET" }, mockEnv);
	...
	it("includes pagination metadata", async () => {
		const mockDb = createSequentialMockDb([{ all: { results: [] } }, { first: { total: 100 } }]);
		vi.mocked(createDbSession).mockReturnValue(mockDb as any);

		const app = createApp();
		const res = await app.request("/cdn/files?limit=25&offset=50", { method: "GET" }, mockEnv);
		...
		const mockDb = {
			prepare: vi.fn().mockReturnValue({
				all: vi.fn().mockResolvedValue({
					results: [{ folder: "images" }, { folder: "videos" }, { folder: "fonts" }],
				}),
			}),
		};
		vi.mocked(createDbSession).mockReturnValue(mockDb as any);
...
describe("DELETE /cdn/files/:id", () => {
	...
	it("returns 404 when file not found", async () => {
		...
		const app = createApp();
		const res = await app.request("/cdn/files/nonexistent-id", { method: "DELETE" }, mockEnv);
	...
	it("deletes file from R2 and database", async () => {
		...
		const app = createApp();
		const res = await app.request("/cdn/files/file-1", { method: "DELETE" }, mockEnv);
