...
describe("GET /status/incidents", () => {
	it("returns all incidents", async () => {
		...
		const app = createApp();
		const res = await app.request("/status/incidents", { method: "GET" }, mockEnv);
	...
	it("passes status filter when provided", async () => {
		...
		const app = createApp();
		await app.request("/status/incidents?status=active", { method: "GET" }, mockEnv);

		expect(statusQueries.getIncidents).toHaveBeenCalledWith(expect.anything(), "active");
	});
});
...
describe("GET /status/incidents/:id", () => {
	it("returns incident by ID", async () => {
		...
		const app = createApp();
		const res = await app.request("/status/incidents/inc-1", { method: "GET" }, mockEnv);
	...
	it("returns 404 when not found", async () => {
		...
		const app = createApp();
		const res = await app.request("/status/incidents/missing", { method: "GET" }, mockEnv);
...
describe("GET /status/components", () => {
	it("returns all components", async () => {
		...
		const app = createApp();
		const res = await app.request("/status/components", { method: "GET" }, mockEnv);
...
describe("GET /status/scheduled", () => {
	it("returns scheduled maintenance", async () => {
		...
		const app = createApp();
		const res = await app.request("/status/scheduled", { method: "GET" }, mockEnv);
