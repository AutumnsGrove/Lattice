...
// publishLatticeCmd is the parent for Lattice-specific publish operations.
var publishLatticeCmd = &cobra.Command{
	Use:   "lattice",
	Short: "Publish Lattice engine to npm and/or GitHub",
	Long:  "Publish @autumnsgrove/lattice to npm, create a GitHub Release, or both.",
}

// publishLatticeNpmCmd publishes Lattice to npm (delegates to gw publish npm).
var publishLatticeNpmCmd = &cobra.Command{
	Use:   "npm",
	Short: "Publish Lattice to npm with registry swap",
	Long:  publishNpmCmd.Long,
	RunE: func(cmd *cobra.Command, args []string) error {
		forwardNpmFlags(cmd)
		return publishNpmCmd.RunE(publishNpmCmd, args)
	},
}
...
var publishLatticeGithubCmd = &cobra.Command{
	Use:   "github",
	Short: "Create a GitHub Release with LLM-generated summary",
	Long: `Create a GitHub Release for Lattice:
  1. Bump version in package.json
  2. Commit and push version bump
  ...
  5. Create GitHub Release with summary`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		root := cfg.GroveRoot
		pkgJSONPath := filepath.Join(root, "libs", "engine", "package.json")
		...
		if !cfg.JSONMode {
			fmt.Print(ui.RenderInfoPanel("GitHub Release Plan", [][2]string{
				...
				{"Version", fmt.Sprintf("%s → %s", currentVersion, newVersion)},
				{"Tag", tag},
				{"Release", "GitHub Release with LLM summary"},
			}))
		}
		...
		if dryRun {
			if cfg.JSONMode {
				return printJSON(map[string]interface{}{
					...
					"version": newVersion,
					"tag":     tag,
					"steps":   []string{"Bump version", "Commit + push", "Create tag", "Generate summary", "Create GitHub Release"},
				})
			}
			ui.Info("DRY RUN — No changes made")
			return nil
		...
		// Bump version
		ui.Info(fmt.Sprintf("Bumping version to %s...", newVersion))
		...
		// Commit and push
		commitMsg := fmt.Sprintf("chore: release @autumnsgrove/lattice %s", tag)
		if _, err := exec.Git("add", pkgJSONPath); err != nil {
			return fmt.Errorf("git add failed: %w", err)
		}
		if _, err := exec.Git("commit", "-m", commitMsg); err != nil {
			return fmt.Errorf("git commit failed: %w", err)
		}
		pushResult, err := exec.Git("push")
		if err != nil {
			return fmt.Errorf("git push failed: %w", err)
		}
		if !pushResult.OK() {
			return fmt.Errorf("push failed: %s", pushResult.Stderr)
		}
		ui.Success(fmt.Sprintf("Version %s committed and pushed", newVersion))

		return createGithubRelease(root, newVersion, false)
	},
}
...
var publishLatticeBothCmd = &cobra.Command{
	Use:   "both",
	Short: "Publish to npm and create GitHub Release in one flow",
	Long: `Combined npm + GitHub Release workflow:
  1. All npm publish steps (bump, build, publish, commit, push)
  2. Create annotated git tag
  3. Generate release summary via LLM
  4. Create GitHub Release with summary`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Step 1: npm publish
		forwardNpmFlags(cmd)
		if err := publishNpmCmd.RunE(publishNpmCmd, args); err != nil {
			return fmt.Errorf("npm publish failed: %w", err)
		}

		if dryRun {
			ui.Info("Would also create GitHub Release")
			return nil
		}

		// Read version that npm just published
		cfg := config.Get()
		pkgData, err := readPackageJSON(filepath.Join(cfg.GroveRoot, "libs", "engine", "package.json"))
		if err != nil {
			return fmt.Errorf("failed to read version after npm publish: %w", err)
		}
		version, _ := pkgData["version"].(string)

		fmt.Println()
		ui.Info("npm publish complete — creating GitHub Release...")
		fmt.Println()

		// Step 2: GitHub release
		return createGithubRelease(cfg.GroveRoot, version, false)
	},
}

// forwardNpmFlags copies changed flags from src command to publishNpmCmd.
func forwardNpmFlags(src *cobra.Command) {
	for _, name := range []string{"bump", "version", "dry-run", "skip-build", "skip-commit", "token", "tag"} {
		if src.Flags().Changed(name) {
			_ = publishNpmCmd.Flags().Set(name, src.Flags().Lookup(name).Value.String())
		}
	}
}

// createGithubRelease creates a git tag, generates release summary, and creates
// a GitHub Release. The version parameter should NOT include the "v" prefix.
func createGithubRelease(root, version string, dryRun bool) error {
	cfg := config.Get()
	tag := "v" + version
	...
	if dryRun {
		if cfg.JSONMode {
			return printJSON(map[string]interface{}{
				"dry_run": true,
				"tag":     tag,
				"steps":   []string{"Create tag", "Generate summary", "Create GitHub Release"},
			})
		}
		ui.Info(fmt.Sprintf("DRY RUN — would create GitHub Release %s", tag))
		return nil
	}
	...
	if !pushResult.OK() {
		return fmt.Errorf("push tag failed: %s", pushResult.Stderr)
	}
	ui.Success("Tag pushed")

	// Generate release summary via LLM
	ui.Info("Generating release summary...")
	summaryScript := filepath.Join(root, "scripts", "generate", "generate-release-summary.sh")
	summaryResult, err := exec.RunInDirWithTimeout(2*time.Minute, root, "bash", summaryScript, tag)
	if err != nil {
		ui.Warning(fmt.Sprintf("Summary generation failed: %s — using basic summary", err))
	} else if !summaryResult.OK() {
		ui.Warning(fmt.Sprintf("Summary generation failed: %s — using basic summary", summaryResult.Stderr))
	} else {
		ui.Success("Release summary generated")
	}

	// Build release body from summary JSON
	summaryFile := filepath.Join(root, "snapshots", "summaries", tag+".json")
	body := buildReleaseBody(summaryFile, version)

	// Create GitHub Release
	ui.Info("Creating GitHub Release...")
	releaseResult, err := exec.RunInDir(root, "gh", "release", "create", tag,
		"--title", fmt.Sprintf("@autumnsgrove/lattice %s", tag),
		"--notes", body)
	if err != nil {
		return fmt.Errorf("failed to create release (is gh CLI installed?): %w", err)
	}
	if !releaseResult.OK() {
		return fmt.Errorf("release creation failed: %s", releaseResult.Stderr)
	}
	...
	fmt.Println()
	ui.Success(fmt.Sprintf("GitHub Release %s created", tag))
	if releaseResult.Stdout != "" {
		ui.Muted(strings.TrimSpace(releaseResult.Stdout))
	}
	fmt.Println()
...
// releaseSummary represents the JSON output from generate-release-summary.sh.
type releaseSummary struct {
	Summary string `json:"summary"`
	Stats   struct {
		TotalCommits int `json:"totalCommits"`
...
// buildReleaseBody constructs the GitHub Release body from a summary JSON file.
// Falls back to a basic body if the file doesn't exist or can't be parsed.
func buildReleaseBody(summaryFile, version string) string {
	fallback := fmt.Sprintf("## @autumnsgrove/lattice v%s\n\nSee the [roadmap](https://grove.autumn.pub/roadmap) for details.", version)
	...
	if len(s.Scopes) > 0 {
		...
		b.WriteString(fmt.Sprintf("Areas: %s | ", strings.Join(scopes, ", ")))
	}
	b.WriteString(fmt.Sprintf("*%d total commits* | ", s.Stats.TotalCommits))
	b.WriteString("[npm](https://npm.pkg.github.com/package/@autumnsgrove/lattice) | ")
	b.WriteString("[changelog](https://grove.autumn.pub/roadmap)")
...
func init() {
	...
	// publish lattice npm (same flags as publish npm)
	publishLatticeNpmCmd.Flags().String("bump", "", "Version bump type (patch, minor, major)")
	publishLatticeNpmCmd.Flags().String("version", "", "Explicit version string (e.g., 1.0.0)")
	publishLatticeNpmCmd.Flags().Bool("dry-run", false, "Show plan without executing")
	publishLatticeNpmCmd.Flags().Bool("skip-build", false, "Skip the build step")
	publishLatticeNpmCmd.Flags().Bool("skip-commit", false, "Skip git commit and push")
	publishLatticeNpmCmd.Flags().String("token", "", "npm auth token (or set NPM_TOKEN env var)")
	publishLatticeNpmCmd.Flags().String("tag", "", "npm dist-tag (e.g., beta, next)")
	publishLatticeCmd.AddCommand(publishLatticeNpmCmd)
